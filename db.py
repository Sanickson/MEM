...
import builtins
if builtins.__dict__.get('_') is None:
    def _(message):
        return message
...
def fdbDataListWinToUtf(datalist):
    newdatalist = []
    for item in datalist:
        newdatalist.append(fdbRowMappingWinToUtf(item))
    return newdatalist

def fdbRowMappingWinToUtf(item):
    if not item:
        return item
    newitem = []
    for key in list(item.keys()):
        if bytes == type(item[key]):
            newitem.append(item[key].decode('windows-1251'))
            #newitem.append(str(item[key], "windows-1251").encode('utf-8'))
        ##elif str == type(item[key]):
        ##    newitem.append(item[key].encode('utf-8'))
        else:
            newitem.append(item[key])
    return dbEngine.fbcore._RowMapping(item._description, newitem)
...
systems_FirstParentFilledIdBase = {}
def getIdSystemOfFirstParentFilledIdBase(id_system, conEngine=None):
    if int(id_system) == -1:
        return '-1'
    if str(id_system) in systems_FirstParentFilledIdBase:
        return systems_FirstParentFilledIdBase[str(id_system)]
    ...
    if d['higher'] is None:
        if d['id_base'] is None:
            systems_FirstParentFilledIdBase[str(id_system)] = '-1'
            ...
            systems_FirstParentFilledIdBase[str(id_system)] = str(id_system)
            return str(id_system)
    if not d['id_base']:
        parentId = getIdSystemOfFirstParentFilledIdBase(d['higher'], conEngine=conEngine)
        systems_FirstParentFilledIdBase[str(id_system)] = parentId
        return parentId
    systems_FirstParentFilledIdBase[str(id_system)] = str(id_system)
    return str(id_system)
...
systems_databases = {}
def getDataBaseBySystemId(id_system, conEngine):
    if id_system in systems_databases:
        return systems_databases[id_system]

    dbparams = dbExec(
        sql='select B.* from ENGINE_SYSTEMS S LEFT JOIN ENGINE_BASES B ON S.ID_BASE=B.ID_BASE WHERE S.ID_SYSTEM=?',
        params=(id_system,),
        fetch='one',
        id_system=-1,
        con=conEngine)
    ...
    systems_databases[id_system] = dbparams
    return dbparams
...
def addConnectToDict(id_system, conEngine=None):
    ...
    if id_system == '-1':
        ...
        try:
            ...
            if sys.exc_info()[0] == dbEngine.OperationalError \
    ...
    if idParentSystem == id_system:
        ...
        dbparams = getDataBaseBySystemId(id_system=id_system, conEngine=conEngine)
        ...
        try:
            th_dic['db'][id_system] = dbEngine.connect(
                                          ...
                                          charset=dbparams['db_charset'] #None
                                          )
        except:
            if sys.exc_info()[0] == dbEngine.OperationalError \
            and sys.exc_value[0] == -902 \
...
def db_connectOpen(id_system, conEngine=None, forceReconnect=False):
    ...
    if 'db' not in th_dic:
        th_dic['db'] = {}
    if 'tr_cnt' not in th_dic:
        th_dic['tr_cnt'] = {}
    if 'id_top_system' not in th_dic:
...
def db_connectClose(th_number):
    def closeBy_db_dict(db):
        ...
        if db:
            for id_top_system, conn in db.items():
                try:
                    conn.close()
                except:
                    pass
...
def main_th_connect_close():
    ...
    if th.__class__.__name__ == "_MainThread":
        ...
        for key, val in list(th_dic['id_top_system'].items()):
            if val == '-1':
...
def dbExec(sql, params=(), fetch='many', id_system=None, conEngine=None, con=None, log=True, congroup=None):
    #print "---db.dbExec STEP1---: %(sql)s"*5%vars()
    lst = []
    for i in params:
        if type(i) == bytes:
            #lst.append(unicode(i, "utf-8").encode(toCharset))
            lst.append(str(i, "utf-8"))
        elif type(i) == str:
            #lst.append(i.encode(toCharset))
            lst.append(i)
        else:
            lst.append(i)
    params = lst
    ...
    try:
        ...
        try:
            ...
            # if con:
            #     c = dbEngine.ConnectionGroup.cursor(connection=con1)
            # else:
            #     c = con1.cursor()
            if congroup:
                c = congroup.cursor(con1)
            else:
                c = con1.cursor()
            ...
            try:
                try:
                    ...
                    if fetch == 'one':
                        dat = fdbRowMappingWinToUtf(c.fetchonemap())
                    elif fetch == 'many':
                        if cfg.fetch_limit is None:
                            dat = fdbDataListWinToUtf(c.fetchallmap())
                        else:
                            dat = fdbDataListWinToUtf(c.fetchmanymap(cfg.fetch_limit))
                    elif fetch == 'all':
                        dat = fdbDataListWinToUtf(c.fetchallmap())
                except:
                ...
                if not con:
                    try:
                        ...
                        if not con1.closed and len(con1.get_active_transaction_ids())>0:
                            con1.rollback()
...
def get_bases_info(id_base=None, conEngine=None, log=True):
    ...
    if id_base is None:
        ...
        for dic_ro in dbs_ro:
            dic = {}
            #по столбцам
            for key, val in list(dic_ro.items()):
                dic[key] = val
            dbs.append(dic)
        for db in dbs:
        ...
        #по столбцам
        db_info = {}
        for key, val in list(db_info_ro.items()):
            db_info[key] = val
        #обязательно перед изменением DB_IP
        if db_info['DB_IP'] == '-1':
