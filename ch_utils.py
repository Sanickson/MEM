...
import db
import Cheetah
cheetahNeedEncode = Cheetah.Version >= '2.1'
cheetahNeedEncode = False


def strInWin1251(str):
    prev_ord = 0
    maybe1251 = False
    for simb in str:
        simb_ord = ord(simb)
        ...
        if simb_ord == 208 or simb_ord == 209:
            ...
            if 223 < simb_ord < 256 or simb_ord == 184:  # lowercase
                return True
            if 191 < simb_ord < 224 or simb_ord == 168:  # uppercase
                return True
        prev_ord = simb_ord
    return maybe1251
...
def dataElemToUtf(d):
    def conv(d, i):
        if type(d[i]) == bytes and strInWin1251(d[i]):
            d[i] = str(d[i], "windows-1251").encode('utf-8')
        elif type(d[i]) == db.dbEngine.fbcore._RowMapping:
            d[i] = db.fdbRowMappingWinToUtf(d[i])
        elif type(d[i]) == list:
            dataElemToUtf(d[i])
        elif type(d[i]) == dict:
            dataElemToUtf(d[i])
...
def strToCheetah(str):
    if cheetahNeedEncode and strInWin1251(str):
        return str(str, "windows-1251").encode('utf-8')
    else:
        return str


def drawTemplate(templ='index', data=None, draw_wo_login=False, module=None, path='templates'):
    """Возвращает cheetah-обработанное html-представление шаблона. Если в качестве шаблона templ используется строка
    ...
    if not draw_wo_login and cpu.getUserVar('uid') is None:
        ...
        if user_init_class:
            for (key, val) in list(user_init_class.__dict__.items()):
                data.append({key: val})
    if cfg.TEST_MESSAGE:
    ...
    if cheetahNeedEncode:
        dataElemToUtf(data)

    if id_system:
        if id_system in si.systems_params:
            ...
            data.append({'system_name': strToCheetah(si.systems_params[id_system]['system_name'])})
            data.append({'top_system_name': strToCheetah(si.systems_params[id_system]['top_system_name'])})
    #else:
    ...
    for s in list(si.systems_params.keys()):
        if s != -1:
...
def import_tmpl(tmpl_name='index', path='templates'):
    """Импортирует и возвращает шаблон tmpl_name по относительному пути path. Пусть может быть задан либо относительно
