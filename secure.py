    ...
    def sec():
        ...
        if pi_n not in grantAccessRefs and not is_static_resource(pi):
            ...
            if rcr not in cpu.getUserVar('granted_system_refs') \
                ...
                return cpu.goToError(_('У Вас нет прав на доступ к ресурсу "%s" либо ресурс отсутствует' % pi_n), from_root=True)
...
def pwd_hash(password):
    """Возвращает хэш пароля pwd согласно алгоритму из настроек.
...
def verify_user(login, password, pwd_is_hash=False):
    """Валидация логина и пароля - возвращает объект usr, который в поле ERROR_CODE содержит код ошибки(0-успех),
...
def term_generate_barcode(login):
    ...
    minutes_dec = dtu.MinutesBetweenDateTimes(dtBegin = datetime.datetime(2010, 0o1, 0o1, 0, 0), dtEnd = now)
...
def term_barcode_mx_date(barcode):
    ...
    d0 = datetime.datetime(2010,1,1,0,0,0)
    return d0 + datetime.timedelta(minutes=minutes)
