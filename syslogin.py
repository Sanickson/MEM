...
class SysLogin(BasePage):
    ...
    @filterXSS([])
    def index(self, termnum=None, forceiface=0, force_browser=0):
        ...
        if forceiface != 0:
            self.setSesVar('forceiface', forceiface)
        else:
            pass
        ...
        if cpu.isTSD():
            ...
            if termnum is not None:
                ...
                pass
            if not self.getSesVar('termnum'):
                return cpu.goToError(mes=_('Не установлен номер оборудования'), from_root=True)
            ...
            params = {'termname': termname, 'mes': mes}
            if term['BEGIN_TIME'] and not term['END_TIME']:
                params['termusername'] = termusername
            return self.drawTemplate(templ=login_index_term, data=[params], draw_wo_login=True)
        elif cpu.isMobile():
            self.setSesVar('termiduser', None)
            self.setSesVar('termusername', None)
            self.setSesVar('termnum', None)
            self.setSesVar('termname', None)
            return self.drawTemplate(templ=login_index_mob, data=[{'mes': None}], draw_wo_login=True)
        else:
            self.setSesVar('termiduser', None)
            self.setSesVar('termusername', None)
            self.setSesVar('termnum', None)
            ...
            if not parsed_agent or 'browser' not in parsed_agent:
    ...
    def verCompare(self, ver1, ver2):
        ver1Spl = ver1.split('.')
        ver2Spl = ver2.split('.')
        n = len(ver1Spl) if len(ver1Spl) < len(ver2Spl) else len(ver2Spl)
        i = 0
        while i < n:
            if int(ver1Spl[i]) < int(ver2Spl[i]):
                return 2
            elif int(ver1Spl[i]) > int(ver2Spl[i]):
                return 1
            i += 1
        return 0
    ...
    def allowed_ua(self, parsed_agent):
        ...
        for ua in cfg.UA_ALLOWED:
            if parsed_agent['browser']['name'] == ua['user-agent']:
                ...
                if self.verCompare(parsed_agent['browser']['version'], ua['min_ver']) in (0, 2):
                    res = False
                    break
                else:
                    res = True
    ...
    def draw_system_list(self, username, password, first_time=0, pwd_is_hash='0'):
        """Валидация логина и пароля и проброс вглубь дерева подсистем.
            ...
            '130' - "Доступ запрещён по данным Active Directory: неверный логин или пароль, пользователь отключён или заблокирован"
            '140' - "Ошибка Active Directory"
            '150' - "Пользователь прошёл проверку AD, но еще не импортировался в RBS"
            REDIRECT_CODE: 0 - REDIRECT_DATA содержит url для проброса (перейти в него изнутри движка так: cp_utils.HTTPRedirect(REDIRECT_DATA))
                           1 - REDIRECT_DATA содержит отрисованную html-страницу со списком систем, если ERROR_CODE==0 или страницу авторизации с ошибкой, если ERROR_CODE!=0
        ...
        # if getattr(cfg, 'AD_INTEGRATION', False):
        #     l = ldap.initialize(cfg.AD_SERVER)
        #     try:
        #         try:
        #             l.bind_s(username + '@' + cfg.AD_DOMAIN, password)
        #         except ldap.INVALID_CREDENTIALS:
        #             result['ERROR_CODE'] = 130
        #             result['ERROR_MSG'] = _('Доступ запрещён в Active Directory: неверный логин/пароль, пользователь отключён или заблокирован!')
        #             result['REDIRECT_CODE'] = 1
        #             result['REDIRECT_DATA'] = self.drawTemplate(templ=login_index,
        #                                                         data=[{'username': username, 'mes': result['ERROR_MSG']},
        #                                                               {'langs': cpu.getConfigLangs()}],
        #                                                         draw_wo_login=True)
        #             return result
        #         except Exception, e:
        #             result['ERROR_CODE'] = 140
        #             str_e = str(e)
        #             if "Can't contact LDAP server" in str_e:
        #                 str_e = 'Невозможно соединиться с AD-сервером'
        #             result['ERROR_MSG'] = _('Ошибка Active Directory:') + '<br>' + str_e
        #             result['REDIRECT_CODE'] = 1
        #             result['REDIRECT_DATA'] = self.drawTemplate(templ=login_index,
        #                                                         data=[{'username': username, 'mes': result['ERROR_MSG']},
        #                                                               {'langs': cpu.getConfigLangs()}],
        ...
        #         l.unbind()
        #
        #     usr = adm.userInfo(adm.userUidByLogin(username))
        #     if usr is None:
        #         result['ERROR_CODE'] = 150
        #         result['ERROR_MSG'] = _('Пользователь еще не импортирован в RBS!') + '<br>' + 'Подождите немного и войдите снова.'
        #         result['REDIRECT_CODE'] = 1
        #         result['REDIRECT_DATA'] = self.drawTemplate(templ=login_index,
        #                                                     data=[{'username': username, 'mes': result['ERROR_MSG']},
        #                                                           {'langs': cpu.getConfigLangs()}],
        ...
        if usr is None:
            result['ERROR_CODE'] = 150
            result['ERROR_MSG'] = _(
                'Пользователь еще не импортирован в RBS!') + '<br>' + 'Подождите немного и войдите снова.'
            result['REDIRECT_CODE'] = 1
            result['REDIRECT_DATA'] = self.drawTemplate(templ=login_index,
                                                        data=[{'username': username, 'mes': result['ERROR_MSG']},
                                                              {'langs': cpu.getConfigLangs()}],
        ...
        if usr['AUTHOR_AD'] == 1:
            ...
            conn = ldap3.Connection(server, user=username + '@' + cfg.AD_DOMAIN, password=str(password, "windows-1251").encode("utf-8"), raise_exceptions=True)
            conn.open()
            try:
                conn.bind()
                ...
                result['ERROR_CODE'] = 130
                result['ERROR_MSG'] = _(
                    'Доступ запрещён: неверный логин/пароль, пользователь отключён или заблокирован!')
                result['REDIRECT_CODE'] = 1
                result['REDIRECT_DATA'] = self.drawTemplate(templ=login_index,
                                                            data=[
                                                                {'username': username, 'mes': result['ERROR_MSG']},
                                                                {'langs': cpu.getConfigLangs()}],
                                                            draw_wo_login=True)
                return result
        ...
        if getattr(cfg, 'SKUD_INTEGRATION', False) and cpu.isPC() and getattr(cfg, 'SKUD_CHECK_PC', False):
            skud_res = db.dbExecDirect(sql='select ALLOW, ERROR_MSG from %s(?)' % cfg.SKUD_MAJOR_DB_ACCESS_PROC,
                                    params=[usr['ID_USER']],
                                    fetch='one', id_base=db.getIdMajorDb())
            allow = skud_res['ALLOW']
            if not allow:
                result['ERROR_CODE'] = 110
                result['ERROR_MSG'] = _('Доступ запрещён по данным СКУД!') + '<br>' + skud_res['ERROR_MSG']
                result['REDIRECT_CODE'] = 2
                result['REDIRECT_DATA'] = result['ERROR_MSG']
                return result
        ...
        #открываем сессию
        old_web_session_id, error_msg = sessions.session_open(uid=usr['ID_USER'], fio=usr['FIO'], equip_kind=cpu.equipKind())
        ...
        create_user_session_class(login=username, uid=usr['ID_USER'], userfio=usr['FIO'], email=usr['EMAIL'],
                                  ...
                                  is_superadmin=usr['IS_SUPERADMIN'], is_localadmin=usr['IS_LOCALADMIN'])
        ...
        if not first_time and probros and go_url:
            ...
            link_found = False
            FULL_REF_NAMES = [system['FULL_REF_NAME'] for system in systems['datalist']]
    ...
    def draw_system_list_term(self, code, termnum):
        ...
        if getattr(cfg, 'SKUD_INTEGRATION', False) and \
            ...
            if not allow:
                if cpu.isTSD():
                    return cpu.goToError(_('Доступ запрещён по данным СКУД!') + '<br>' + skud_res['ERROR_MSG'])
                else:
                    return self.exception_inner(tmpl_name='exception_skud_access_mob',
                                                data={'userfio': usr['FIO'], 'skud_error_msg': skud_res['ERROR_MSG']})
    ...
    def login(self, username='', password='', mes='', first_time=0, pwd_is_hash='0'):
        """Аутентификация. Возвращает html-код с ошибкой, списком систем или редиректит в зависимости от результата
    ...
    def login_ajax(self, username='', password='', pwd_is_hash='0'):
        """Ajax-авторизация. Возвращает результат вызова self.draw_system_list в json-формате.
    ...
    def login_term(self, code='', mes=''):
        if mes != '':
            return self.drawTemplate(templ=login_index_mob if cpu.isMobile() else login_index_term,
                                     data=[{'mes':mes}],
                                     draw_wo_login=True)
        if code == '':
            return self.drawTemplate(templ=login_index_mob if cpu.isMobile() else login_index_term,
        ...
        if termnum is None and cpu.isTSD():
            return self.drawTemplate(templ=login_index_term,
                                     data=[{'mes': _('Утерян номер оборудования')}],
                                     draw_wo_login=True)
    ...
    def login_change_passwd(self, username='', passwd='', passwd2='', uid=None):
        """Изменение временного пароля пользователя. Пароль и повторённый пароль проверяются на заполненность,
