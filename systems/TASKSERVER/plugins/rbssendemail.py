...
from email.mime.base import MIMEBase # Модуль формирования сообщений из нескольких частей
...
class Email():
    ...
    def __init__(self, parent_class, **kwargs):
        ...
        self.coding = kwargs.get('coding', 'windows-1251')
        self.Content_Type = kwargs.get('Content_Type', 'text/plan')
    ...
    def send_email(self):
        ...
        if self.connect():
            ...
            msg = 'From: %s\nTo: %s\r\nContent-Type: %s; charset="%s"\r\nSubject: %s\n\n%s' \
                  % (self.from_address, ','.join(self.to_address), self.Content_Type, self.coding, self.subject,
                     self.message)
            ...
            try:
                ...
                message = c.m_e_smtp_send % self.smtp_server + str(exc)
                self.parent_class.log_file(message,
                                           terms=2,
                                           save_log_db=True)
                self.result_class = c.plugin_error
