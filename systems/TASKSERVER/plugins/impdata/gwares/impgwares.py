...
class Gwares():
    ...
    def __init__(self, parent_class, obj=None):
        ...
        if parent_class.gwares_check_assortment == '1':
            suppliers = obj.find(self._parent_class.name_space + 'suppliers')
    ...
    def _get_characteristic(self):
        ...
        if self.__obj is not None:
            characteristics = self.__obj.find(self._parent_class.name_space + 'characteristics')
    ...
    def save(self):
            ...
            if res['status'] == k.kr_sql_error:
                ...
                if self.waresid is not None:
                    if self.__type_import == 1:
                        ...
                        ''' ШК '''
                        self.barcode(self.__obj.find(self._parent_class.name_space + 'barcodes'), self._parent_class.create_ttx)
        ...
        if self.__type_import == 2 and self._delete_marker != '1':
            ...
            wares_units = self.__obj.find(self._parent_class.name_space + 'waresunits')
    ...
    def barcode(self, barcodes, create_ttx, unit=None, unit_uuid=None):
        ...
        #barcodes = self.__obj.find(self._parent_class.name_space + 'barcodes')
            ...
            if wares_barcode:
                ...
                if len(self._characteristic) == 0:
                    ...
                    for itm in wares_barcode:
                        sql_params = [self._code, itm['unit'] if not unit_uuid else None, itm['barcode'], itm['factor'],
                            ...
                            for unit_name in unit_list:
                                for itm in wares_barcode:
                                    if (itm['characteristic'] is None or itm['characteristic_id'] is None)and itm['unit'] == unit_name:
                                        sql_params = [self._code, itm['unit'] if unit_uuid else None, itm['barcode'], itm['factor'],
                                                      itm['uweight'], itm['ulength'], itm['uheight'], itm['uwidth'],
    ...
    def _set_slice(self):
        ...
        """
        if self.__obj is not None:
            slices = self.__obj.find(self._parent_class.name_space + 'sliced')
    ...
    def _wares_unit(self, wares_units, type_wares_unit):
        ...
        for obj in wares_units:
            if self._xml_get_value(obj, 'type') == type_wares_unit:
                unit = None
                unit_uuid = self._xml_get_value(obj, 'unit', flag='N')
                unit = unit_uuid
                if unit or unit_uuid:
                    ...
                    if len(self._characteristic) == 0:
                        ...
                        if self.__save_wares_unit(sql_params, unit):
                            ''' находим ШК '''
                            barcodes = obj.find(self._parent_class.name_space + 'barcodes')
                            if barcodes is not None:
    ...
    def __save_wares_tax_sale(self):
        ...
        nds_wares = self.__obj.find(self._parent_class.name_space + self._parent_class.xml_tax_name_tag)
    ...
    def __save_assortment(self):
        ...
        if self._parent_class.gwares_create_assortment == '1':
            suppliers = self.__obj.find(self._parent_class.name_space + 'suppliers')
            ...
            assort_rc = self.__obj.find(self._parent_class.name_space + 'assort_rc')
    ...
    def __save_wares_type(self):
        ...
        if self._parent_class.gwares_create_type == '1':
            statuses = self.__obj.find(self._parent_class.name_space + 'statuses')
    ...
    def __save_wares_mpo(self):
        ...
        if self._parent_class.gwares_create_mpo == '1':
            mpos = self.__obj.find(self._parent_class.name_space + 'mpos')
            if mpos is not None:
                if mpos is not None:
                    for obj in mpos:
