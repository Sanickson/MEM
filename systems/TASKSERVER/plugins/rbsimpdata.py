...
class Plugin(BP.BasePlugin):
    ...
    def run(self):
        ...
        self.name_space = self.ParserXML(self.taskparamsxml, 'name_space')
        if not self.name_space: self.name_space = ''

        ## Для импорта справочника объектов

        ''' импорт форматов '''
        formats = xmlfile.find(self.name_space + 'formats')
        if formats is not None:
            self.import_formats(formats)

        ''' импорт банков '''
        banks = xmlfile.find(self.name_space + 'banks')
        if banks is not None:
            self.import_banks(banks)

        '''  импорт контрагентов '''
        customers = xmlfile.find(self.name_space + 'customers')
        if customers is not None:
            self.import_customers(customers)

        '''  импорт производителей '''
        makers = xmlfile.find(self.name_space + 'makers')
        if makers is not None:
            self.import_makers(makers)

        ''' импорт должностей '''
        dolgns = xmlfile.find(self.name_space + 'dolgns')
        if dolgns is not None:
            self.import_dolgns(dolgns)

        ''' импорт сотрудников '''
        # mans = xmlfile.find(self.name_space + 'mans')
        # if mans is not None:
        #     self.import_mans(mans)

        # импорт пользователей
        users = xmlfile.find(self.name_space + 'users')
        if users is not None:
            self.import_users(users)

        ''' импорт подразделений '''
        departmens = xmlfile.find(self.name_space + 'departmens')
        if departmens is not None:
            self.import_departmens(departmens)

        ''' импорт складов '''
        warehouses = xmlfile.find(self.name_space + 'warehouses')
        if warehouses is not None:
            self.import_warehouses(warehouses)

        ''' фирмы '''
        firms = xmlfile.find(self.name_space + 'firms')
        if firms is not None:
            self.import_firms(firms)

        '''Импорт автомобилей'''
        cars = xmlfile.find(self.name_space + 'cars')
        if cars is not None:
            self.ImportCars(cars)

        ## Для импорта справочника товаров
        # импорт периодов
        periods = xmlfile.find(self.name_space + 'periods')
        if periods is not None:
            self.ImportPeriods(periods)

        # импорт налоговых ставок
        taxs = xmlfile.find(self.name_space + 'taxs')
        if taxs is not None:
            self.ImportTaxs(taxs)

        ''' импорт справочника ед измерения '''
        if self.createunits == '1':
            units = xmlfile.find(self.name_space + 'units')
            if units is not None:
                self.import_units(units)

        # импорт товаров
        gwares = xmlfile.find(self.name_space + 'gwares')
    ...
    def import_recipe(self, recipes):
        ...
        for obj in recipes:
            ...
            recipe.save()
            if recipe.recipe_id:
                wares = obj.find(self.name_space + 'wares')
    ...
    def ImportCars(self, cars):
        ...
        for obj in cars:
            name =  self.xml_get_value_by_attr(obj, 'name')
            status =  self.xml_get_value_by_attr(obj, 'status')
            id =  self.xml_get_value_by_attr(obj, 'id')

            res = self.ExecuteSQL('execute procedure RBS_Q_CARS_INSSEL(?,?,?,?)',
                                  sqlparams = [name, status, id, 'I'],
                                  fetch='one',
                                  ExtVer=True)
            if res['status'] == krconst.kr_sql_error:
                self.LogFile(krconst.kr_message_error_importcars % name + krconst.kr_term_double_enter)
        ...
        for obj in waresunits:
            if self.xml_get_value_by_attr(obj, 'type') == type:
                ...
                if res['status'] == krconst.kr_sql_error:
                    ...
                    # находим ШК
                    waresbarcode = []
                    barcodes = obj.find(self.name_space + 'barcodes')
    ...
    def CreateShedule(self, customers):
        ...
        for obj in customers:
            ...
            code = self.xml_get_value_by_attr(obj, 'code')
            schedules = obj.find(self.name_space + 'schedules')
            ...
            if customercode:
                ...
                if res['status'] == krconst.kr_sql_error:
                    ...
                    if res['datalist']:
                        ...
                        gwares = obj.find(self.name_space + 'gwares')
                        if gwares is not None:
                            for wares in gwares:
                        ...
                        objects = obj.find(self.name_space + 'objects')
        ...
        if DepartmentIsInstall:
            ...
            for groupmeneger in groupmenegers:
                ...
                if waresgrid:
                    ...
                    managers = groupmeneger.find(self.name_space + 'managers')
        ...
        for obj in wsets:
            ...
            if typewset == 'A':
                ...
                if resData:
                    ...
                    objbondwsets = obj.find(self.name_space + 'objbondwsets')
                    ...
                    # состав набора
                    waresinsets = obj.find(self.name_space + 'waresinsets')
    ...
    def import_contracts(self, contracts):
        ...
        for contract in contracts:
            ...
            if res['status'] == krconst.kr_sql_error:
                ...
                makers = contract.find(self.name_space + 'makers')
