...
class Plugin(Bp.BasePlugin):
    ...
    def run(self):
        ...
        if self.queueparamsxml:
            ...
            self.name_printer = self.ParserXML(self.queueparamsxml, 'printer')

            if not self.name_printer:
    ...
    def create_print_report_file(self, html, file_name):
        ...
        sql_text = 'select p.zoom from wm_printers p where p.name=?'
        res = self.execute_sql(sql_text,
                               sql_params=[self.name_printer],
                               fetch='one'
                               )
        if res['status'] == krconst.kr_sql_error:
            self.LogFile(res['message'])
        else:
            if res['datalist']['zoom']:
                zoom_str = '--zoom ' + str(res['datalist']['zoom']) + ' '

        if self.type_convert == 'pdf':
            if dimension:
                dimension_str = ' --page-width ' + dimension['width'] + \
                                'mm --page-height ' + dimension['height'] + 'mm '
            convert_str = 'xvfb-run /usr/bin/wkhtmltopdf -L 0 -T 0 -R 0 -B 0 ' + zoom_str + dimension_str + tmp_file_html + ' ' + file_name + ' >> ./log/stdout.log 2>&1'
    ...
    def print_file_report(self, file_name_report):
        ...
        if self.parent.k_conf.os_platform == 'linux':
            self.log_file('lp -d ' + self.name_printer + ' ' + file_name_report + krconst.kr_term_double_enter)
            str_print = 'lp -d ' + self.name_printer + ' ' + file_name_report
            return self.print_file_report_linux(str_print)
            #try:
            #    self.log_file('lp -d ' + self.name_printer + ' ' + file_name_report + krconst.kr_term_double_enter)
            #    os.system('lp -d ' + self.name_printer + ' ' + file_name_report)
            #except:
            #    self.LogFile(krconst.kr_message_error_errorreportpdfprint % file_name_report)
            #    self.log_file('lp  -o page-bottom=0 -o page-top=0 -o page-left=0 -o page-right=0 -o media=a4 -o scaling=100 -d ' + self.name_printer + ' ' + file_name_report + krconst.kr_term_double_enter)
            #    os.system('lp  -o page-bottom=0 -o page-top=0 -o page-left=0 -o page-right=0 -o media=a4 -o scaling=100 -d ' + self.name_printer + ' ' + file_name_report)
            #    self.result['result'] = krconst.plugin_error
            #    return False
        return True
