...
class Plugin(Bp.BasePlugin):
    ...
    def run(self):
        ...
        try:
            os.system('chmod 777 ' + file_name_exp_temp)
    ...
    def sql_res_to_str(self, row):
        ...
        for key in list(row.keys()):
            if key != self.detail_params:
                row_str = row[key]
                if type(row[key]).__name__ == 'bytes':
                    row_str = row_str.decode("utf-8", errors="ignore")
                str_result = str_result + row_str + c.kr_term_enter
        return str_result
