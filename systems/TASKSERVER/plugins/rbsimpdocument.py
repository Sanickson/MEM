...
class Plugin(Bp.BasePlugin):
    ...
    def run(self):
        ...
        if 'loadplan' in self.filenames:
            if not self.name_space == '':
                self.name_space = '{WMSRBSLOADPLAN}'
            self.load_plan = True
        ...
        '''
        if xml_file.getroot().tag == self.name_space + 'root':
            ''' в документе обязательно должен присутствовать тег typedoc
                по нему будем определять какую секцию конфига читать
            '''

            if self.load_plan:
                self.section = 'LOADPLAN'
                documents = xml_file.find(self.name_space + 'loadplan')
            else:
                documents = xml_file.find(self.name_space + 'document')
            ...
            '''
            if documents.find(self.name_space + 'task') is not None:
                self.section = self.xml_get_value_by_attr(documents.find(self.name_space + 'task'), 'typetask')
        ...
        for document in documents:
            if not self.delete_document(document):
                ...
                if document_res['status'] == krconst.kr_sql_error:
                    ...
                    if docid > 0:
                        ''' Создадим связь задания и документа '''
                        if self.queuebond_create == 1 and self.section != 'LOADPLAN':
                            self.create_queue_bond(docid)
                        ...
                        if document.tag != self.tagcargo:
                            cargo = document.find(self.name_space + self.tagcargo)
                        else:
                            cargo = document
                        if cargo:
                            self.pcargo = []
                            for position in cargo:
                                if self.section != 'LOADPLAN':
                                    self.pcargo = []
                                    ...
                                    # если есть подчиненные к cargo записи
                                        if self.tagcargobond:
                                            try:
                                                cargobonds = position.find(self.name_space + self.tagcargobond)
                                            except:
                                                cargobonds = None
                                            if cargobonds:
                                                pcargobondinit = []
                        ...
                        if self.import_with_task:
                            cargo = document.find(self.name_space + 'task')
                            for position in cargo:
                                self.pcargo = []
                                self.pcargo += self.pcargoinit
                                self.pcargo += self.get_value_params(position,
                        ...
                        if self.tagdocbond is not None:
                            docbonds = document.find(self.name_space + self.tagdocbond)
                            ...
                            if name_proc_after:
                                if len(name_proc_after) == 1:
                                    ...
                                    if is_delete == '0':
                                        if self.section == 'LOADPLAN':
                                            pcargo_str = ''
                                            for itm in self.pcargo:
                                                if pcargo_str != '':
                                                    pcargo_str += ','
                                                pcargo_str += itm
                                            self.pcargo = self.pcargoinit
                                            self.pcargo.append(pcargo_str)
                                            wares = self.ExecuteSQL(self.proccargo,
                                                                    sqlparams=self.pcargo,
                                                                    fetch='none',
                                                                    ExtVer=True)
            ...
            for rbskey in p_str.split(','):
                ...
                if externalkey != 'None':
                    ...
                    if rbskey == 'SNYATIE' and externalkey != 'None':
                        if val == 'Да' or val == '1':
                            docbonds = obj_xml.find(self.name_space + 'docbonds')
                            if docbonds is not None:
                                for docbond in docbonds:
                                    if externalkey in self.convertparamsbool:
