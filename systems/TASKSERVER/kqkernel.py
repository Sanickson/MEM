...
class QKernel(object):
    ...
    def restart_dangling_turn(self):
        ...
        try:
            ...
            self.log_server('Ошибка при восстановлении заданий ' + exc.args[0], krconst.log_error)
    ...
    def get_queue_task(self):
        ...
        sql_text = '''select rt.tasktype, rt.code, rt.name, rt.autorun , rt.modulename, rt.quetaskid, rt.moduleid, rt.pollinterval,
                                            rt.params, rt.actionsparams, rt.filename, rt.logfilename, rt.delsuccessfiles, rt.logrenewnum, rt.quantity,
                                            rt.logextended
                                            from RBS_Q_GETQUEUETASK(?) rt'''
        sql_params = [self.q_server_id]
        # sql_text += ' where rt.code in (?)'
        # sql_params += ['RBSEXPDOC']
        res = self.db.dbExec(sql_text,
                             params=sql_params, fetch='many')
        if res:
            lst = []
            for item in res:
