...
class UserProfile(object):
    ...
    def validate_length_fields(self, kwargs, length_fields={}):
        ...
        res = 1
        fields = []
        for field, constraints in list(length_fields.items()):
            length = len(kwargs[field]) # длина значения
            for constraint_name, constraint_val in list(constraints.items()):
                if constraint_name == 'strict':
                    if length > 0 and length not in constraint_val:
                        ...
                        fields.append({'name': field, 'errmes': _('Некорректная длина данных. Ожидается строго ') +
                                                                _(' или ').join([str(x) for x in constraint_val]) + _(' символа(ов)')})
...
#                      ,'uid' : resdb['ID_USER']}
    ...
    def user_change_passwd(self, login, oldpasswd, passwd, passwd2, author_ad, old_author_ad, reset_passwd=False):
        ...
        try:
            adm.userVerifyAndChangePassword(actor_login=login, login=login, oldpasswd=oldpasswd, newpasswd=passwd,
                ...
                reset_passwd=reset_passwd, author_ad=author_ad, old_author_ad=old_author_ad)
            ...
            return (0, fields)
        except adm.EUserVerifyAndChangePasswordNewPwdEmpty as exc:
            fields.append({'name': 'passwd', 'errmes': str(exc)}) #Нельзя задавать пустой новый пароль!
            return (0, fields)
        except adm.EUserVerifyAndChangePasswordBadRepeatPwd as exc:
            fields.append({'name': 'passwd2', 'errmes': str(exc)}) #Неверно повторён новый пароль!
            return (0, fields)
        except adm.EUserVerifyAndChangePasswordBadOldPwd as exc:
            fields.append({'name': 'oldpasswd', 'errmes': str(exc)}) #Неверный старый пароль!
            return (0, fields)
        except adm.EUserVerifyAndChangePasswordBadPassword as exc:
            fields.append({'name': 'passwd', 'errmes': str(exc)}) #Недостаточна сложность пароля!
            return (0, fields)
        except adm.EUserNotFoundAD as exc:
            fields.append({'name': 'auth_ad_chk', 'errmes': str(exc)}) #Логина нет в домене!
            return (0, fields)
        except adm.EUserVerifyAndChangePassword as exc:
...
class Profile(BasePage):
    ...
    def passwd(self, fields={}, success=None):
        ...
        has_barcode = pu.iif(getattr(cfg, 'TERMINAL_link', None) in granted_system_refs(self.getUserVar('uid'))
                             ...
                             '1', '')
        has_authorizAD = adm.userInfo(self.getUserVar('uid'))['AUTHOR_AD']
        return self.drawTemplate(templ=passwd, data=[fields, {'system_name':self.sysname,
                                                              'has_barcode': has_barcode,
                                                              'success': success,
                                                              'author_ad': has_authorizAD
                                                              }])
    passwd.exposed = True
    ...
    @filterXSS(['oldpasswd', 'passwd', 'passwd2', 'auth_ad_chk'])
    def changepasswd(self, **kwargs):
        "непосредственное изменение пароля"
        usrProfile = UserProfile()
        if 'auth_ad_chk' not in kwargs: kwargs['auth_ad_chk'] = '0'
        auth_ad_chk = kwargs['auth_ad_chk']

        #если у пользователя была доменная авторизация, то текущий пароль проверяем в AD
        old_author_ad = adm.userInfo(self.getUserVar('uid'))['AUTHOR_AD']

        #если авторизация не доменная проверяем пароли
        if auth_ad_chk != '1':
        ...
        succ, fields = usrProfile.user_change_passwd( login=self.getUserVar('login'), oldpasswd=kwargs['oldpasswd'],
                                                     ...
                                                     author_ad=auth_ad_chk, old_author_ad=old_author_ad )
        if succ == 0:
            #неудачно
            res['fieldvalues'] = kwargs
            res['fields'] = fields
