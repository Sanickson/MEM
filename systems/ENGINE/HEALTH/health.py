...
class Health(BaseMainSystem):
    ...
    def get_block_python(self, rmode='dic', ismobile='0'):
        ...
        dic['PYTHON_THREADS'] = getattr(cfg, 'thread_pool', 20) + pu.iif(getattr(cfg, 'server2', False), getattr(cfg, 'thread_pool2', 20), 0)
        dic['PYTHON_ACTIVE_THREADS'] = len(list(cherrypy.tools.status.seen_threads.items()))
        ...
        if self.opt('grant',None,'delete_aorder_stat','other'):
            ...
            try:
                dbExecDirect(sql='execute procedure HEALTH_R_QUEUE_STAT_DELETE',
                    ...
                    fetch = 'none',
                    id_base=id_base)
            except Exception as exc: return self.pyDumps(ext_data={'res':'0','errMes':exc[1]})
