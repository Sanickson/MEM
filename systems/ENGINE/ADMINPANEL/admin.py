...
from fdb import Error as ExcEr
...
class AdminPanel(BasePage):
    ...
    def systems_systems(self):
        ...
        systems = pu.kbToPy(db.dbExec(sql="select lpad('', 6*8*egs.level_out, '&nbsp;') spaces, egs.FULL_REF_NAME, "
                              "  S.* "
        ...
        for system in systems:
            ...
            system['VERSION'] = getattr(obj, 'VERSION', None)
        return self.drawTemplate(templ=admin_systems_systems, data=[{'datalist': systems}])
    ...
    def systems_users(self, use_filter=False, sel_users=[], returnDic=None):
        ...
        if use_filter:
            users = [x for x in users if str(x['ID']) in sel_users]
        users = {'users': users}
    ...
    def get_all_system_options(self, id_system):
        """Опции, описанные разработчиком в файле options.py
    ...
    def systemsUpdateSystem(self, **kwargs):
        ...
        if('disabled' in kwargs): kwargs['disabled']='1'
    ...
    def validate_fileupload_reference(self, file):
        fres, fmes, fpath = upload(file=file, ext=['zip'], sysname='ADMINPANEL', uid='', test=True)
        if fres == 0:
    ...
    def qDownloadReference(self, id_system, fileupload, **kwargs):
        result = db.dbExec(sql="select FULL_REF_NAME from ENGINE_GET_FULL_REF_NAME(?)", params=(id_system,),
                           fetch="one", id_system=-1)

        if fileupload.filename:
            res, fields = self.validate_fileupload_reference(fileupload)
            if res == 1:
                res, mes, path = upload(file=fileupload, ext=['zip'], sysname='', uid='')
            else:
                return self.pyDumps({'errMes': fields['errmes']})

        path_folder = '../static'+ result['FULL_REF_NAME'] + '/help'
        old_dir_name = '_old'
        old_folder = os.path.join(path_folder, old_dir_name)
        test_dir_name = '_new'
        test_folder = os.path.join(path_folder, test_dir_name)
        indexname = 'index.htm'
        ...
        errmes = None
        if not os.path.isfile(os.path.join(test_folder, indexname)):
            indexhtml = None
            for item in os.listdir(test_folder):
                if item.endswith('.htm'):
                    if indexhtml:
                        errmes = _('Не возможно однозначно установить index.htm')
                    else:
                        indexhtml = item
            if not errmes:
                if indexhtml:
                    os.rename(os.path.join(test_folder, indexhtml), os.path.join(test_folder, indexname))
                else:
                    errmes = _('В архиве нет файла index.htm')

        if not errmes and sys.version_info.major >= 3 and 'encoding' in kwargs and kwargs['encoding'] != 'utf-8':
            try:
                f = open(os.path.join(test_folder, indexname), encoding=kwargs['encoding'])
                txt = f.read()
                f.close()
                f = open(os.path.join(test_folder, indexname), 'w', encoding='utf-8')
                f.write(txt)
                f.close()
            except Exception:
                errmes = _('Ошибка переконвертации')

        if errmes:
            shutil.rmtree(test_folder)
            return self.pyDumps({'errMes': errmes})

        if not os.path.exists(path_folder):
            os.mkdir(path_folder)
        elif len(os.listdir(path_folder)) > 0:
            if os.path.exists(old_folder):
                shutil.rmtree(old_folder)
            os.mkdir(old_folder)
            for file_object in os.listdir(path_folder):
                if not file_object in [old_dir_name, test_dir_name]:
                    os.rename(os.path.join(path_folder, file_object), os.path.join(old_folder, file_object))
        for file_object in os.listdir(test_folder):
            os.rename(os.path.join(test_folder, file_object), os.path.join(path_folder, file_object))
        shutil.rmtree(test_folder)

        return self.pyDumps({'data': '../..'+ result['FULL_REF_NAME'] +'/help/' + indexname})
    qDownloadReference.exposed = True
    ...
    def users(self, focused_id='', layer_id=None):
        ...
        return self.drawTemplate(templ=admin_users, data=[{'focused_id': self.cStr(focused_id)},
                                                          {'layers': lays, 'layer_id': layer_id}])
    users.exposed=True

    @filterXSS([])
    def qusersList(self, fio=None, email=None, ad=None, status=None, focused_id='', filter_cmb=None):
        layer_id = self.cInt(filter_cmb) #важно, иначе сравнение с -2 не работает!
        lays = None
        if layer_id == -2:
            #разрешаем только суперадминам видеть других суперадминов
            d = adm.usersList(layer_id=None, include_superadmins=adm.userIsSuperAdmin(), fio=fio, email=email, ad=ad, status=status)
            #фильтруем из всех слоёв только суперадминов
            d = [u for u in d if u['IS_SUPERADMIN'] is not None and u['IS_SUPERADMIN'] != 0]
        else:
            #разрешаем только суперадминам видеть других суперадминов
            d = adm.usersList(layer_id=layer_id, include_superadmins=adm.userIsSuperAdmin(), fio=fio, email=email, ad=ad, status=status)
        return self.pyDumps(data=d, ext_data={'focused_id': self.cStr(focused_id),'layer_id': layer_id})
    qusersList.exposed=True

    @filterXSS([])
    def userFioInfo(self, lastname, name):
        """Получить информацию о пользователе по ФИ
        """
        server = Server(cfg.AD_SERVER)
        conn = Connection(server, user=cfg.AD_USER, password=cfg.AD_PASSWORD)
        conn.bind()
        #lastname = unicode(lastname, "utf-8").encode('cp1251')
        lastname = str(lastname, "utf-8")
        #name = unicode(name, "utf-8").encode('cp1251')
        name = str(name, "utf-8")
        ldapfilter = '(&(objectCategory=Person)(objectClass=user)(!(UserAccountControl:1.2.840.113556.1.4.803:=2))(anr=%s %s*))' % (lastname, name)

        conn.search(cfg.AD_SEARCH_TREE,
                    ldapfilter,
                    SUBTREE,
                    attributes=['cn', 'sAMAccountName', 'telephoneNumber', 'ipPhone', 'mail']
                    )
        if len(conn.entries) == 0:
            conn.unbind()
            return self.pyDumps(data={'errMes': 'Пользователь с таким именем не найден'})
        elif len(conn.entries) > 1:
            conn.unbind()
            return self.pyDumps(data={'errMes': 'Найдено несколько пользователей с такими данными'})
        else:
            l = conn.entries[0]
            data = {'LOGIN': l.sAMAccountName.value, 'TELEPHONE': l.telephoneNumber.value, 'EMAIL': l.mail.value,
                    'FIO': l.cn.value, 'IPPHONE': l.ipPhone.value}
            conn.unbind()
            return self.pyDumps(data=data)
    userFioInfo.exposed = True
    ...
    def ajaxChangePassword(self, login, passwd, passwd2, send_mail, author_ad= None):
        ...
        if usr['ERROR_CODE'] != 0:
            ...
            if usr['ERROR_CODE'] in (3, -1):
                ...
                res = 'passwd2_invalid'
            elif usr['ERROR_CODE'] == 10:
                res = 'unk_user_ad'
            else:
                res = 'unknown'

            return self.pyDumps(ext_data={'RES': res,'ERROR_MSG': usr['ERROR_MSG']})
        else:
            data = adm.userInfo(usr['ID_USER'])
            if send_mail=='1':
                email = data['EMAIL']
                ...
                if res == 0:
                    return self.pyDumps(ext_data={'RES': 'unknown',
                                           'ERROR_MSG': 'Пароль сгенерирован, но не может быть отправлен.\n'+str(err)
                                    })
                else:
                    return self.pyDumps(data=data, ext_data={'RES': 'ok',
                                           'ERROR_MSG': '',
    ...
    def ajaxNewUser(self, *args, **kwargs): # id_user=None, login=None, fio=None, email=None):
        if 'dlgedit_passw_edt' not in kwargs: kwargs['dlgedit_passw_edt'] = '0'
        id_user=None
        data = adm.userAdd(is_registration=False, fio=kwargs['dlgedit_fio_edt'].strip(), password=None, login=kwargs['dlgedit_login_edt'],
                    ...
                    conEngine=None, handle_manually=True, params_utf8=True, system_user=False, author_ad=kwargs['dlgedit_passw_edt'])
        if 'OUT_ID_USER' in data:
            id_user = data['OUT_ID_USER']
        return self.pyDumps(data=data, ext_data={'ID_USER': id_user})
    ajaxNewUser.exposed=True

    @filterXSS([])
    def ajaxGetGuid1C(self, id_user):
        'присутствует ли интеграция со СКУД'
        id_user = self.cInt(id_user)
        if getattr(cfg, 'SKUD_INTEGRATION', False):
            skud_integration = 1
            guid_1c_select = db.dbExecDirect(sql="""select EXTERNALID
                      FROM OBJECT
            ...
            if guid_1c_select is not None:
                guid_1c = guid_1c_select['EXTERNALID']
            else:
                guid_1c = ''
        else:
            skud_integration = 0
    ...
    def ajaxEditUser(self, *args, **kwargs): # id_user=None, login=None, fio=None, email=None):
        'редактирует параметры пользователя во всех связанных с ним БД систем'
        if getattr(cfg, 'SKUD_INTEGRATION', False):
        ...
        return self.pyDumps(data=adm.userEdit(id_user=self.cInt(kwargs['dlgedit_old_id_edt']),
                     ...
                     params_utf8=True,
                     guid_1c=guid_1c,
                     author_ad=kwargs['dlgedit_passw_edt']), ext_data={'OLD_ID': kwargs['dlgedit_old_id_edt'], 'ID_USER': kwargs['dlgedit_id_edt']})
    ...
    def updStatusUser(self, id_user, status):
        try:
            data=db.dbExec(sql="execute procedure ENGINE_USERSTATUS_UPD(?,?)",
                        params=(id_user, status), fetch='none', id_system=-1)
        except FBExc as exc:
            return self.pyDumps({'errMes': exc[1]})
        else:
            return self.pyDumps(data=data, ext_data={'ID_USER': id_user})
    updStatusUser.exposed=True

    @filterXSS([])
    def qUserInfo(self, id_user):
        return self.pyDumps(data=adm.userInfo(uid=id_user, conEngine=None))
    qUserInfo.exposed=True
    ...
    def roles(self):
        ...
        return self.drawTemplate(templ=admin_roles, data=[{'datalist': d,
                                                           'AD_INTEGRATION': getattr(cfg, 'AD_INTEGRATION', False)}])
    roles.exposed=True
    ...
    def ajaxNewRole(self, *args, **kwargs):
        if getattr(cfg, 'AD_INTEGRATION', False):
            ad_group_name = kwargs['dlgroleedit_ad_group_name_edt']
        else:
            ad_group_name = None
        return self.dump(
    ...
    def ajaxEditRole(self, *args, **kwargs):
        ...
        ri = adm.roleInfo(id_role=id_role)
        if getattr(cfg, 'AD_INTEGRATION', False):
            ad_group_name = kwargs['dlgroleedit_ad_group_name_edt']
        else:
            ad_group_name = ri['AD_GROUP_NAME']
        return self.dump(adm.roleEdit(id_role=id_role,
                 role_name=kwargs['dlgroleedit_name_edt'],
                 role_code=ri['ROLE_CODE'],
                 ad_group_name=ad_group_name,
                 conEngine=None,
                 handle_manually=True,
                 params_utf8=True)
    ...
    def ajaxDlgRoleOptions_load(self, id_role, id_system):
        """Возвращает список опций со слеенными в одну строку атрибутами по роли и системе.
    ...
    def roles_roles(self):
        """Содержимое диалога выбора роли из списка.
    ...
    def roles_attrs(self, id_system, obj_id, opt_id):
        """Выбрать атрибуты из аналогичных у опции.
    ...
    def cpstatus(self, simplelist=None):
        ...
        dl=[]
        for id, ts in list(cherrypy.tools.status.seen_threads.items()):
            ...
            dl.sort(key=lambda item: item['number'])
        if pu.is_linux():
            ps=pu.linux_process_status()
        else:
            ps=None
        if simplelist is None:
            return self.drawTemplate(templ='admin_cpstatus', data=[{'datalist': dl, 
                                                                    'thactivecount': len(dl),
                                                                    'thcount': getattr(cfg, 'thread_pool', 20) + pu.iif(getattr(cfg, 'server2', False), getattr(cfg, 'thread_pool2', 20), 0),
    ...
    def layer_layer(self, include_disabled=0):
        ...
        for l in lst_layers:
            tar = [t for t in lst_tarifs if t['LAYER_ID'] == l['LAYER_ID']][0]
            l['ID_TAR'] = tar['ID_TAR']
            ...
            if tar['DATE_FROM'] and tar['EXPIRE_DAYS']:
                ...
                l['EXPIRE_DATE'] = tar['DATE_FROM'] + datetime.timedelta(days=tar['EXPIRE_DAYS'])
            else:
                l['EXPIRE_DATE'] = None
    ...
    def tar_dlglayers_load(self, id_tar):
        ...
        for tar in lst_tarifs:
            layer = [l for l in lst_layers if l['LAYER_ID'] == tar['LAYER_ID']][0]
            tar['CODE'] = layer['CODE']
    ...
    def tarhistory_load(self, layer_id):
        ...
        for th in lst_tarhistory:
            tar = [t for t in lst_tarifs if t['ID_TAR'] == th['ID_TAR']][0]
    ...
    def tarhistory_get_curtar(self, layer_id):
        ...
        if tar['EXPIRE_DAYS']:
            ...
            tar['EXPIRE_DATE'] = tar['DATE_FROM'] + datetime.timedelta(days=tar['EXPIRE_DAYS'])
        else:
