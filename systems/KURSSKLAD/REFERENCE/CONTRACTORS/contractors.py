...
class Contractors(QuickHelper,KSprav):
    def __init__(self):
        self.datef = '%d.%m.%Y'
    ...
    def ajaxCategoryAddEdit(self, **kwargs):
        name = str(kwargs['Name'], "utf-8").encode("windows-1251")
    ...
    def delContractor(self, compid):
        res = self.dbExec(sql='execute procedure DEL_CONTRACTORS(?)',params=[compid],fetch='none')
        return self.pyDumps(res)
    delContractor.exposed = True
