...
class SpWares(KSprav,WaresLotIncomes):
    ...
    def delWUBarcode(self,barcid):
        try:
            self.dbExec(sql='delete from waresbarcode where waresbarcodeid = ?',params=[barcid],fetch='none')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
    ...
    def chgWUBarcode(self,barcid,barcode):
        try:
            self.dbExec(sql='execute procedure WH_SPWARES_CHGBARCODE(?,?)',params=[barcid,barcode],fetch='none')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
