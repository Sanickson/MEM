...
class Depart(RCommon):
    ...
    def listDepart(self,depid=None):
        try:
            data = self.dbExec(sql="select * from K_DEPART_LISTDEPART(?)",params=[depid],fetch="all")
    ...
    def delDepart(self,**kwargs):
        try:
            data=self.dbExec(sql="execute procedure K_DEPART_DELDEPART(?)",params=[kwargs['depid']],fetch="none")
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
    ...
    def cngDepart(self,**kwargs):
        ...
        params = [kwargs['depid'],
                  ...
                  kwargs['hid']]
        try:
            data=self.dbExecC(sql="select * from K_DEPART_CNGDEPART(?,?,?,?)", params=params, fetch="one")
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
    ...
    def listPosts(self):
        try:
            data = self.dbExecC(sql="select * from DOLGN",params=[],fetch="all")
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
    ...
    def addPosts(self,did,sname,fname):
        try:
            data = self.dbExecC(sql="select * from K_POSTS_ADDPOST(?,?)",params=[sname,fname],fetch="one")
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
