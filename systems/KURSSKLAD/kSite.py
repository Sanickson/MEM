...
class kSite(WHCommon):       
    ...
    def changeSiteStatus(self, siteid):
        try:
            data = self.dbExec(sql="select * from K_SITE_STATUS_CHANGE(?)", params=[siteid], fetch='one')
        except Exception as exc:
            return self.pyDumps({'errMes': exc[1]})
        return self.pyDumps(data=data, ext_data={'SITEID': siteid})
    changeSiteStatus.exposed = True

    def getSitePalletVolume(self, siteid):
        try:
            data = self.dbExec(sql="select s.SITEID, s.NAME, s.PALLETVOLUME from site s where siteid = ?", params=[siteid], fetch='one')
        except Exception as exc:
            return self.pyDumps({'errMes': exc[1]})
        return self.pyDumps(data=data)
    getSitePalletVolume.exposed = True

    def setSitePalletVolume(self, siteid, palletvolume):
        try:
            data = self.dbExec(sql="update site s set s.PALLETVOLUME = ? where s.siteid = ?", params=[palletvolume, siteid], fetch='none')
        except Exception as exc:
            return self.pyDumps({'errMes': exc[1]})
        return self.pyDumps(data=data)
    setSitePalletVolume.exposed = True