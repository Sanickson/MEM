...
class Scale(WHCommon):
    ...
    def GetScaleWaresMassa(self, scaleid):
        line = 'PLU;TYPE;LABEL_NUMBER;BARCODE_NUMBER;BARCODE_PREFIX;PRICE;TARE;CODE;BEST_BEFORE;SHELF_LIFE;CERTIFICATE;GROUP;CENTERING;NAME;CONTENT;INFO_TYPE;INFO\n'

        data = self.dbExec(sql='select * from k_scale_priceprint(?)', params=[scaleid], fetch='all')

        if data and data['datalist']:
            for item in data['datalist']:
                line += self.ValToStr(item['CODE']) + ';0;1;1;0;0;0;'
                line += self.ValToStr(item['CODE']) + ';0;0;0;0;0;'
                line += self.ValToStr(item['NAME']) + ';0;1;0\n'

        filename = 'Scale_' + time.strftime('%Y.%m.%d_%H-%M-%S', time.localtime()) + '.csv'
        ...
        return line
    GetScaleWaresMassa.exposed = True
