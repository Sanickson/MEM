...
class DocManager(TDocTypes,KSprav,TDocPrintCommon,TaskManagerView,WLImages):
    ifaceCode = 'DOCMANAGER'
    
    def docTypes(self, id=None):
        if self.getIfaceVar('isadmin') == 1:
            wmsesid = None
        else:
            wmsesid = self.getIfaceVar('wmsesid')
        try:
            dt = self.dbExec(sql='select * from K_DOCMANAGER_DOCTYPES(?) order by NAME',params=[wmsesid],fetch='all')
    ...
    def index(self, id_system=None):  
        ...
        if self.opt('edit', None, 'gtdchange'):
            gtdedit = 1
        return self.drawTemplate(templ=tmplmain, data=[{'admin':admin,'viewunit':viewunit, 'gtdedit': gtdedit}]) 
    index.exposed = True
    ...
    def listDocs(self, docid=None, dbeg=None, dend=None, docnum=None, doctype=None, fromobj=None, toobj=None,
                 sumFrom=None, sumTo=None, status=None, throughobj=None, docsubtype=None, flags=None):
        ...
        if doctype:
            ...
            doctype = str(
                self.dbExecC(sql="select * from document where docid=?", fetch='one', params=[docid])['DOCTYPE'])
        ...
        ext_data = {}
        doctypedata = self.dbExec(sql="select * from doctype where doctid=?", params=[doctype], fetch='one')
        ext_data['haswares'] =doctypedata['HASWARES']
        ext_data['typecode'] =doctypedata['CODE']
        ext_data['PRICECODE'] = \
            self.dbExec(sql="select * from K_DOCMANAGER_GETPRICEID(?)", params=[doctype], fetch='one')['CODE']
        if status == 'null': status = None
        if docsubtype == 'null': docsubtype = None
        try:
            DSetDocument = self.dbExecC(sql="SELECT * FROM K_DOCMANAGER_LISTDOCS(?,?,?,?,?,?,?,?,?,?,?,?,?)", params=(
                fromobj, throughobj, toobj, doctype, status, docnum, dbeg, dend, sumFrom, sumTo, docid, docsubtype,
                flags),
                                        fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
    ...
    def listCargo(self, docid, cargoid=None, dbid=None):
        #if dbid: dbid is None
        try:
            DSet = self.dbExec(sql="SELECT * from k_wh_listcargo(?,?,?)", params=(docid, cargoid, self.toNone(dbid)),fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=DSet,ext_data={'docid':docid})
    listCargo.exposed = True

    def listTaskes(self, docid, dbid=None):
        try:
            DSet = self.dbExec(sql="SELECT * from K_DOCMANAG_LISTTASK(?,?)", params=(docid, self.toNone(dbid)),
                               fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=DSet, ext_data={'docid': docid, 'dbid': dbid})
    ...
    def infoCargo(self, id):
        try:
            DSet=self.dbExec(sql="SELECT * FROM K_GET_INFO_CARGO(?)",params=(id,),fetch='one')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
    ...
    def cargoAdd(self, docid, wid, price, amount, docsum=None):
        ...
        try:
            cg=self.dbExec(sql="select * from K_CARGO_I(?,?,?,?,?,?)",params=(docid,wid,None,price,amount,docsum),fetch='one')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
    ...
    def cargoUpd(self, docid, cargoid, wid, price, amount, docsum=None):
        ...
        try:
            cg=self.dbExec(sql="execute procedure K_CARGO_U(?,?,?,?,?,?,?)",params=(docid,cargoid,wid,None,price,amount,docsum),fetch='none')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
    ...
    def docStatSet(self, docid, status):
        ...
        try:
            self.dbExecC(sql="execute procedure WH_DOCMNGR_DOCSTATUSSET(?,?)", params=[docid, status], fetch='None')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
    ...
    def calcDocWUPrice(self, docid, wuid):
        try:
            res = self.dbExecC(sql="select * from K_CALC_DOC_WU_PRICE(?,?)", params=(docid,wuid), fetch='one')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
    ...
    def docQueueInfo(self,docid,dbid=None):
        return self.pyDumps(self.dbExec('select * from RBS_Q_QUEUEDOCINFO_GET(?,?,?)',[docid,'EI',self.toNone(dbid)],fetch='all'))
    docQueueInfo.exposed = True
    
    def docReQueue(self,docid):
        try:
            data = self.dbExec('execute procedure RBS_Q_CREATETASKEXPORT(?,?,?,?)',[self.getUserVar('userfio'),docid,None,None],fetch='none')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
    ...
    def getWaresLot(self, docid, dbid=None):
        try:
            data = self.dbExec(sql="select * from K_DOCMANAGER_WARES_LOT(?,?)", params=[docid,self.toNone(dbid)], fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
    ...
    def waresLotItem(self, wlotid):
        try:
            data = self.dbExec(sql="select * from K_DOCMANAGER_WARES_LOT_ITEM(?)", params=[wlotid], fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
    waresLotItem.exposed = True
    
    def getWaresLotWares(self, docid, dbid=None):
        try:
            data = self.dbExec(sql="select * from K_DOCMANAGER_WARES_LOT_WARES(?,?)", params=[docid, self.toNone(dbid)], fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
    getWaresLotWares.exposed = True
    
    def getWaresLotHist(self, twids, dbid=None):
        try:
            data = self.dbExec(sql="select * from K_DOCMANAGER_WARES_LOT_HIST(?,?)", params=[twids, self.toNone(dbid)], fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
    getWaresLotHist.exposed = True
    
    def docBondCrossTbl(self, docs, dbid=None):
        try:
            data = self.dbExec(sql="select * from K_DOCMANAGER_CROSSDOCS(?,?)", params=[docs,self.toNone(dbid)], fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
    ...
    def getBocBondType(self):
        try:
            data = self.dbExec(sql="select * from docbondtype", params=[], fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
    ...
    def createDocBond(self, doc1, doc2, db):
        try:
            data = self.dbExec(sql="select * from K_DOCMANAGER_CREATEDOCBOND(?,?,?)", params=[doc1, doc2, db], fetch='one')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
    ...
    def deleteDocBond(self, dbid):
        try:
            data = self.dbExec(sql="delete from docbond db where db.DOCBONDID=?", params=[dbid], fetch='none')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
    ...
    def getLastCargoPrice(self, docid, waresid, amount):
        try:
            data = self.dbExec(sql="select * from K_DOCMANAGER_GETLASTCARGO(?,?,?)", params=[docid, waresid, amount], fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
    ...
    def ajaxGetIncomeByDate(self, waresid, date1, date2, docid):
        try:
            data = self.dbExec(sql="select * from K_DOCMANAGER_GETDATESCARGO(?,?,?,?)", params=[waresid, date1, date2, docid], fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
    ...
    def getWLsByDoc(self, docid):
        try:
            data = self.dbExec(sql="select list(wl.wlotid) as wls from wareslot wl where wl.docid = ?", params=[docid], fetch='one')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data=data)
