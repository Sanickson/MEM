...
def coalesceField(item, field, default='&nbsp;'):
    ...
    from fdb import DESCRIPTION_NAME as dn
...
def TimeStampToDate(timestamp, longYear=None):
    import time
    if timestamp is None: return '&nbsp;'
    sdate=str(timestamp).split('.')[0]
    #sformatFrom='%Y-%m-%d %H:%M:%S'
    sformatFrom='%Y-%m-%d' if len(sdate.split(' '))==1 else '%Y-%m-%d %H:%M:%S'
...
def TimeStampToMonthDate(timestamp,):
    import time
...
def waresQuantity(q, sucnt, vuf, vuc, muf, muc, rformat='s'):
    if not sucnt:
        r = viewQuantity(q=q, viewufactor=vuf, viewucode=vuc, mainufactor=muf, mainucode=muc)
        if rformat == 'f':
            r += ' <b><u>('+ Quantity(q) + ')</u></b>'
        return r
    else:
        if q is None:
            return '&nbsp;'
        else:
            return Quantity(q) + muc  + '=<b>' + str(sucnt) + '</b>'
