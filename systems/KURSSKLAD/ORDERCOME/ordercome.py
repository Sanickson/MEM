...
class OrderCome(WHCommon):
    ...
    def incomes(self, objid, dbeg, dend):
        try:
            d = self.dbExec(sql='select * from WH_ORDERCOME_LISTINCOMES(?,?,?)', params=[objid, dbeg, dend], fetch='all')
    ...
    def incomeWares(self, taskid, waresid=None):
        try:
            d = self.dbExec(sql='select * from WH_ORDERCOME_LISTINCOMEWARES(?,?)', params=[taskid,waresid], fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
    ...
    def incomeWaresOrders(self, taskid, waresid):
        try:
            d = self.dbExec(sql='select * from WH_ORDERCOME_LISTINCOMEWARESORD(?,?)', params=[taskid,waresid], fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
    ...
    def chgOrders(self, **args):
        ...
        for arg in args:
            if arg=='waresid': waresid = args[arg]
                ...
                quantities += args[arg] + ';'
        try:
            d = self.dbExec(sql='execute procedure WH_ORDERCOME_CHGORDERS(?,?,?,?,?)', params=[taskid, waresid, self.getIfaceVar('wmsid'), taskwares, quantities], fetch='none')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
    ...
    def orders(self, objid, dbeg, dend):
        try:
            d = self.dbExec(sql='select * from WH_ORDERCOME_LISTORDERS(?,?,?)', params=[objid, dbeg, dend], fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data = d)
    orders.exposed = True
    ...
    def orderWares(self, taskid):
        try:
            d = self.dbExec(sql='select * from WH_ORDERCOME_LISTORDERWARES(?)', params=[taskid], fetch='all')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        return self.pyDumps(data = d, ext_data = {'taskid':taskid})
