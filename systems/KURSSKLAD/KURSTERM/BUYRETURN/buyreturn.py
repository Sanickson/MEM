...
class TBuyReturn(TCommonTerm):
    ...
    def scanMain(self, barcode=None):
        ...
        if bcInfo and bcInfo['result']==0:            
            if bcInfo['usercode']=='DOCUMENT':
                ...
                try:
                    data = self.dbExec(sql='SELECT * FROM K_WH_BUYRETURN_CHKSITE(?,?)', params=[int(bcInfo['recordid']), self.getIfaceVar('wmsid')], fetch='one')
                except Exception as exc:
                    return self.taskList(mes=self.fbExcText(exc))
    ...
    def taskWaresEdit(self, tid, wid, pid, pdate, clear='0', mes=None):
        if clear=='1':
            try:
                self.dbExec(sql='execute procedure K_WH_BUYRETURN_DO_CLEAR(?,?,?,?)',params=[tid,wid,pdate,pid],fetch='none')
            except Exception as exc:
    ...
    def taskEnd(self, tid):
        try:
            t = self.dbExec(sql='execute procedure K_WH_BUYRETURN_TASKEND(?)', params=[tid], fetch='none')
        except Exception as exc:
            return self.task(tid=tid, mes=self.fbExcText(exc))
    ...
    def taskWaresLotAdd(self, tid, waresid, wuid=None, productdate=None, barcode=None, q=None, wlnumber=None, mes=None):
        if barcode and q and float(q)>0.0001:
            try:
                r = self.dbExec(sql="select * from K_WH_BUYRETURN_DO(?,?,?,?,?,?)",fetch='one',params=[tid,wuid,q,barcode,productdate,self.getIfaceVar('wmsid')])
            except Exception as exc:
                mes=self.mesAdd(mes, self.fbExcText(exc))
            else: 
                if r['mes']:
    ...
    def writeOff(self, palletid):
        ...
        mes = ''
        try:
            self.dbExec(sql='execute procedure K_WH_BUYRETURN_WRITEOFF(?,?)',params=[palletid,self.getIfaceVar('wmsid')],fetch="none")
        except Exception as exc:
            mes = self.fbExcText(exc)
    ...
    def supplReturn(self, palletid):
        ...
        mes = ''
        try:
            self.dbExec(sql='execute procedure K_WH_BUYRETURN_RETURNTO(?,?)',params=[palletid,self.getIfaceVar('wmsid')],fetch="none")
        except Exception as exc:
            mes = self.fbExcText(exc)
        return self.taskList(mes=mes)
    ...
    def reAllocWaresSave(self, **args):
        ...
        try: 
            ...
            return self.scanReallocWares(mes=self.fbExcText(exc), pid=args['pid'], siteid=args['siteid'], subtype=args['subtype'], waresid=args['wid'])
        else: 
