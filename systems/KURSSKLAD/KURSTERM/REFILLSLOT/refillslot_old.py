...
class TRefillSlot(TCommonTerm):
    ...
    def rfsGetTask(self,tid=None):
        ...
        except Exception as exc: raise HTTPRedirect('rfsMain?mes=%s' % (self.fbExcText(exc)))
    ...
    def rfsTask(self, id, barcode=None, mes=None, chkTask = '0'):
        ...
        except Exception as exc: raise HTTPRedirect('rfsMain?mes=%s' % (exc[1]))
        if barcode:            
            bcInfo = self.kBarCodeInfo(barcode)
            if bcInfo and bcInfo['result']==0:
                mes = _('Invalid barcode')    
    ...
    def rfsTaskWaresPallet(self, tid, wid, pid=None):
        ...
        if pid:
            ...
            except Exception as exc: mes = exc[1]
    ...
    def rfsTaskWaresSave(self, **args):
        ...
        except Exception as exc: raise HTTPRedirect('rfsTaskWares?tid=%s&wid=%s&mes=%s' % (args['tid'], args['wid'], exc[1]))
    ...
    def rfsTaskWaresCancel(self, tid, wid):
        ...
        except Exception as exc: raise HTTPRedirect('rfsTaskWaresPallet?tid=%s&wid=%s&mes=%s' % (tid, wid, exc[1]))
    ...
    def rfsTaskEnd(self, id):
        ...
        except Exception as exc: raise HTTPRedirect('rfsTask?id=%s&mes=%s' % (id, exc[1]))
