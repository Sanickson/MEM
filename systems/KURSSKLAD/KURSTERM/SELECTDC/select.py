...
from systems.KURSSKLAD.KURSTERM.SELECTDC.templates.taskCWaresAlco import taskCWaresAlco
from systems.KURSSKLAD.KURSTERM.SELECTDC.templates.taskCWaresAlcoEnd import taskCWaresAlcoEnd
from systems.KURSSKLAD.KURSTERM.SELECTDC.templates.taskCWaresW import taskCWaresW
from systems.KURSSKLAD.KURSTERM.SELECTDC.templates.taskCWaresWTaraQuant import taskCWaresWTaraQuant
from systems.KURSSKLAD.KURSTERM.SELECTDC.templates.taskCWaresWU import taskCWaresWU
from systems.KURSSKLAD.KURSTERM.SELECTDC.templates.taskCWaresLotIncome import taskCWaresLotIncome
from systems.KURSSKLAD.KURSTERM.SELECTDC.templates.taskCWaresWUEndConf import taskCWaresWUEndConf
...
class TSelect(TCommonTerm):
    ...
    tmplTaskCWaresAlco = taskCWaresAlco
    tmplTaskCWaresAlcoEnd = taskCWaresAlcoEnd
    tmplTaskCWaresW = taskCWaresW
    tmplTaskCWaresWTaraQuant = taskCWaresWTaraQuant
    tmplTaskCWaresWU = taskCWaresWU
    tmplTaskCWaresLotIncome = taskCWaresLotIncome
    tmplTaskCWaresWUEndConf = taskCWaresWUEndConf
    ...
    def taskJoin(self, id):
        try: self.dbExec(sql='execute procedure WH_DC_SELECT_JOINTASK(?,?)', params=[id,self.getIfaceVar('wmsid')], fetch='none')
        except Exception as exc: raise HTTPRedirect('listTaskes?mes=%s' % self.fbExcText(exc))
    ...
    def taskWaresJoin(self, id, errUrl=None):
        try: 
            ...
            raise HTTPRedirect('listTaskes?mes=%s' % (self.fbExcText(exc)))
    ...
    def qTaskWaresDo(self, twid, q=None):
        ...
        try: self.dbExec(sql='execute procedure WH_DC_SELECT_TASKWARES_DO(?,?)', params=[twid,q], fetch='none')
            ...
            if q: raise HTTPRedirect('taskCWaresQuant?twid=%s&mes=%s' % (twid, self.fbExcText(exc)))
            else: raise HTTPRedirect('taskCWares?twid=%s&mes=%s' % (twid, self.fbExcText(exc)))
    ...
    def qTaskSetSite(self, tid, barcode):
        ...
        try: self.dbExec(sql='execute procedure WH_DC_SELECT_SETTASKSITE(?,?)', params=[tid, barcode], fetch='none')
        except Exception as exc: raise HTTPRedirect('taskEndSite?tid=%s&mes=%s' % (tid, self.fbExcText(exc)))
    
    def qTaskSetTransUnit(self, tid, barcode):
        """ Местоположение поддона отборки """
        try: self.dbExec(sql='execute procedure WH_SELECTDC_SETTASKTRANSUNIT(?,?,?)', params=[tid, barcode, self.userGetManId()], fetch='none')
        except Exception as exc: raise HTTPRedirect('taskTransUnit?id=%s&mes=%s' % (tid, self.fbExcText(exc)))
    ...
    def qTaskCWaresNoPlace(self, tid, twid):
        ...
        except Exception as exc: raise HTTPRedirect('taskC?id=%s&mes=%s' % (tid, self.fbExcText(exc)))
    ...
    def qTaskCWaresNoAmount(self, twid):
        ...
        try: 
            ...
            raise HTTPRedirect('taskCWares?twid=%s&mes=%s' % (twid, self.fbExcText(exc)))
    ...
    def taskC(self, id, mes=None):        
        ...
        if self.logSel: 
            logWrite('taskC',ext_params={'tid':id,'wmsid':self.getIfaceVar('wmsid')})
        self.taskJoin(id)
    ...
    def taskCtwClear(self, twid):
        ...
        except Exception as exc: raise HTTPRedirect('taskCWares?twid=%s&mes=%s' % (twid, self.fbExcText(exc)))
        else: raise HTTPRedirect('taskCWares?twid=%s'%(twid))
    taskCtwClear.exposed = True       
    
    
    def qtaskCWaresWUEnd(self, taskid, twid):
        try:
            ...
            raise HTTPRedirect('taskC?id=%s'%taskid)
    
    qtaskCWaresWUEnd.exposed = True

    def qtaskCWaresWUEndEmpty(self, taskid, twid):
        self.qTaskWaresDo(twid=twid, q=0)
        raise HTTPRedirect('taskC?id=%s' % taskid)
    qtaskCWaresWUEndEmpty.exposed = True
    
    def taskCWaresLotIncome(self, twid, barcode, amount=None):
        try:
            r = self.dbExec(sql="select * from WH_SELECTDC_LOTINCOME_SCAN(?,?,?,?)",
                        params=[twid, self.getIfaceVar('wmsid'), barcode, amount], fetch='one')
        except Exception as exc:
            raise HTTPRedirect('taskCWares?twid=%s&mes=%s' % (twid, self.fbExcText(exc)))
        else:
            if r['TASKID']:
                raise HTTPRedirect('taskC?id=%s' % r['TASKID'])
            else:
                raise HTTPRedirect('taskCWares?twid=%s' % twid)
    taskCWaresLotIncome.exposed = True

    def taskCWaresAlco(self, twid, barcode=None, mes=None):
        if barcode:
            try:
                r = self.dbExec(sql="select * from WH_SELECTDC_ALCO_SCAN(?,?)",
                                params=[twid, barcode], fetch='one')
            except Exception as exc:
        ...
        return self.drawTemplate(templ=self.tmplTaskCWaresAlco, data=[tw, t, wl, {'mes': mes, 'reloadurl': 'taskCWares?twid=%s' % twid}])
    taskCWaresAlco.exposed = True

    def taskCWaresAlcoEnd(self, twid, barcode=None, mes=None):
        if barcode:
            try:
                r = self.dbExec(sql="select * from WH_SELECTDC_ALCO_END(?,?,?)",
                                params=[twid, self.getIfaceVar('wmsid'), barcode], fetch='one')
            except Exception as exc:
                mes = self.fbExcText(exc)
            else:
                raise HTTPRedirect('taskC?id=%s' % r['TASKID'])
        ...
        t = self.taskInfo(tw['TID'])
        wl = self.qTaskWaresLots(twid)
        return self.drawTemplate(templ=self.tmplTaskCWaresAlcoEnd, data=[tw, t, wl, {'mes': mes, 'backurl': 'taskCWaresAlco?twid=%s' % twid}])
    taskCWaresAlcoEnd.exposed = True
    ...
    def taskCWares(self, twid, barcode=None, mes=None):
            ...
            if bcInfo:
                if bcInfo['usercode'] == 'SITE':
                    if self.kId(tw['SITEID']) != self.kId(bcInfo['recordid']):
                        ...
                        if tw['WARES_IS_VET'] == '1':
                            if not tw['QS'] or float(tw['QO']) != float(tw['QS']):
                                t = self.taskInfo(tw['TID'])
                                return self.drawTemplate(templ=self.tmplTaskCWaresWUEndConf, data=[tw, t])
                            else:
                                self.qtaskCWaresWUEnd(twid=twid, taskid=tw['TID'])
                        elif wT == 'W':
                            try:
                                self.dbExec(sql="execute procedure K_WH_SELECTW_TWEND(?,'0')",
                                            params=[twid], fetch='none')
                ...
                flagContinue = True
                if float(tw['TARAWEIGHT']) > 0.0001:
                    bc = self.whBarCodeWaresWeightInfo(barcode=barcode)
                    if bc['datalist'] and len(bc['datalist']) > 0 and bc['datalist']['BCSUBTYPE']=='EAN13':
                        flagContinue = False
                        itemWares = None
                        for item in bc['datalist']:
                            if self.kId(item['WID']) == self.kId(tw['WID']):
                                itemWares = item
                        if itemWares:
                            return self.taskCWaresWTaraQuant(twid=twid, wweight=itemWares['WWEIGHT'])
                        else:
                            mes = _('Отсканирован весовой ШК не этого товара')
                if flagContinue:
                    try:
                        self.dbExec(sql='execute procedure WH_SELECT_TW_SCANBARCODE(?,?,?)',
                                    params=[twid, barcode, self.getIfaceVar('wmsid')], fetch='none')
                    except Exception as exc:
                        mes = self.fbExcText(exc)
                    else:
                        tw = self.qTaskWaresInfo(twid=twid)
        ...
        if tw['WARES_IS_VET'] == '1':
            ...
            if tw['fs'] == 'A':
                templ = self.tmplTaskCWaresAlco
            elif tw['fs'] == 'I':
                try:
                    wl = self.dbExec(sql='select * from WH_SELECTDC_LOTINCOME_INFONEXT(?)', params=[twid], fetch='one')
                except Exception as exc:
                    mes = self.fbExcText(exc)
                    raise HTTPRedirect('taskC?id=%s&mes=%s' % (tw['TID'], mes))
                templ = self.tmplTaskCWaresLotIncome
            elif tw['fs'] == 'U':
                templ = self.tmplTaskCWaresWU
            else:
    ...
    def taskCWaresWTaraQuant(self, twid, **args):
        tw = args['tw'] if 'tw' in args else self.qTaskWaresInfo(twid=twid)
        ...
        if 'quant' in args:
            ...
            try:
                self.dbExec(sql='execute procedure K_WH_SELECT_SETWEIGHT(?,?,?,?)',
                            params=[twid, self.getIfaceVar('wmsid'), weight, None], fetch='none')
            except Exception as exc:
                mes = self.fbExcText(exc)
            else:
                raise HTTPRedirect('taskCWares?twid=%s' % twid)
        ...
        params = {
            'mes': mes,
            'backurl': 'taskCWares?twid=%s' % twid
        }
        if 'wweight' in args:
            params['WWEIGHT'] = args['wweight']
        return self.drawTemplate(templ=self.tmplTaskCWaresWTaraQuant, data=[tw, t, wl, params])
    taskCWaresWTaraQuant.exposed = True
            ...
            if bcInfo:
                if bcInfo['USERCODE'] == 'SITE':
                    if self.kId(tw['SITEID']) != self.kId(bcInfo['RECORDID']):
                        ...
                        if tw['VUID'] and self.kId(wuid) == self.kId(tw['VUID']):
                            q = float(q)*float(tw['VUFACTOR'])
    ...
    def taskCWaresSite(self, twid, barcode = None, mes = None):
        ...
        if self.logSel:
            logWrite('taskCWaresSite', ext_params={'twid': twid, 'barcode': barcode, 'mes': mes, 'wmsid': self.getIfaceVar('wmsid')})
        tw = self.qTaskWaresInfo(twid=twid)        
        if barcode:
            bcInfo = self.whBarCodeInfo(barcode)
            if bcInfo:
    ...
    #----------------------------------------------------------------------------------------------------------------------
    # Метод выполнения - P - поддон - нужно отобрать один поддон товара, в задании только этот товар
    #----------------------------------------------------------------------------------------------------------------------
    # Пока тупо обрабатываем стандартным методом
    taskP = taskC
    taskP.exposed = True
    ...
    def WIndex(self, barcode=None, mes=None):
        if barcode:
            bcInfo = self.whBarCodeInfo(barcode)
            if bcInfo and bcInfo['USERCODE'] == 'PALLET':
                raise HTTPRedirect('taskWPallet?palletid=%s' % bcInfo['RECORDID'])
            else:
                mes = _('ШК не обрабатывается')
        if not mes:
    ...
    def taskWPallet(self, palletid, barcode=None, mes = None):
        ...
        if barcode:
            ...
            if bcInfo and len(bcInfo['datalist']) == 1:
                raise HTTPRedirect('taskWPalletWares?palletid=%s&waresid=%s'%(palletid, bcInfo['datalist'][0]['WID']))
    ...
    def qtaskWPalletWares(self, palletid, waresid):
        ...
        except Exception as exc: raise HTTPRedirect('taskWPallet?palletid=%s&mes=%s' % (palletid, self.fbExcText(exc)))
        else: return pw
    ...
    def taskWPalletWaresPos(self, palletid, waresid, twid, barcode=None, mes = None):
        ...
        if barcode:
            bcInfo = self.whBarCodeInfo(barcode)
            if bcInfo:
                if bcInfo['USERCODE']=='TASK':
                    t = self.qTaskWares(twid)
                    if t['taskid'] == self.kId(bcInfo['RECORDID']):
                        raise HTTPRedirect('taskWtwEnd?twid=%s' % twid)
                mes = _('Не верный ШК')
            else:
                bc = self.whBarCodeWaresInfo(barcode)
                if len(bc['datalist']) == 1:
                    bc0 = bc['datalist'][0]
                    if bc0['USERCODE'] == 'WARESWEIGHT' or bc0['USERCODE'] == 'SUPWEIGHT':
                        try:
                            self.dbExec(sql='execute procedure K_WH_SELECT_SETWEIGHT(?,?,?)',
                                        params=[twid, self.getIfaceVar('wmsid'), bc0['AMOUNT']], fetch='none')
                        except Exception as exc:
                            mes = self.fbExcText(exc)
                    else:
                        mes = _('Не верный ШК')
                else:
                    mes = _('Не возможно однозначно утсановить данные по ШК')
    ...
    def taskWtwEnd(self, twid, pc='0'):
        ...
        except Exception as exc: raise HTTPRedirect('taskWPalletWaresPos?palletid=%s&waresid=%s&twid=%s&mes=%s' % (tw['palletid'], tw['waresid'], twid,
                                                                                                                   self.fbExcText(
                                                                                                                   exc[
                                                                                                                       1])))
    ...
    def taskWtwClear(self, palletid, waresid, twid):
        try: self.dbExec(sql='execute procedure K_WH_SELECT_SETWEIGHT(?,?,?)',params=[twid,self.getIfaceVar('wmsid'),None],fetch='none')
        except Exception as exc: raise HTTPRedirect('taskWPalletWaresPos?palletid=%s&waresid=%s&twid=%s&mes=%s' % (palletid, waresid, twid,
                                                                                                                   self.fbExcText(
                                                                                                                   exc[
                                                                                                                       1])))
        else: raise HTTPRedirect('taskWPalletWaresPos?palletid=%s&waresid=%s&twid=%s'%(palletid,waresid,twid))
    ...
    def taskWPalletWaresUnit(self, palletid, waresid, twid, barcode=None, mes = None):
        ...
        if barcode:
            bcInfo = self.whBarCodeInfo(barcode)
            if bcInfo and bcInfo['USERCODE'] == 'TASK':
                t = self.qTaskWares(twid)
    ...
    def qTaskWaresDoU(self, twid, q=None):
        ...
        try: self.dbExec(sql='execute procedure K_WH_SELECTW_TASKWARES_DO(?,?)', params=[twid,q], fetch='none')
            ...
            if q: raise HTTPRedirect('taskWPalletWaresUnitQ?twid=%s&mes=%s' % (twid, self.fbExcText(exc)))
            else: 
                tw = self.qTaskWares(twid)
                raise HTTPRedirect('taskWPalletWaresUnit?palletid=%s&waresid=%s&twid=%s&mes=%s' % (tw['palletid'], tw['waresid'], twid,
                                                                                                   self.fbExcText(
                                                                                                       exc[1])))
    ...
    def qTaskFWaresNoPlace(self, twid):
        """ Нет места на поддоне """
        try: t = self.dbExec(sql='select * from K_WH_SELECT_POS_ENDTASK(?)', params=[twid], fetch='one')
        except Exception as exc: raise HTTPRedirect('taskFWares?twid=%s&mes=%s' % (twid, self.fbExcText(exc)))
    ...
    def qTaskFWaresNo(self, twid):
        ...
        except Exception as exc: raise HTTPRedirect('taskFWaresPallet?twid=%s&mes=%s' % (twid, self.fbExcText(exc)))
        else: 
            tw = self.qTaskWares(twid)
    ...
    def qTaskFNextWares(self, tid): 
        ...
        return self.dbExec(sql='SELECT * FROM RBS_TSELECTDC_TASKF_NEXTWARES(?)', params=[tid], fetch='all')
        
    def qTaskFWaresUDo(self, twid, q=None):
        """ Подтверждение отборки """
        try: self.dbExec(sql='execute procedure WH_DC_SELECT_TASKWARES_DO(?,?)', params=[twid,q], fetch='none')
        except Exception as exc:
            if q: raise HTTPRedirect('taskFWaresUQuant?twid=%s&mes=%s' % (twid, self.fbExcText(exc)))
            else: raise HTTPRedirect('taskFWares?twid=%s&mes=%s' % (twid, self.fbExcText(exc)))
    ...
    def taskF(self, id, mes=None):
        ...
        if lenTw==0:
            raise HTTPRedirect('taskEndPrint?tid=%s'%(id))
        elif lenTw==1:
    ...
    def taskFWares(self, twid, barcode=None, mes=None):
            ...
            if bcInfo:
                if bcInfo['usercode']=='PALLET': 
                    if self.kId(tw['TWPALLETID']) != self.kId(bcInfo['RECORDID']):
                        mes = _('Не верный поддон')
                    else:
                        if wT=='W':
                            raise HTTPRedirect('taskFWaresEnd?twid=%s' % twid)
                        else: 
                            self.qTaskFWaresUDo(twid)
                            raise HTTPRedirect('taskF?id=%s' % tw['TID'])
                else:
                    mes = _('Не верный ШК')
            elif wT=='W':
                bc = self.whBarCodeWaresInfo(barcdoe=barcode, waresid=tw['WID'])
                if len(bc['datalist'])==1:
                    bc0 = bc['datalist'][0]
                    if bc0['usercode']=='WARESWEIGHT' or bc0['usercode']=='SUPWEIGHT':
                        try:
    ...
    def taskFWaresPallet(self, twid, barcode = None, mes = None):
        ...
        if barcode:
            ...
            if bcInfo:
                ...
                mes = _('Не верный ШК')
        t = self.taskInfo(tw['TID'])
        return self.drawTemplate(templ=self.tmplTaskFWaresPallet, data=[tw, t, {'mes':mes, 'reloadurl':'taskF?id=%s'%(tw['TID'])}])
    ...
    def qTaskMListWares(self, tid, wid=None):
        ...
        try:
            ...
            raise HTTPRedirect('taskM?id=%s&mes=%s' % (tid, self.fbExcText(exc)))
        else:
            return w
    ...
    def taskM(self, id, barcode=None, mes=None):
        ...
        if barcode:
            ...
            if bcInfo:
                ...
                bc = self.whBarCodeWaresInfo(barcode)
                if len(bc['datalist'])==1 and bc['datalist'][0]['usercode'] == 'WARES':
    ...
    def taskMWaresSave(self, **args):
        ...
        try:
            ...
            url = 'taskMWares?tid=%s&wid=%s&mes=%s' % (args['taskid'], args['waresid'], self.fbExcText(exc))
