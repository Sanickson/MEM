...
class TSelect(TCommonTerm):
    ...
    def taskJoin(self, id):
        ...
        except Exception as exc: raise HTTPRedirect('listTaskes?mes=%s' % (exc[1]))
    ...
    def qTaskSetSite(self, tid, barcode):
        ...
        except Exception as exc: raise HTTPRedirect('taskEndSite?tid=%s&mes=%s' % (tid, exc[1]))
    ...
    def qTaskSetTransUnit(self, tid, barcode):
        ...
        except Exception as exc: raise HTTPRedirect('taskTransUnit?id=%s&mes=%s' % (tid, exc[1]))
    ...
    def qTaskCWaresNoPlace(self, tid, twid):
        ...
        except Exception as exc: raise HTTPRedirect('taskC?id=%s&mes=%s' % (tid, exc[1]))
    ...
    def taskCtwClear(self, twid):
        ...
        except Exception as exc: raise HTTPRedirect('taskCWares?twid=%s&mes=%s' % (twid, self.fbExcText(exc[1])))
    ...
    def qtaskWPalletWares(self, palletid, waresid):
        ...
        except Exception as exc: raise HTTPRedirect('taskWPallet?palletid=%s&mes=%s' % (palletid, self.fbExcText(exc[1])))
        else: return pw
            ...
            if bcInfo and bcInfo['result']==0:
                if bcInfo['usercode']=='WARESWEIGHT': 
                    if waresid==self.kId(bcInfo['RECORDID']):
                        ...
                        except Exception as exc: mes = self.fbExcText(exc[1])
                    else: mes = _('Invalid barcode')
    ...
    def qTaskFWaresNoPlace(self, twid):
        """ Нет места на поддоне """
        try: t = self.dbExec(sql='select * from K_WH_SELECT_POS_ENDTASK(?)', params=[twid], fetch='one')
        except Exception as exc: raise HTTPRedirect('taskFWares?twid=%s&mes=%s' % (twid, exc[1]))
    ...
    def qTaskFWaresNo(self, twid):
        ...
        except Exception as exc: raise HTTPRedirect('taskFWaresPallet?twid=%s&mes=%s' % (twid, exc[1]))
        else: 
            tw = self.qTaskWares(twid)
    ...
    def taskFWaresWClear(self, twid):
        """ Очистить вес """
        try: self.dbExec(sql='execute procedure K_WH_SELECT_SETWEIGHT(?,?,?)',params=[twid,self.getIfaceVar('wmsid'),None],fetch='none')
        except Exception as exc: raise HTTPRedirect('taskFWares?twid=%s&mes=%s' % (twid, self.fbExcText(exc[1])))
        else: raise HTTPRedirect('taskFWares?twid=%s'%(twid))
