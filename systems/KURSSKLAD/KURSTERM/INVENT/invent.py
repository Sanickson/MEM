...
from systems.KURSSKLAD.KURSTERM.INVENT.templates.taskAPalletFP import taskAPalletFP
...
from systems.KURSSKLAD.KURSTERM.INVENT.templates.taskAStackChkF import taskAStackChkF
from systems.KURSSKLAD.KURSTERM.INVENT.templates.taskAStackChkFConf import taskAStackChkFConf
# L
...
class TInvent(TCommonTerm):
    ...
    tmplTaskAPalletFP = taskAPalletFP
    ...
    tmplTaskAStackChkF = taskAStackChkF
    tmplTaskAStackChkFConf = taskAStackChkFConf
    # L
    tmplTaskL = taskL
    ...
    def main(self, mes=None):
        ...
        if t['datalist'] and t['datalist'][0]['ALGORITHM'] == 'L':
            tid = t['datalist'][0]['TASKID']
            # self.qTaskLCheckJoinedSite(tid, mes)
            raise HTTPRedirect('taskL?tid=%s&mes=%s' % (tid, mes))
    ...
    def taskAStackDo(self, tid, sid):
        try:
            ...
            return self.fbExcText(exc)
        else:
            raise HTTPRedirect('taskA?tid=%s' % tid)

    def taskAStackFDo(self, tid, sid, cnt):
        try:
            self.dbExec('execute procedure WH_TERM_INVENT_STACKFDO(?,?,?)', [tid, sid, self.getIfaceVar('wmsid')], 'none')
        except Exception as exc:
            raise HTTPRedirect('taskAStackChkFull?tid=%s&sid=%s&cnt=%s&mes=%s' % (tid, sid, cnt, self.fbExcText(exc)))
        else:
            raise HTTPRedirect('taskA?tid=%s' % tid)

    taskAStackFDo.exposed = True
    ...
    def taskAStackCnt(self, tid, sid, cnt1=None, cnt2=None, mes=None):
        ...
        if cnt1 is None:
            ...
            if s['S_PALCURCNT']:
                s_palcurcnt = int(s['S_PALCURCNT'])
            else:
                s_palcurcnt = 0
            if int(cnt1) != s_palcurcnt:
                params['cnt1'] = cnt1
            else:
                params['mes'] = self.taskAStackDo(tid=tid, sid=sid)
        elif cnt1 != cnt2:
            params['mes'] = _('Количества не совпадают пересчитайте еще раз')
        else:
            raise HTTPRedirect('taskAStackChkFull?tid=%s&sid=%s&cnt=%s' % (tid, sid, cnt1))
    ...
    def taskAStackChkFull(self, tid, sid, cnt, barcode=None, mes=None, conf='0'):
        s = self.siteStackInfo(siteid=sid)
        t = self.taskInfo(tid)

        if barcode:
            try:
                rchk = self.dbExec('select * from WH_TERM_INVENT_STACK_FPALSET(?,?,?,?,?)',
                                   [tid, sid, self.getIfaceVar('wmsid'), barcode, conf], 'one')
            except Exception as exc:
                mes = self.fbExcText(exc)
            else:
                if rchk and rchk['confmes']:
                    return self.drawTemplate(templ=self.tmplTaskAStackChkFConf, data=[s, t, rchk, {'mes': mes, 'cnt': cnt}])
        p = self.dbExec('select * from WH_TERM_INVENT_STACK_CHKPALLIST(?,?)', [tid, sid], 'all')
        return self.drawTemplate(templ=self.tmplTaskAStackChkF, data=[s, t, p, {'mes': mes, 'cnt': cnt}])

    taskAStackChkFull.exposed = True
    ...
    def taskAPallet(self, tid, pid, mes=None):
        ...
        t = self.taskInfo(tid)
        if p and p['PTYPECODE'] == 'FP':
            w = self.dbExec('select * from WH_PALLETFP_INFO(?)', [pid], 'one')
            templ = self.tmplTaskAPalletFP
        else:
            w = self.dbExec('select * from WH_TERM_INVENT_PALLETWARES(?,?)', [pid, self.getIfaceVar('wmsid')], 'all')
            templ = self.tmplTaskAPallet
        params = {'mes': mes, 'treeName': _('Поддон')}
        return self.drawTemplate(templ=templ, data=[p, t, w, params])
    ...
    def taskAScanPalletWares(self, tid, pid, barcode, wid=None):
        def redirect():
            if wid:
                raise HTTPRedirect(
                    'taskAPalletWares?tid=%s&pid=%s&wid=%s&mes=%s' % (tid, pid, wid, _('Ничего не найдено или ШК не обрабатывается!')))
            else:
                raise HTTPRedirect(
    ...
    def taskAPalletWaresSave(self, **args):
        ...
        try:
            ...
            args['pid'], args['wid'], args['tid'], self.fbExcText(exc))
    ...
    def taskAPalletSiteSet(self, tid, pid, sid):
        ...
        try:
            ...
            raise HTTPRedirect('taskAPalletSite?tid=%s&pid=%s&sid=%s&mes=%s' % (tid, pid, sid, self.fbExcText(exc)))
        else:
    ...
    def taskAPalletDelete(self, tid, sid, pid=None):
        if pid:
            try:
                ...
                raise HTTPRedirect('taskASite?tid=%s&sid=%s&mes=%s' % (tid, sid, self.fbExcText(exc)))
        params = {
    ...
    def qTaskLJoinPallet(self, tid, pid):
        ...
        try:
            ...
            raise HTTPRedirect('taskL?tid=%s&mes=%s' % (tid, self.fbExcText(exc)))
    ...
    def taskLPalletDelete(self, tid, wid, pid):
        try:
            ...
            raise HTTPRedirect('taskLPallet?tid=%s&pid=%s&wid=%s&mes=%s' % (tid, pid, wid, self.fbExcText(exc)))
        else:
            raise HTTPRedirect('taskL?tid=%s' % (tid))
    ...
    def taskLPalletSiteSet(self, tid, pid, sid, wid):
        ...
        try:
            ...
            raise HTTPRedirect(
                'taskLPalletSite?tid=%s&pid=%s&sid=%s&wid=%s&mes=%s' % (tid, pid, sid, wid, self.fbExcText(exc)))
        else:
