...
class RefillSlot(WHCommon):
    ...
    def listWares(self, objid, dbeg, dend):
        try:
            data = self.dbExecC(sql='select * from K_REFILLSLOT_LISTWARES(?,?,?) order by SNAME',params=[objid,dbeg,dend],fetch='all')
    ...
    def createTask(self, objid, waresid, amount=None):
        try:
            data = self.dbExecC(sql='select * from K_REFILLSLOT_TASKCREATE(?,NULL,?,NULL)',params=(objid,waresid), fetch='one')
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
