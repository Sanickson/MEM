...
def MinutesBetweenDateTimes(dtBegin, dtEnd):
    ...
    diff = dtEnd - dtBegin
    return diff.days * 24 * 60 + int(diff.seconds / 60)
...
def mxToday():
    ...
    return t - datetime.timedelta(hours=t.hour, minutes=t.minute, seconds=t.second, microseconds=t.microsecond)
...
def StrTomxTime(_timeStr, fmt={'hour':'%H', 'min':'%M'}, delim=":"):
    ...
    _time = datetime.timedelta(hours=h, minutes=m)
...
def formatMxDateTime(mxDateTime, format=None, id_system=None, sformatFrom=None):
    ...
    if sformatFrom is None:
        sformatFrom = '%Y-%m-%d' if sdate.find(' ')==-1 else '%Y-%m-%d %H:%M:%S'
...
def LastTodayTime():
    return mxToday() + datetime.timedelta(hours=23, minutes=59, seconds=59)
