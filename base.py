...
from systems.ENGINE.templates.exit_androidterm import exit_androidterm
from systems.ENGINE.templates.exit_byebye import exit_byebye
import sys
import fdb
...
class BasePage(SuperBasePage):
    ...
    def index(self, id_system=None):
        """Инициализируем параметры системы, в которую вошли.
        ...
        if id_system is not None:
            ...
            if not dis:
                ...
                return cpu.goToError(_('Подсистема не найдена либо отключена.'))
            if dis['DISABLED']:
    ...
    def clearSesVars(self):
        "accuracy clears sessions variables (save id_system, user_init_class, sys_options, termnum, forceiface)"
        ...
        forceiface = self.getSesVar('forceiface') # сохраняем, чтоб возвращало правильно и во время тестов на терминальную страницу
    ...
    def tsdAndroidExit(self, code=None):
        ...
        if code:
            if code == cfg.TERMINAL_LOGOFF_BARCODE:
                ...
                if not mes:
                    ...
                    raise cherrypy.HTTPRedirect('exit_byebye')
            else:
                mes = _('Не верный ШК')
        params = {
            'termname': self.getSesVar('termname'),
        ...
        return self.drawTemplate(templ=exit_androidterm, data=[params], draw_wo_login=True)

    tsdAndroidExit.exposed = True

    def exit_byebye(self):
        return self.drawTemplate(templ=exit_byebye, data=[], draw_wo_login=True)
    exit_byebye.exposed = True
    ...
    def exception_inner(self, exception='', title='', tmpl_name=None, data=None):
        ...
        data_all = {'time': dtu.GetCurDateTime(shortYear=None, dateSeparator='.', timeSeparator=':'),
            ...
            'back': 1,
            'termnum': self.getSesVar('termnum'),
            'forceiface': self.getSesVar('forceiface'),
    ...
    def dbExec(self, sql, params=(), fetch='many', **args):
        ...
        #    return self.pyDumps(data={'res':'uidnull', 'mes':'Сессия утеряна! Автоматическое перенаправление на стартовую страницу...', 'location': cfg.init_url})
            #raise cherrypy.HTTPRedirect(cfg.init_url)
        if 'id_system' in args:
            id_system = args['id_system']
        elif hasattr(self, 'id_system') and self.id_system:
            id_system = self.id_system
        else:
            id_system = self.cInt(self.getIfaceVar("id_system"))
        try:
            res = db.dbExec(sql=sql, params=params, fetch=fetch, id_system=id_system, con=None, conEngine=None) # exec on current system
    ...
    def drawTemplate(self, templ='index', data=None, **args):
        """Возвращает cheetah-обработанное html-представление шаблона. Если в качестве шаблона templ используется
        ...
        draw_wo_login = args['draw_wo_login'] if 'draw_wo_login' in args else False
        path = args['path'] if 'path' in args else 'templates'

        # для шаблонов, порождённых от engine_main, в левом верхнем углу рисует комбик изменения зоны
        if type(templ) in (bytes, str):
    ...
    def tmplCallerName(self):
        ind = 0
        prevDrawTemplate = False
        flag = True
        while flag:
            fr = getframe(ind)
            fName = fr.f_code.co_name
            if fName == 'drawTemplate':
                prevDrawTemplate = True
            elif prevDrawTemplate:
                return fName
            flag = not fr.f_back is None
            ind = ind + 1
        return ''
    ...
    def pyDumpsExc(self, exc, data=None, ext_data=None):
        def getJson(errMes, errAll=None, data=None, ext_data=None):
            if ext_data is None:
                ext_data = {}
            ext_data['errMes'] = errMes
            if not errAll is None:
                ext_data['errAll'] = errAll
            return self.pyDumps(data=data, ext_data=ext_data)

        if type(exc) == fdb.ProgrammingError:
            spl = str(exc).split('\\n- ')
            if len(spl) > 4:
                return getJson(errMes=spl[4], errAll=str(exc), data=data, ext_data=ext_data)
            else:
                return getJson(errMes=str(exc), data=data, ext_data=ext_data)
        elif type(exc) == fdb.DatabaseError:
            spl = str(exc).split('\\n- ')
            if len(spl) > 4 and spl[2].find('exception') > -1 :
                return getJson(errMes=spl[4], errAll=str(exc), data=data, ext_data=ext_data)
            else:
                return getJson(errMes=str(exc), data=data, ext_data=ext_data)
        else:
    ...
    def coreQIfaceTblUserColumns(self, tblCode):
        try:
            clm = self.dbExec(sql="select * from IFACETBL_USERCLM(?,?) ", fetch='all',
                              params=[self.getUserVar('uid'), tblCode], id_system=-1)
            th = self.dbExec(sql="select * from IFACETBL_THB(?,?) ", fetch='one',
                             params=[tblCode, self.getUserVar('uid')], id_system=-1)

        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        else:
            ext_data = {'tblCode': tblCode, 'thExt': th['HTMLBL'], 'keyFld': th['KEYFLD']}
            return self.pyDumps(data=clm, ext_data=ext_data)

    coreQIfaceTblUserColumns.exposed = True

    def coreQIfaceTblColspan(self, tblCode):
        try:
            clm = self.dbExec(sql="select * from IFACETBL_THB(?)", fetch='all',
                              params=[tblCode], id_system=-1)
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        else:
            ext_data = {'tblCode': tblCode}
            return self.pyDumps(data=clm, ext_data=ext_data)

    coreQIfaceTblColspan.exposed = True

    def coreQIfaceTblCfgList(self, tblCodes):
        try:
            l = self.dbExec(sql="select * from IFACETBLCFG_LIST(?)", fetch='all', params=[tblCodes], id_system=-1)
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        else:
            return self.pyDumps(data=l)

    coreQIfaceTblCfgList.exposed = True

    def coreQIfaceTblCfgListClm(self, tblId):
        try:
            clm = self.dbExec(sql="select * from IFACETBLCFG_LISTCLM(?,?)", fetch='all',
                              params=[tblId, self.getUserVar('uid')], id_system=-1)
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        else:
            return self.pyDumps(data=clm)

    coreQIfaceTblCfgListClm.exposed = True

    def coreQIfaceTblCfgListClmSave(self, tblId, clmList):
        try:
            self.dbExec(sql="execute procedure IFACETBL_USERCLM_SAVE(?,?,?)", fetch='none',
                        params=[tblId, self.getUserVar('uid'), clmList], id_system=-1)
        except Exception as exc:
            return self.pyDumpsExc(exc=exc)
        else:
            return self.pyDumps(data={'isOk': 1})

    coreQIfaceTblCfgListClmSave.exposed = True
