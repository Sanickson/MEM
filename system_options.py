...
def lock_instance(id_system):
    if int(id_system) not in Locks_Instances:
...
class SysOptions(object):
    ...
    def opt(self, opt_id, opt_attr, obj_id, obj_type='element'):
        ...
        if not opt_id:
            ...
            if v!={}:
                #print "v=", v
                for k in list(v.keys()): #need .keys()!!!
                    if v[k]=={}:
...
def get_exr_users_merged_rights(id_system, sel_users, instance_location='', options_instances_subdir='', include_roles_options=True):
    ...
    #фильтр по sel_users
    for sel_user in sel_users:
        if int(sel_user) in users_view_dic:
                ...
                for cur_users in users:
                    ...
                    for cur_user in user:
                        ...
                        for option in options:         # option = {'id': 'add', 'par1': 'val1', 'par2': 'val2'}
                            ...
                            if id in list(dic.keys()):       # dic[id] = {'par3': 'val3', 'par4': 'val4'}
                                dic[id].update(option) # объединить dic[id] + option => dic[id]
                            else:
                                dic[id] = option
...
def get_exr_role_rights(id_role, id_system=None, conEngine=None):
    """Возвращает список опций и атрибутов по системе id_system и роли id_role.
...
returns:
    ...
    for user_id in sorted(iter(users_view_dic.keys()), lambda user_id1, user_id2: cmp([x for x in users if x['ID']==user_id1][0]['NAME'],
                                                                            [x for x in users if x['ID']==user_id2][0]['NAME'])):
        user_name = [x for x in users if x['ID']==user_id][0]['NAME']
        tree += '<li class="opt_user"><span>%s (%s)</span>' % (user_name, user_id)
        if len(list(users_view_dic[user_id].keys())): # empty users w/o options has no expandabled
            tree += '<ul>'
        ...
        for obj in sorted(iter(users_view_dic[user_id].keys()), lambda korteg1, korteg2: pu.iif(korteg1[1]==korteg2[1], cmp(korteg1[0], korteg2[0]), cmp(korteg1[1], korteg2[1]))):
            ...
            if old_obj_type != obj_type:
                ...
                tree += '<li class="opt_type"><span>Тип: %s (%s)</span><ul>' % ([x for x in types if x['ID']==obj_type][0]['NAME'], obj_type)

            tree += '<li class="opt_obj"><span>Объект: %s (%s)</span><ul>' % (object_names.get(obj_id, ''), obj_id)

            for option_id in list(users_view_dic[user_id][obj].keys()): #option_id = id_opt1
                option_dic = [x for x in options if x['ID']==option_id] # {'NAME':'Редактировать', 'ID':'edit'}
                if option_dic:
        ...
        if len(list(users_view_dic[user_id].keys())): # empty users w/o options has no expandable
...
def get_view_name(options_pattern_location, view_tag):
    return [x for x in get_views(options_pattern_location) if x['ID']==view_tag][0]['NAME']
...
def get_source_select(pattern, element, options_instances_subdir=None, id_system=None, obj_type=None):
        ...
        if source == 'db':
            ...
            if (os.path.isfile(fn)):
                ...
                #print
                source_select = [{'ID': xml.getAttrValue(object, 'id', 'cp1251'), 'NAME': xml.getAttrValue(object, 'name', 'cp1251')} for object in objects]
                ...
                source_select = [dic for i, dic in enumerate(source_select) if dic['ID'] not in [d['ID'] for d in source_select[:i]]]
            else:
                #raise SysOptionsException('Instance is not loaded! XML file "%s" not exists or no rights!' % fn)
                pass # для того, чтобы построить дерево, не имея xml
...
def get_options_exr():
    options = pu.kbToPy(db.dbExec(sql="select * from EXR_OPT_DICT",
                     ...
                     fetch='all',
                     id_system=-1))
    return [{'ID': option['OPT_ID'], 'NAME': option['OPT_NAME']} for option in options]
...
def get_objects_by_type_exr(id_system, obj_type):
    ...
    if type_source == 1:
        ...
        objects = pu.kbToPy(db.dbExec(sql='select * from EXR_OBJ o'
                                      ...
                                      fetch='all',
                                      id_system=-1))
        return [{'ID': object['OBJ_ID'], 'NAME': object['OBJ_NAME']} for object in objects]
...
def get_attrs(id_system, type_id, obj_id, opt_id, options_instances_subdir):
    ...
    if (os.path.isfile(fn)):
                        ...
                        for cur_user in user:
                            ...
                            for option in options:         # option = {'id': 'add', 'par1': 'val1', 'par2': 'val2'}
                                ...
                                if option['id'] == str(opt_id):
                                    for id, val in list(option.items()):
                                        if id != 'id':
        ...
        #source_select = set(source_select) #элементами не могут быть словари
        #source_select = [x for x in source_select]
        attrs = [dic for i, dic in enumerate(attrs) if dic['ID'] not in [d['ID'] for d in attrs[:i]]]
    else:
        #raise SysOptionsException('Instance is not loaded! XML file "%s" not exists or no rights!' % fn)
        pass # для того, чтобы построить дерево, не имея xml
    return attrs
...
def get_attrs_exr(id_system, obj_id, opt_id):
    ...
    attrs = pu.kbToPy(db.dbExec(sql='select distinct a.ATTR_NAME, a.ATTR_VAL, a.MERGE_SYMBOL'
                     ...
                     fetch='all',
                     id_system=-1))
    return [{'ID': attr['ATTR_NAME'],
                             'VALUE': attr['ATTR_VAL'],
                             'MERGE_SYMBOL': attr['MERGE_SYMBOL']} for attr in attrs]
...
def save_tree(options_instances_subdir, id_system, so_js_dic):
    ...
    try:
        ...
        for id_user, dic in list(so_js_dic.items()):
            ...
            # добавляем опции
            for js_key, options in list(dic.items()): #js_key='id_objA(name_obj)type_objA', options={id_opt1: {par1: val1, par2: val2}, id_opt2: {par3: val3, par4: val4}}
                ...
                for id_opt, attrs in options.items(): #id_opt - id_opt1, attrs - {par1: val1, par2: val2}
