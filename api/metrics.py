...
def metric(fn):
    """Декоратор для методов-метрик классов Metric, расположенных в файлах metric.py в папках систем
...
def get_methods(system):
    "Получает методы-метрики по системе, имеющей класс метрик"
...
class BaseMainMetric(BasePage):
    """Класс для маппинга пустых корневых метрик, не имеющих определённых разработчиком метрик.
    ...
    def index(self, id_system=None):
        "Отрисовует сведения о классе метрик и его методах"
    ...
    def get_system(self, obj):
        "Получить сведения о классе метрик"
        import py_utils as pu
...
class MetricMixin(BaseMainMetric):
    """Базовый класс для метрик подсистем"""
    STATE_OK = 200
    STATE_ERROR = 500
...
class Metrics(BasePage):
    ...
    def index(self, id_system=None):
        "Отрисовует список всех систем-метрик"
        BasePage.index(self)
        return self.drawTemplate(templ=metrics_main, data=[{'systems': self.get_systems(self)}])
    index.exposed = True

    def get_systems(self, root):
        "Получить рекурсивный список всех дочерних систем-метрик (без методов-метрик) и их потомков"
        import py_utils as pu
        systems = []
    ...
    def mapMetric(self, root, mapTo):
        """Маппит к объекту mapTo методы классов Metric,
    ...
    def mapErr(self, err_str, id_system):
        """Записывает в логи ошибки маппинга систем-метрик.
