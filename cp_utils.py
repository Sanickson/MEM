...
class Server(object):
    ...
    def read_config(self):
        ...
        '/':  {
                ...
                'tools.encode.encoding': cfg.CONTENT_CODING,  # установить явно заголовки http-ответа в нужную нам кодировку
                ...
                'tools.sessions.locking': 'early', #'explicit','implicit'
        ...
        if cfg.server1:
            server1_cfg = {'server.socket_host': cfg.socket_host1, 
                           'server.socket_port': cfg.socket_port1, 
                           'server.socket_timeout': getattr(cfg, 'socket_timeout1', 1),
                           'server.thread_pool': getattr(cfg, 'thread_pool', 20)}
            ...
            if cfg.server2:
                server2_cfg = {'server.2.socket_host': cfg.socket_host2, 
                               'server.2.socket_port': cfg.socket_port2, 
                               'server.2.socket_timeout': getattr(cfg, 'socket_timeout2', 1),
                               'server.2.thread_pool': getattr(cfg, 'thread_pool2', 20)}
            ...
            server2_cfg = {'server.socket_host': cfg.socket_host2, 
                           'server.socket_port': cfg.socket_port2,
                           'server.socket_timeout': getattr(cfg, 'socket_timeout2', 1),
                           'server.thread_pool': getattr(cfg, 'thread_pool2', 20)}
            if getattr(cfg, 'server2', None) == 'https':
    ...
    #wait=False - грязный стоп - с ошибками kinterbasdb в логе (except их не ловит)
    def stop_webserver(self, wait, start_updater=False):
        cherrypylog("App engine shutting down from web-interface", context='APP.WEBSERVER')
        from py_utils import is_linux, iif
        if (wait):
            ...
            if start_updater and is_linux():
                user = cfg.upd_ssh_user
                ip = cfg.upd_ssh_ip
                port_ssh = cfg.upd_ssh_port
                port_ssh = iif(port_ssh, port_ssh, 22)
                #SSH ='sudo -u %s ssh -p %s %s@%s ' % (user, port_ssh, user, ip)
                SSH ='ssh -p %s %s@%s ' % (port_ssh, user, ip)
                #sql_log = os.path.join(sql_log_path, sql + cfg.sql_log_suffix)
                #res = os.system(SSH + '"cd %s;./isql -database %s -user %s -password %s -i %s >>%s 2>&1"' % (base['FB_BIN_PATH'], get_db_action_location(base), base['UPD_USER'], base['UPD_PASS'], file_location, sql_log))
                #res = os.system(SSH + '"sudo sh -c \\\"cd %s;./isql %s -u %s -p %s -i %s >>%s 2>&1\\\""' % (base['FB_BIN_PATH'], get_db_action_location(base), base['UPD_USER'], base['UPD_PASS'], file_location, sql_log))
                #res = os.system(SSH + '"echo -e \\\"%s\\n\\\" | sudo -S sh -c \\\"%s >>%s 2>&1\\\""' % ('123456', cfg.upd_location,
                #command = SSH + '"echo -e \\"%s\\n\\" | sudo -S sh -c \\"%s >>%s 2>&1\\""' % ('123456', cfg.upd_location,
                #    iif('/' in cfg.error_log_file or '\\' in cfg.error_log_file,
                #        cfg.error_log_file,
                #        self.basepath + '/log/' + cfg.error_log_file))
                command = SSH + '"sudo %s"' % (cfg.upd_location)
                #print command
                #res = os.system(SSH + '"sudo %s "' % (cfg.upd_location))
                #res = os.system(command)  # exec os cmd with waiting and handle errors
                subprocess.Popen(command, shell=True)  # exec os cmd without waiting and no handle errors
    ...
    def getCfgParams(self):
        return {
           ...
           'socket_timeout1':getattr(cfg, 'socket_timeout1', 1),
           'thread_pool':getattr(cfg, 'thread_pool', 20),
           'server2':cfg.server2,
           'socket_host2':cfg.socket_host2,
           'socket_port2':cfg.socket_port2,
           'socket_timeout2':getattr(cfg, 'socket_timeout2', 1),
           'thread_pool2':getattr(cfg, 'thread_pool2', 20),
           'db_path':cfg.db_path,
           'db_user':cfg.db_user,
           'gzip_on':cfg.gzip_on
        }
...
def exit(module, redirect=True):
    ...
    forceiface = getSesVar('forceiface')
    ...
    if forceiface is not None:
        setSesVar('forceiface', forceiface)
    #if ip_addr is not None:
    #    setSesVar('ip_addr', ip_addr)

    if redirect:
        # return goToIndex() # - не учитывает лишь установленный forceiface без isTSDByHeaders при работе с PC

        # в случае терминала или forceiface в SysLogin.index определяется какую страницу рисовать по http-заголовкам
        # или сохранённым здесь getSesVar('forceiface') getSesVar('forceiface')

        # при потере termnum и при входе с PC с forceiface=1 отрисовуем страницу для PC, иначе сервер попытается перенаправить
        ...
        # при потере termnum и при входе с forceiface=1 и для мобильного устройства и для PC отрисовуем мобильную страницу
        elif isMobile():
...
def setIfaceVar(module, var, val):
    ...
    #cherrypy.session.acquire_lock()
    try:
        if iname + 'IfaceSessionStorage' in cherrypy.session:
...
def setUserVar(var, val):
    ...
    if 'anonymous_init_dic' in cherrypy.session:
...
def isTSDByHeaders():
    if type(getattr(cfg, 'UA_TERMINAL_BROWSER', None)) in (str,):
        return getattr(cfg, 'UA_TERMINAL_BROWSER', None) in cherrypy.request.headers.get('User-Agent', [])
    elif type(getattr(cfg, 'UA_TERMINAL_BROWSER', None)) == list or type(getattr(cfg, 'UA_TERMINAL_BROWSER', None)) == tuple:
...
def isMobileByHeaders():
    if type(getattr(cfg, 'UA_MOBILE_BROWSER', None)) in (str,):
        return getattr(cfg, 'UA_MOBILE_BROWSER', None) in cherrypy.request.headers.get('User-Agent', [])
    elif type(getattr(cfg, 'UA_MOBILE_BROWSER', None)) == list or type(getattr(cfg, 'UA_MOBILE_BROWSER', None)) == tuple:
...
def isTSD():
    if getSesVar('forceiface') == 'T':
        return True
    return isTSDByHeaders()

def isMobile():
    if getSesVar('forceiface') == 'M':
        return True
    return isMobileByHeaders()

def isPC():
    return not isTSD() and not isMobile()
...
def goToIndex(mes='Сессия утеряна! Автоматическое перенаправление на стартовую страницу...'):
    """Перенаправление на СТАРТОВУЮ страницу для PC- и терминальных интерфейсов
...
def goToError(mes, from_root=False, title=''):
    """ Перенаправление на страницу с описанием ошибки в случае какого-либо системного сообщения(ошибки)
...
def HTTPRedirect(url):
    """Перенаправление на страницу url, которая сохраняется в истории браузера (нажатие F5 оставляет на ней)
...
def truncSQLException(s):
    """Вырезка текста ошибки из exception Firebird.
...
def import_exc_tmpl(tmpl_name=None):
    """Импорт шаблона сообщений об ошибках с учётом вида оборудования и клиента.
...
def handle_error():
    """Глобальная обработка неотловленных ошибок.

    """
    from py_utils import iif, convToBytes, convToConsole, is_local_var, is_args, is_kwargs, is_parent_var, is_simple_argument, decodeXStr, format
    ...
    if isAjax():
        ...
        if errtype.find("AjaxException") != -1:
            ...
            #ответ юзерам
            cherrypy.response.body = [convToBytes(secureDbPaths(dat))]
...
def th_number_to_id(th_number):
    for thread_ident, i in cherrypy.engine.thread_manager.threads.items():
        if i == th_number:
            return thread_ident
...
def set_coding_hooks():
    ...
    def _attempt_charset():
        ...
        if not isAjax(): # для Ajax оставляем utf-8, определённый в _cpreqbody.py (class Entity)
            cherrypy.request.body.attempt_charsets.insert(0, 'utf-8')
    ...
    def form_params_encoding():
        ...
        coding = iif(isAjax(), 'utf-8', 'utf-8')

        for key, value in list(cherrypy.request.params.items()):
            if not isinstance(value, list):
                if isinstance(value, str):
                    if bytes == type(value):
                        cherrypy.request.params[key] = value.decode(coding)
            else:
                for i in range(len(value)):
                    if isinstance(value[i], str):
                        if bytes == type(value):
                            value[i] = value.decode(coding)
    ...
    def _query_string_encoding():
        ...
        cherrypy.request.query_string_encoding = iif(isAjax(), 'utf-8', 'utf-8')
    #cherrypy.request.hooks.attach(point="on_start_resource", callback=_query_string_encoding, failsafe=None, priority=50)

    def onFinalize():
        if not 'UTF-8' in cherrypy.response.headers.encodings:
            cherrypy.response.headers.encodings.append('UTF-8')
    cherrypy.request.hooks.attach(point="before_finalize", callback=onFinalize, failsafe=None, priority=50)
...
#
# @cherrypy.tools.register('on_end_resource')
# def onEndResource():
#     cherrypy.response.headers.encodings.append('UTF-8')
#     print(cherrypy.request)
...
def getConfigLangs():
    ...
    langs_dic = dict([(lang['LANG_CODE'], lang['LANG_NAME']) for lang in langs['datalist']])

    for lang in cfg.LOCALE_ALL:
        if lang in list(langs_dic.keys()):
