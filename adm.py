...
errmsgUserNotFoundAD = _('Логин пользователя не найден в Active Directory')
...
class EAdm(Exception):
    "Класс ошибок процедур БД Engine, возвращающих параметры ERROR_CODE и ERROR_MSG"
...
class EUserAdd(EAdm):
    "Класс ошибок добавления пользователя"
    pass

class EUserAddBadLogin(EUserAdd):
    "Ошибка логина несистемного или системного пользователя"
    pass
...
class EUserAddBadEmail(EUserAdd):
    "Почтовый адрес (email) введен неверно"
    pass
...
class EUserEdit(EAdm):
    "Класс ошибок редактирования пользователя"
    pass
...
class EUserNotFoundAD(EUserEdit):
    "Пользователя с заданным логином не существует в домене"
    pass
...
class EUserDel(EAdm):
    "Класс ошибок удаления пользователя"
    pass

class EUserChangePassword(EAdm):
    "Класс ошибок принудительного установки/изменения пароля пользователя без проверки старого пароля"
    pass
...
class EUserVerifyAndChangePassword(EAdm):
    "Класс ошибок установки/изменения пароля пользователя с проверкой старого пароля"
    pass
...
class ERightAdd(EAdm):
    "Ошибка добавления прав пользователю"
    pass

class ERightAddUserNotFound(ERightAdd):
    "Пользователь, которому назначается право, не найден"
    pass

class ERightDel(EAdm):
    "Ошибка снятия прав пользователю"
    pass

class ERightAddDefaults(ERightAdd):
    "Ошибка добавления умалчиваемых прав пользователю"
    pass
...
class ERoleOptAdd(EAdm):
    """Класс ошибок добавления опции роли
...
class ERoleOptAddOptNotFoundInDict(ERoleOptAdd):
    """Опция не найдена в справочнике опций

    """
    pass

class ERoleAdd(EAdm):
    """Класс ошибок добавления роли

    """
    pass

class ERoleAddBadRoleCode(ERoleAdd):
    """Ошибка несистемного или системного кода роли
...
class ERoleAddExistsADGroupName(ERoleAdd):
    """Имя группы AD уже занято

    """
    pass

class ERoleAddExistsRoleName(ERoleAdd):
    """Название роли уже занято

    """
    pass

class ERoleEdit(EAdm):
    """Класс ошибок редактирования роли
...
class ERoleEditExistsADGroupName(ERoleEdit):
    """Имя группы AD уже занято
...
class ERoleDel(EAdm):
    """Класс ошибок удаления роли

    """
    pass

class ERoleDelSystemRoleAccessDenied(ERoleDel):
    """Нет прав удалять системную роль

    """
    pass

class ERoleAssign(EAdm):
    """Класс ошибок назначения роли

    """
    pass

class ERoleAssignUserNotFound(ERoleAssign):
    """Пользователь, которому назначается роль, не найден

    """
    pass

class ERoleUnassign(EAdm):
    """Класс ошибок снятия роли с пользователя

    """
    pass

class ERoleRightAdd(EAdm):
    """Класс ошибок назначения права роли на систему

    """
    pass

class ERoleRightDel(EAdm):
    """Класс ошибок снятия права роли с системы
...
class EBarcodeGen(EAdm):
    """Класс ошибок генерации ШК

    """
    pass

class EBarcodeDel(EAdm):
    """Класс ошибок удаления ШК
...
def _add_user_to_base(id_user, old_id_user, id_base, fio, conEngine, conDb, guid_1c='unmodified', conGroup=None):
    """Процедура для внутреннего использования.
    ...
    if id_base is not None and id_base != -1:
        params = db.dbExec(sql="""select B.USERS_TABLE_NAME, B.USERS_TABLE_ID_FIELD, B.USERS_TABLE_FIO_FIELD
                      ...
                      fetch='one',
                      id_system= -1,
                      con=conEngine, conEngine=conEngine, congroup=conGroup)
        ...
        d2 = db.dbExec(sql="""SELECT FIRST 1 %(users_table_id_field)s
                     ...
                     params=(old_id_user,),
                     fetch='one',
                     con=conDb, conEngine=conEngine, congroup=conGroup)
        if d2 is not None:
            # нашли по id
            # print "нашли по ID"
...
def _try_del_user_from_base(id_user, id_base, conEngine=None, conDb=None, checkRights=True, conGroup=None):
    """Процедура для внутреннего использования.
    ...
    if id_base is not None and id_base != -1:
        ...
        d = db.dbExec(sql="""SELECT FIRST 1 r.ID_ENGINE_RIGHT
                     ...
                     fetch='one',
                     id_system= -1,
                     con=conEngine, congroup=conGroup)
            ...
            if d2 is None or not checkRights:
                ...
                if users_table_name and users_table_id_field:
                    db.dbExec(sql="""DELETE FROM %(users_table_name)s
                                 ...
                                 #id_system=id_system,
                                 con=conDb,
                                 conEngine=conEngine, congroup=conGroup
                                )
...
def usersList(layer_id=None, include_superadmins=False, conEngine=None, fio=None, email=None, ad=None, status=None):
    ...
    """
    return pu.kbToPy(db.dbExecC(sql="select * from ENGINE_USER_GET_INFO(?,?,?,?,?,?,?)",
                               params=[None, layer_id, int(include_superadmins), fio, email, ad, status],
                               fetch='all',
                               id_system= -1,
                               con=conEngine))
...
def userInfo(uid=None, conEngine=None):
    ...
    {'ID_USER': ID_USER, 'FIO': FIO, 'EMAIL': EMAIL, 'PHONENUMBER': PHONENUMBER, 'LAYER_ID': LAYER_ID, , 'LANG_CODE': LANG_CODE, 'IS_SUPERADMIN': IS_SUPERADMIN,
     'LOGIN': LOGIN, 'LASTDATE': LASTDATE, 'COMMENTS': COMMENTS, 'LIST_ROLES': LIST_ROLES, 'CNT_ROLES': CNT_ROLES, 'AUTHOR_AD': AUTHOR_AD}
...
def userUidByLogin(login=None, conEngine=None):
    """Получить uid пользователя по логину.

    :param login: логин пользователя
    :type login: str
    :param conEngine: объект соединения с БД Engine. Если передан, то доступ к ней происходит в его контексте и без commit'а в конце
    :type conEngine: объект соединения kinterbasdb
    :returns: uid или None, если задан несуществующий логин.

    """
    if login is None:
        return None

    uid_dic = db.dbExec(sql="select first 1 ID_USER from ENGINE_USERS where login=?", params=[login], fetch='one', id_system= -1, con=conEngine)
    if uid_dic is None:
        return None
    return uid_dic['ID_USER']

def userRights(uid=None, spaces_per_level=24, higher_ref=None, recurse=1, superadmin_see_all=False, conEngine=None):
    """Древовидный список доступных пользователю систем, без учёта прав его ролей,
...
def userRoles(uid=None, conEngine=None, only_assigned=False):
    ...
    :param conEngine: объект соединения с БД Engine. Если передан, то доступ к ней происходит в его контексте и без commit'а в конце
    :type conEngine: объект соединения kinterbasdb
    :param only_assigned: выбрать только роли, назначенные пользователю (True), или весь список ролей, в котором индикатором назначенности роли пользователю служит поле ID_USER_ROLE is not None (False)
    :type only_assigned: bool
    :returns:
    Cписок прав, отсортированный по имени роли, в виде списка словарей:
        [{'ID_ROLE': ID_ROLE, 'ROLE_NAME': ROLE_NAME, 'LASTDATE': LASTDATE, 'ID_USER_ROLE': ID_USER_ROLE, 'ROLE_CODE': ROLE_CODE, 'AD_GROUP_NAME': AD_GROUP_NAME},
        ...
        ],
        где
        ID_ROLE - ID роли
        ...
        LASTDATE - дата/время последнего добавления/изменения права
        ID_USER_ROLE - ID связи пользователя и роли. Если None, то роль не назначена пользователю, иначе - назначена
        ROLE_CODE - служебный код роли
        AD_GROUP_NAME - название соответствующей роли группы в Active Directory

    """
    if uid is None:
        uid = cpu.getUserVar('uid')

    return pu.kbToPy(db.dbExec(sql='select r.ID_ROLE, r.ROLE_NAME, eur.LASTDATE, eur.ID_USER_ROLE, r.ROLE_CODE, r.AD_GROUP_NAME'
                                   '       from engine_roles r'
                                   + pu.iif(not only_assigned, ' left', '') +
                                   '         join engine_user_roles eur on r.id_role=eur.id_role and eur.id_user=?'
...
def userIsSuperAdmin(uid=None, conEngine=None, use_cache=False):
    """Проверка, является ли пользователь с ID=uid суперадмином.
...
def userValidateLogin(login, system_user=False):
    """Является ли валидным логин пользователя.
...
def userValidatePassword(password):
    """Является ли валидным пароль пользователя (достаточна его сложность).
...
def userValidateEmail(email):
    """Является ли валидным email пользователя. Пустой email считается валидным и преобразуется при вставке в БД в NULL.
...
def userCheckLoginUnique(login, id_user=None, conEngine=None):
    """Является ли логин уникальным в БД, без учета пользователя с ID=id_user.
    ...
    return (db.dbExec(sql="select * FROM ENGINE_USER_LOGIN_CHECK(?,?)",
                      ...
                      fetch='one',
                      id_system= -1,
                      con=conEngine)['IS_VALID'] != 0
            )

def userCheckEmailUnique(email, id_user=None, to_return_found_id_user=False):
    """Является ли email уникальным в БД, без учета пользователя с ID=id_user.
    Параметр id_user полезен при редактировании пользователя.
...
def userIsSystemLogin(login):
    """Является ли системным логин пользователя.
...
def userAdd(is_registration, fio, password, login, email=None, phonenumber=None, id_user=None, layer_id=None,
            conEngine=None, handle_manually=False, params_utf8=False, system_user=False, comment=None, author_ad=None):
    ...
    :type uid: bool
    :param comment: комментарий на добавление пользователя
    :type comment: str
    :param author_ad: разрешать доменную авторизацию пользователя
    :type author_ad: bool
    :returns:
    ...
    # login = None if login is None else str(login) if params_utf8 else pu.convToUTF8(login)
    # fio = None if fio is None else str(fio) if params_utf8 else pu.convToUTF8(fio)
    # email = None if email is None else str(email) if params_utf8 else pu.convToUTF8(email)
    # phonenumber = None if phonenumber is None else str(phonenumber) if params_utf8 else pu.convToUTF8(phonenumber)
    # password = None if password is None else str(password) if params_utf8 else pu.convToUTF8(password)
    # comment = None if comment is None else str(comment) if params_utf8 else pu.convToUTF8(comment)
    # author_ad = None if author_ad is None else str(author_ad) if params_utf8 else pu.convToUTF8(author_ad)
...
def userDel(id_user, handle_manually=False, delFromBases=False, layer_id=None):
    """Удаление пользователя. Админом, удалившим пользователя, считается текущий залогиненный пользователь.
        ...
        try:
            try:
                ...
                for (id_base, conDb) in lst_granted_bases:
                    _try_del_user_from_base(id_user=id_user, id_base=id_base,
                                            conEngine=conEngine, conDb=conDb, checkRights=False, conGroup=group)

                # from engine
                d = db.dbExec(sql='select * from ENGINE_USER_DEL(?,?)',
                              params=(cpu.getUserVar("uid"), id_user),
                              fetch='one', id_system= -1,
                              con=conEngine, conEngine=conEngine, congroup=group)
                if d['ERROR_CODE'] != 0 and d['ERROR_CODE'] != 2:
                    raise ESuccess
                    #return self.dump(d)
...
def userEdit(id_user, fio=None, login=None, email=None, phonenumber=None, lang=None, new_id_user=None, layer_id=None,
             handle_manually=False, params_utf8=False, guid_1c='unmodified', comment=None, author_ad=None):
    """Изменение параметров пользователя. Лицом, изменившим пользователя,
    ...
    :param guid_1c: код 1С для интеграции со СКУД. Если guid_1c=='unmodified', то не изменяем его.
    :type guid_1c: str
    :param comment: комментарий на редактирование пользователя
    :type comment: str
    :param author_ad: разрешать доменную авторизацию пользователя
    :type author_ad: bool
    :returns:
    ...
    #comment = None if comment is None else pu.convToWin(comment) if params_utf8 else comment
    #author_ad = None if author_ad is None else pu.convToWin(author_ad) if params_utf8 else author_ad
    ...
    if int(author_ad) == 1:
        server = ldap3.Server(cfg.AD_SERVER)
        conn = ldap3.Connection(server, user=cfg.AD_USER, password=cfg.AD_PASSWORD)
        conn.bind()
        ldapfilter = '(&(objectCategory=Person)(objectClass=user)(!(UserAccountControl:1.2.840.113556.1.4.803:=2))(sAMAccountName=%s))' % (
        login)
        conn.search(cfg.AD_SEARCH_TREE,
                    ldapfilter,
                    ldap3.SUBTREE,
                    attributes=['cn', 'sAMAccountName']
                    )
        len_f = len(conn.entries)
        conn.unbind()
        if len_f == 0:
            d = {'ERROR_CODE': 10, 'ERROR_MSG': errmsgUserNotFoundAD, 'ID_USER': None, 'LASTDATE': None,
                 'COMMENTS': None}
            if handle_manually:
                return d
            else:
                raise EUserNotFoundAD(d['ERROR_CODE'], d['ERROR_MSG'])
    ...
    try:
        try:
            old_dic = db.dbExec(sql="""SELECT FIO, LOGIN FROM ENGINE_USERS WHERE ID_USER=?""",
                                params=[id_user], fetch='one',
                                id_system= -1, con=conEngine, conEngine=conEngine)
            ...
            if new_id_user != id_user or (fio != old_fio and fio is not None) \
                ...
                for base in db.dbExec(sql="""SELECT distinct es.ID_BASE
                                               ...
                                               WHERE es.higher is null and es.id_base is not null
                                   """,
                              params=[id_user, cfg.systems_order], fetch='all',
                              id_system= -1, con=conEngine, conEngine=conEngine
                             )['datalist']:
                    conDb = db.getConDb(id_base=base['ID_BASE'], conEngine=conEngine, layer_id=layer_id)
                    #if conDb != conEngine: #т.к. es.id_base is not null, то здесь не могут быть системы, коннектящиеся к Engine.FDB
            ...
            d = db.dbExec(sql='select * from ENGINE_USER_EDIT(?,?,?,?,?,?,?,?,?,?,?)',
                              params=(cpu.getUserVar("uid"), id_user, new_id_user, login, fio, email,
                                      phonenumber, lang, layer_id, comment, author_ad),
                              fetch='one', id_system= -1, con=conEngine, conEngine=conEngine, congroup=group)
            ...
            for (id_base, conDb) in lst_granted_bases:
                _add_user_to_base(id_user=new_id_user, old_id_user=id_user, id_base=id_base, fio=fio,
                    conEngine=conEngine, conDb=conDb, guid_1c=guid_1c, conGroup=group)
...
def userChangePassword(id_user, passwd, conEngine=None, handle_manually=False, params_utf8=False, author_ad= None):
    """Установка/изменение пароля пользователя. Лицом, изменившим пароль пользователя,
    считается текущий залогиненный пользователь или никто.
...
def userVerifyAndChangePassword(actor_login, login, oldpasswd, newpasswd, repeatnewpasswd, conEngine=None,
                                handle_manually=False, params_utf8=False, reset_passwd=False, author_ad=None, old_author_ad=None):
    """Установка/изменение пароля пользователя. Лицом, изменившим пароль пользователя,
    ...
    :param author_ad: разрешать доменную авторизацию пользователя
    :type author_ad: bool
    :param old_author_ad: авторизация пользователя до изменения (1-доменная, 0 и none - обычная)
    :type old_author_ad: bool
    :returns:
    Если handle_manually=False, то возвращается
    None
    ...
    #если у пользователя была доменная павторизация, то старый пароль проверяем в домене
    if oldpasswd is not None and int(old_author_ad) == 1:
        login = pu.iif(login is None, cpu.getUserVar("uid"), login)

        server = ldap3.Server(cfg.AD_SERVER)
        conn = ldap3.Connection(server, user=login + '@' + cfg.AD_DOMAIN, password=str(oldpasswd, "windows-1251").encode("utf-8"), raise_exceptions=True)
        conn.open()
        try:
            conn.bind()
        except ldap3.core.exceptions.LDAPInvalidCredentialsResult:
            d = {'ERROR_CODE': 4, 'ERROR_MSG': 'Неверный старый пароль!', 'ID_USER': None, 'LASTDATE': None,
                 'COMMENTS': None}
            if handle_manually:
                return d
            else:
            ...
            exp = {'ERROR_CODE': 140}
            str_e = str(e)
            if "Can't contact LDAP server" in str_e:
                str_e = 'Невозможно соединиться с AD-сервером'
            exp['ERROR_MSG'] = _('Ошибка Active Directory:') + '<br>' + str_e
            return exp
        finally:
            conn.unbind()
    ...
    if int(author_ad) == 1:# не сохраняем пароли
        # None -> default user
        actor_login = pu.iif(actor_login is None, cpu.getUserVar("uid"), actor_login)
        login = pu.iif(login is None, cpu.getUserVar("uid"), login)
        #oldpasswd = newpasswd = repeatnewpasswd = calc_hash_in_db = None
        newpasswd = repeatnewpasswd = calc_hash_in_db = None

        server = ldap3.Server(cfg.AD_SERVER)
        conn = ldap3.Connection(server, user=cfg.AD_USER, password=cfg.AD_PASSWORD)
        conn.bind()
        ldapfilter = '(&(objectCategory=Person)(objectClass=user)(!(UserAccountControl:1.2.840.113556.1.4.803:=2))(sAMAccountName=%s))' % (login)
        conn.search(cfg.AD_SEARCH_TREE,
                    ldapfilter,
                    ldap3.SUBTREE,
                    attributes=['cn', 'sAMAccountName']
    ...
    d = db.dbExec(sql='select * from ENGINE_PASSWD_CHANGE(?,?,?,?,?,?,?,?,?)',
                params=(actor_login, login, oldpasswd, newpasswd, repeatnewpasswd, calc_hash_in_db, reset_passwd, author_ad, old_author_ad),
                fetch='one',
                id_system= -1,
                con=conEngine
               )
...
def userGenLogin(fio=None, system_user=False, system_prefix_code='sys', system_layer_id=None, system_use_salt=False, conEngine=None):
    """Генерирует уникальный login пользоветеля.
    ...
    def get_salt():
        ...
        max_salt_len = 7
        return "".join([random.choice(salt_set) for i in range(max_salt_len)])
...
def getSystemUsers(id_system=None, conEngine=None):
    """Получить пользователей системы id_system через непосредственные права пользователей или права ролей.
...
def rightAdd(is_registration, id_user, id_system, handle_manually=False, layer_id=None):
    """Назначение права пользователю id_user на систему id_system.
    ...
    try:
        ...
        try:
            ...
            _add_user_to_base(id_user=id_user, old_id_user=id_user, id_base=id_base, fio=fio,
                conEngine=conEngine, conDb=conDb, conGroup=group)
...
def rightDel(id_user, id_system, handle_manually=False, layer_id=None, delFromBase=False):
    """Снятие с пользователя права на систему. Администратором, снявшим право с пользователя, считается текущий авторизованный пользователь.
    ...
    if not delFromBase:
        ...
        try:
            ...
            try:
                ...
                # from system
                _try_del_user_from_base(id_user=id_user, id_base=id_base,
                                        conEngine=conEngine, conDb=conDb, checkRights=True, conGroup=group)
...
def rightAddDefaults(is_registration, id_user, handle_manually=False, layer_id=None):
    """Назначение прав на системы по умолчанию пользователю id_user.
...
def roleValidateRoleCode(role_code, system_role=False):
    """Является ли валидной роль с указанным кодом. Системная роль не может иметь пустой код. Несистемная - может.
...
def roleIsSystemRole(role_code):
    """Является ли системной роль с указанным кодом. Пустой код роли считается несистемным
...
def roleAdd(role_name, role_code=None, system_role=False, conEngine=None, handle_manually=False, params_utf8=False,
            ad_group_name=None):
    """Заведение роли. Текущий авторизованный пользователь считается администратором, который завёл роль.
    ...
    :param params_utf8: воспринимать входные параметры в кодировке utf-8, вместо cp1251
    :type params_utf8: bool
    :param ad_group_name: имя группы Active Directory, соответствующее роли
    ...
    Если handle_manually=False, то в случае ошибки генерирует исключения:
        ...
        -ERoleAddExistsADGroupName - Имя группы AD уже занято
    ...
    # ad_group_name = None if ad_group_name is None else pu.convToWin(ad_group_name) if params_utf8 else ad_group_name
    ...
    d = db.dbExec(sql='select * from ENGINE_ROLE_ADD(?,?,?,?)',
                    params=(cpu.getUserVar("uid"), # cpu.getUserVar("uid") не м.б. '', только None
                            ...
                            ad_group_name          # в процедуре '' заменяется на NULL
                           ),
                    fetch='one',
                    id_system=-1,
                    con=conEngine
    ...
    if handle_manually:
        ...
        if d['ERROR_CODE'] != 0:
            ...
            if d['ERROR_CODE'] == 3:
                raise ERoleAddExistsADGroupName(d['ERROR_CODE'], d['ERROR_MSG'])
            else:
...
def roleEdit(id_role, role_name, role_code, conEngine=None, handle_manually=False, params_utf8=False,
             ad_group_name=None):
    """Изменение параметров роли. Текущий авторизованный пользователь считается администратором, который завёл роль.
    Роли работают только БЕЗ СЛОЁВ.
    ...
    Если handle_manually=False, то в случае ошибки генерирует исключения:
        ...
        -ERoleEditExistsADGroupName - Имя группы AD уже занято
    ...
    if handle_manually:
        ...
        if d['ERROR_CODE'] != 0:
            ...
            if d['ERROR_CODE'] == 4:
                raise ERoleEditExistsADGroupName(d['ERROR_CODE'], d['ERROR_MSG'])
            else:
...
def rolesList(includeSystemRoles=False, conEngine=None):
    ...
    Cписок словарей, каждый из которых описывает роль, отсортированный по ROLE_NAME:
            [{'ID_ROLE': ID_ROLE, 'ROLE_NAME': ROLE_NAME, 'ROLE_CODE': ROLE_CODE, 'LASTDATE': LASTDATE, 'COMMENTS': COMMENTS, 'AD_ROLE_NAME': AD_ROLE_NAME},
            ...
            ]

    """
    lst = pu.kbToPy(db.dbExec(sql="select * from ENGINE_ROLES_INFO(?)", params=[None], fetch='all', id_system= -1, con=conEngine))
    lst = [d for d in lst if includeSystemRoles or not roleIsSystemRole(d['ROLE_CODE'])]
    return lst
...
def roleDel(id_role, handle_manually=False, delUsersFromBases=False, forceDelSystemRole=False):
    """Удаление роли. Администратором, удалившим роль, считается текущий авторизованный пользователь.
        ...
        try:
            try:
                ...
                for system in db.dbExec(sql="""select s.id_system
                        ...
                        if id_base not in [d[0] for d in lst_granted_bases]:
                            conDb = db.getConDb(id_base=id_base, conEngine=conEngine, layer_id=None)
                            group.add(conDb)
                ...
                for user in users:
                    for (id_base, conDb) in lst_granted_bases:
                        _try_del_user_from_base(id_user=user['id_user'], id_base=id_base,
                                                conEngine=conEngine, conDb=conDb, checkRights=True, conGroup=group)

                # from engine
                d = db.dbExec(sql='select * from ENGINE_ROLE_DEL(?,?)',
                              params=(cpu.getUserVar("uid"), id_role ),
                              fetch='one', id_system=-1,
                              con=conEngine, conEngine=conEngine, congroup=group )
                if d['ERROR_CODE'] not in (0, 2):
                    raise ESuccess
...
def roleAssign(id_role, id_user, handle_manually=False):
    """Назначение роли. Администратором, назначившим роль, считается текущий авторизованный пользователь.
    ...
    try:
        try:
            ...
            d = db.dbExec(sql='select * from ENGINE_ROLE_ASSIGN(?,?,?)',
                            params=(cpu.getUserVar("uid"), id_role, id_user),
                            fetch='one', id_system=-1,
                            con=conEngine, conEngine=conEngine, congroup=group)
            if d['ERROR_CODE']:
                raise ESuccess
...
def roleUnassign(id_role, id_user, handle_manually=False, delFromBases=False):
    """Снятие роли с пользователя. Администратором, снявшим роль с пользователя, считается текущий авторизованный пользователь.
...
def roleRightAdd(id_role, id_system, handle_manually=False):
    """Назначение права роли. Администратором, назначившим право роли, считается текущий авторизованный пользователь.
    ...
    try:
        ...
        try:
            ...
            for user in db.dbExec(sql="""select EUR.id_user, U.fio from ENGINE_USER_ROLES EUR
                                  ...
                                  con=conEngine, conEngine=conEngine, congroup=group)['datalist']:
...
def roleRightDel(id_role, id_system, handle_manually=False, delUsersFromBase=False):
    """Снятие с роли права на систему. Администратором, снявшим право с роли, считается текущий авторизованный пользователь.
    Если пользователя или роли не существует, это не считается ошибкой.
...
def roleRights(id_role, spaces_per_level=24, higher_ref=None, recurse=1, conEngine=None):
    """Дерево систем, на которые имеет права роль.
...
def roleUsers(id_role, conEngine=None):
    """Список пользователей, которым назначена роль, отсортированный по ФИО.
...
def roleAddOpt(roles, obj_id, obj_name, type_id, id_system, opt_id, attrs, rewrite,
    ...
    """Добавление опции, атрибутов, и, возможно, объекта, одной роли, всем или списку ролей.
...
def roleDelAllOpts(id_role, id_system, conEngine=None):
    """Удалить все опции роли по системе.
...
def barcodeList(id_user=None, conEngine=None):
    """Возвращает список ШК пользователя.
...
def barcodeTypeInfo(id_type=0, conEngine=None):
    """Получить информацию о типе ШК пользователя.
...
def barcodeInfo(id_type=0, id_user=None, conEngine=None):
    """Получить информацию о ШК пользователя.
...
def barcodeGen(id_type=0, id_user=None, conEngine=None):
    """Генерация ШК пользователя заданного типа.
    ...
    if id_type == 0:
        ...
        if type_code == 'POLESOFTCASHIER':
            ...
            barcode = ''.join([random.choice(salt) for x in range(24)])
        else:
