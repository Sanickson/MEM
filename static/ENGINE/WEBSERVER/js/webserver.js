    ...
    function set_timer(freq) {
        tmr = $.timer(freq, function (timer) {
            ...
            if (typeof(beg_time)!='undefined' && typeof(beg_sec)!='undefined'){
                ...
                //alert(0);
                $.getJSON('stop_webserver', {start_updater: ($('#start_updater_chk').attr('checked')?1:0)}, function(json){
                    if (json.mes)
...
//      title: '',
    ...
    $("#updater_logs_btn").click(function(){
      $("#dlgupdlogs").load('upd_logs_dlg',
        function(){
          $("select#ls").unbind('dblclick').dblclick(function(){
              fillUpdLogFileList($('option:selected', $(this)));
          });
          $("#dlgupdlogs_close_btn").unbind('click').click(function(){$("#dlgupdlogs").dialog("close");});
          $("#dlgupdlogs_sort_btn").unbind('click').click(function(){
            var $sort_btn = $(this);
            if ($sort_btn.attr('sort') == "desc"){
                $sort_btn.attr('sort', "asc");
                $('span', $sort_btn).text(' ' + _('Сортировать по убыванию'));
                $('img', $sort_btn).attr('src', '/ENGINE/images/circular/arrow_down.png');
            }
            else {
                $sort_btn.attr('sort', "desc");
                $('span', $sort_btn).text(' ' + _('Сортировать по возрастанию'));
                $('img', $sort_btn).attr('src', '/ENGINE/images/circular/arrow_up.png');
            }
            fillUpdLogFileList();
          });
          $("#only_today").unbind('click').click(function(){
            fillUpdLogFileList();
          });

          $("#dlgupdlogs").show().dialog("open");
        });
    });
  }
);
...
function fillUpdLogFileList($option_selected){
    var subdir = '.';
    if ($option_selected){
        // мы дважды кликнули по опции - надо перейти или в новый каталог или показать файл
        var ar = $option_selected.text().split(/\s+/);
        subdir = ar[0] == 'd' ? ar[1] : '.';
        if (subdir == '.'){
            // показать файл
            $.ajax({async: false,
                ...
                success: function (html, textStatus){
                    $('#dlgupdlogview').html(html);
                    $("#dlgupdlogview_close_btn").unbind('click').click(function(){$("#dlgupdlogview").dialog("close");});
                    $("#dlgupdlogview").show().dialog("open");
                }
            });
            return;
        }
    }
    var only_today = $('#only_today').attr('checked') ? 1 : 0;
    var sort = $("#dlgupdlogs_sort_btn").attr('sort');

    $.ajax({async: false,
        url: "upd_ls_dir",
        data: {subdir: subdir, sort: sort, only_today: only_today},
        dataType: "json",
        success: function (json, textStatus)
        {
            $('select#ls').empty();
            //$('select#ls').append('<option value="..">..</option>');

            for (var i=0; i < json.ext_data.files.length; i++)
                $('select#ls').append(jQuery.format(
                            '<option value="{0}">{0}</option>'
                            ,
                            (json.ext_data.files[i][0]=='d'?'d':'&nbsp;')
                                + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + json.ext_data.files[i][1]
                        ));
            //$("select#ls").trigger("change");
            $('#dlgupdlogs_caption').text(json.ext_data.dir_loc);
        }
    });
}
