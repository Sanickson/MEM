...
(function (jQuery) {
    jQuery.fn.BoxIt = function (options) {
        ...
        var defaults = {
            ...
            unselectTip: 'Отменить все', //tooltip
      ...
      function bi_add_thClmRowSpan(context, el, injection, elClass) {
        var element = jQuery('<' + el + ' rowspan="' + settings.thRowSpan + '">' + injection + "</" + el + ">");
        if (elClass) element.addClass(elClass);
        if (settings.column == 'first') jQuery(context).prepend(element);
        if (settings.column == 'last') jQuery(context).append(element);
        if (!isNaN(settings.column)) jQuery(el + ":nth-child(" + settings.column + ")", context).before(element);
      ...
      function bi_setHighlight(box)
        ...
        return this.each(function () {
            ...
            if (jQuery("thead tr").length > 1 && settings.thRowSpan == 0){
                ...
                jQuery("thead tr", this).each(function(i){
                    ...
                    if (settings.column == 'last') {
                      ...
                      var first_th = jQuery('th:first', this);
                      if (first_th.attr('colspan') && parseFloat(first_th.attr('colspan')) > 1)
                      // If is 'colspan' - we will expand it
                        first_th.attr('colspan', parseFloat(first_th.attr('colspan')) + 1);
                      else
                      // If colspan is not present, we will add a n-th column
                        bi_add_nth_column(this, 'th', '');
                    }
            ...
            else if (settings.thRowSpan > 0 ){
              bi_add_thClmRowSpan(jQuery("thead tr:first"), 'th', '');
              //jQuery("thead tr:first").prepend('<th rowspan="' + settings.thRowSpan + '"></th>');
            }
            ...
            if (isNaN(settings.column)) {
                if (settings.thRowSpan == 0){
                  ...
                  jQuery("thead tr:first th:" + settings.column + "-child", this).append("<input type='checkbox'/><span class='boxitcnt'><span/>");
                  headerCheckbox = jQuery("thead tr:first th:" + settings.column + " input:checkbox", this);
                }
                if(settings.mode == 'extended') {
                  var items = [];
                  if (settings.selectTip)
                    items.push({name: settings.selectTip, selector: 'all'});
                  if (settings.unselectTip)
                    items.push({name: settings.unselectTip, selector: 'none'});
                  if (settings.additionalItems)
                    items = items.concat(settings.additionalItems);
                  //jQuery("thead tr:last th:" + settings.column + "-child input", this)
                  headerCheckbox.dropCheckbox({
                      items: items,
                      width: 130,
                      allSelector: 'all',
                      noneSelector: 'none',
                      callback: function (el, selector) {
                        if (settings.callbackDefBefore) {
