...
var menu_pressed_flag = false;

function KeyDownAction(evt) {
    var keyCode = getKeyCode(evt);
    if (keyCode == codeBackUrl){
      esc_pressed_flag=true;
      return true;
    }
    else if (keyCode == codeMenuUrl){
      menu_pressed_flag=true;
      return true;
    }
    else if (elemIsNumber(getTarget(evt))){
      if (evt.shiftKey){
        // Shift 51 = #
        // Shift 56 = *
        if (keyCode == 51 || keyCode == 56)
          calcucaltorElement = getTarget(evt);
      }
      else{
        // 106 = NumPad *
        if (keyCode == 106)
      ...
      if (GScaning!=false) {
          if (getKeyCode(evt) == 13) {
              if (GScaning == 'hscan'){
                ...
                if (GScanElementForm && frmScanSubmit(GScanElementForm))
                 CancelAction(evt);
              }
              else if (GScaning == 'no'){
                 CancelAction(evt);
              ...
              if (eventKeyDownIsScannerPrefix(evt)) {
                  //KEY Scanner
...
function elemIsNumber(elem){
  var id = elem.id;
  if (!id)
    id='';
  var idSpl = id.split(':');
  return idSpl.length > 1 && (idSpl[2] == 'float' || idSpl[2] == 'int');
}

function calculator(){
  var flagPrompt = true;
  var msg = 'Введите арифметическое выражение, допускаются цифры, *, /, +, -, скобки';
  var str = '';
  while (flagPrompt){
    str = prompt(msg, str);
    if (str !== null){
      if (/[0123456789()*\/+-]+/.test(str)){
        ...
        if (flagEval){
          flagPrompt = false;
          calcucaltorElement.value = v;
        }
        else{
          msg = 'Ошибка вычисления значения выражения';
        }
      }
      else{
        msg = 'В выражении используются запрещенные символы';
      }
    }
    else{
      flagPrompt = false;
...
function KeyUpAction(evt) {
    ...
    //bypass commas
      ...
      if (usePoint) return handlePointKeyUp(evt); //переходы по стрелкам    
        ...
        if (menu_pressed_flag==true){
           menu_pressed_flag = false;
           var menuEl=document.getElementById('tasklist')
           if (menuEl!=null){
              var t = menuEl.href;
              if (t!=null && t!='#') {
                   testflag2=2;
...
function androidPrepare(needFT){
  function getHrefFromA(a){
    var url = '';
    if (a != null){
        var t = a.href;
        if (t!=null && t!='#') {
  ...
  var title = '';
  var elZoneInfo = document.getElementById('zoneinfo');
  if(elZoneInfo != null ){
    title = elZoneInfo.innerText;
  }
  var subTitle = '';
  var elActionCaption = document.getElementById('actioncaption');
  if(elActionCaption != null ){
    subTitle = elActionCaption.innerText;
  }

  var needScanner = '0';
  if (typeof window['getScanElement'] == 'function'){
    needScanner = getScanElement() ? '1' : '0';
  }
  var backUrl = getHrefFromA(document.getElementById('backurl'));
  var systemsUrl = getHrefFromA(document.getElementById('tasklist'));
  var reloadUrl = getHrefFromA(document.getElementById('reloadurl'));
  var noErr = true;
  try{
      Android.appPrepare(title, subTitle, needScanner, backUrl, systemsUrl, reloadUrl);
  }
  catch(e) {
    noErr = false;
  ...
  if (noErr == true){
    if(elActionCaption != null ){
      elActionCaption.remove();
    }
    var elNavigation = document.getElementById('navigation');
    if(elNavigation != null ){
       elNavigation.style.display = 'none';
    }

    needFocusTimeout = needFT;
  }
}
