...
function stepSystems(){
  ...
  $("#step_content").load('systems_systems', function(){
    ...
    .find("tbody > tr").contextMenu({menu:'tblCnMenu'}, function(action,el,pos) {
      ...
      if(action=='download_ref') DownloadReference(el);
    })
    ;
    //scroll
...
function DownloadReference($tr) {
    var $dlg = $("div#dlg-reference-download");
    if ($dlg.length == 0){
        $dlg = $('<div/>').attr('id','dlg-reference-download').addClass('flora')
          .html('<form id="frm_reference_download" action="upload" method="post" enctype="multipart/form-data">'+
            '<input type="hidden" name="id_system" />'+
            '<input type="file" name="fileupload" required/>'+
            '<button id="upload_btn" type="submit"><img src="'+eng_img+'/actions/arrow_up.png"/>&nbsp;Залить</button>'+
            '<hr><div style="font-size: 75%">'+
            'Кодировка файлов в архиве: '+
            '<select name="encoding" style="font-size: 100%;">' +
            '<option value="cp1251">Русская (Windows)</option>' +
            '<option value="utf-8">Линукс</option>' +
            '</select></div>'+
            '</form>');
        $dlg.dialog({
          resizable: false, draggable:false,
          modal: true, overlay:{backgroundColor:'#000',opacity: 0.5}, autoOpen: false,
          height: 150,
          width: $(document.body).width()*0.5
        });

        $dlg.find('input[name=fileupload]').unbind('change').change(function () {
             var re = new RegExp("\.Zip$", "i");
             if (!re.test($(this).val())){
               $(this).val('');
               alert("Разрешены только Zip-файлы!");
               return;
             }
             $("#frm_reference_download").removeAttr("disabled");
        });

        $("form#frm_reference_download")
             .ajaxForm({
                 url: 'qDownloadReference',
                 type: 'POST',
                 dataType: 'json',
                 data: {},
                 success: function (json) {
                     if (json.data.DATA){
                        ...
                        window.open(json.data.DATA);
                        $dlg.dialog("close");
                     }

                     else if (json.data.ERRMES){
                         alert(json.data.ERRMES);
                     }
                 }
             })
             .end();
    }
    $('#frm_reference_download').find('input[name=id_system]').val($tr.attr('id'));
    $dlg.dialog("open");
    $dlg.dialog('option', 'title', 'Загрузка справки: ' + $tr.find('td.show_name').text());
}
...
function editOpt(elem, is_edit){
  var id_user, type_id, obj_id, opt_id;
  if (elem){
