$(document).ready(function () {
	...
	$('body').append('<style>#dvSiteProgress td span {padding-right:3px;}</style>')
    ...
    $("#dvTabs").tabs({
        onShow: function (a) {
            ...
            else if (activeTab == 4) { // Не проведенные документы
                notApprDocs();
            }
            else if (activeTab == 5) { // Корзина
                trash();
            }
        },
        initial: 0,
    ...
    $("#dvWares,#dvPallet,#dvProgress,#dvTest,#dvNotInventPallet,#dvNotApprDocs,#dvTrash")
      .css({"height": height + 'px'}).attr("height", height);
...
function tblTaskWares(json){
    var taskstat = $('#spanTaskId').attr('status');
    function trHTML(r, tstat) {
        return  '<td class="chk"><input type="checkbox"></td>'+
                ...
                '<td class="number">'+kNumber(tstat == '2' ? r.QUANTITY : r.REST)+'</td>'+
        ...
        if (!$('#tblWares').length){
          var html = '<table id="tblWares"><thead>\
            ...
            for(var i=0;i<json.data.length;++i) {
                ...
                html+='<tr taskwaresid="'+r.TASKWARESID+'" waresid="'+r.WARESID+'" data-scanquant="'+kNumber(r.SCANQUANT)+'" data-rest="'+kNumber(r.REST)+'" ' +
                  ...
                  'data-q="' + kFloat(taskstat == '2' ? r.QUANTITY : r.REST) + '" ' +
                  'data-datetime="' + kDateTime(r.LASTDATE) + '">'
                  + trHTML(r, taskstat)+'</tr>';
            }
            var tmalgorithm = $('#spanTaskId').attr('tmalgorithm');
          ...
          $dvTaskWares.html(html)
            ...
            for(var i=0;i<json.data.length;i++){
                ...
                if ($tr.length) {
                    $tr.html(trHTML(r, taskstat));
                    $tr.kScrollToTr().rowFocus();
                }
                else {
                    $tr = $('<tr/>').attr('taskwaresid',r.TASKWARESID).attr('waresid',r.WARESID).html(trHTML(r, taskstat));
...
function trash() {
  if (!$('#spanTaskId').attr('taskid')){
    showMes('Внимание', 'Выберите инвентаризацию!');
    return;
  }
    ...
    $.request({
      ...
      success: function(json) {
        ...
        for (var i=0; i<json.data.length; i++){
          ...
          tr['VQUANTITY'] = viewQuantity(tr.AMOUNT, tr.VUFACTOR, tr.VUCODE, tr.MUFACTOR, tr.MUCODE);
        ...
        success: function(json) {
            ...
            for (var i=0; i<json.data.length; i++) {
                html += '<tr>\
                            ...
                            <td class="text">{WNAME}</td>\
                            <td class="text">{DOCS}</td>\
                         </tr>'.format(json.data[i]);
            }
            html += '</tbody><tfoot><tr><th colspan="3">&nbsp;</th></tr></tfoot></table>';

             $("#dvNotApprDocs").css("text-align","center")
                .html(html)
                .find('table')
                    .kTblScroll()
                    .kTblSorter()
...
function tabNotInventPallet() {
    ...
    $.request({
        ...
        success: function(json) {
            var html = '<table><thead><tr>\
                    <th ksort="text">МП</th>\
                    <th ksort="text">Паллеты</th>\
                <tr></thead><tbody>';
            for (var i=0; i < json.data.length; i++) {
                var r = json.data[i];
                html += '<tr>\
                            <td class="">{SNAME}</td>\
                            <td class="text">{PNUMBER}</td>\
                         </tr>'.format(r);
            }
        ...
        success: function(json) {
            ...
            $('#dvProgress').html(html)
                .find('table')
                    .kTblScroll()
                    .kTblSorter()
					.find('tbody tr').dblclick(siteProgress).end()
...
function siteProgress() {
    ...
    $.request({
        url: 'getSiteProgress',
        data: {siteid: siteid, taskid: taskid},
        success: function(json) {
          var html = '<table><thead><tr>\
                            <th ksort="text">Ряд/МП</th>\
                            <th ksort="">Паллет</th>\
            ...
            html += '</tbody></table>';
            $('#dvSiteProgress').dialog('destroy').remove();
            $('<div id="dvSiteProgress" class="flora"></div>').html(html)
              .dialog({height:500,width:500,modal:true,resizable:false,draggable:true,title:'Прогресс',overlay:{backgroundColor:'#000',opacity: 0.5}})
                .find('table')
                    .kTblScroll()
                    .kTblSorter()
                .find('tbody tr').end()
