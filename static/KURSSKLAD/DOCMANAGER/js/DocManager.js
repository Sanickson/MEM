...
$(document).ready(function(){   
  function scan(param) {
    $.getJSON('scan', param, function (json) {
      if (!showErr(json)) {
        if ($("#cmbDocType").val() == json.data[0].DOCTID) {
          $.tblDocs(json);
        }
        else {
          if ($('#tblDocs').length == 0 ||
            confirm('Переключиться на тип документа:\n' + json.data[0].DOCTNAME +
              '?\n--------------------------------------------------------------------\n' +
              'Текущая таблица документов очистится!!!')) {
            $("#cmbDocType").val(json.data[0].DOCTID);
            docTypeChg.call($("#cmbDocType"), {
              afterEnd: function () {
                $.tblDocs(json);
              }
            });
  ...
  function docTypeChg(O) {
    ...
    $.getJSON('DocTypeStatuses', {doctid: $(this).val()}, function (JSON) {
        if (!showErr(JSON)) {
            ...
            Block('Получение подтиповов документа!');
            $.getJSON('docTypeSubTypes', {doctid: JSON.ext_data.doctid}, function (json) {
                if (!showErr(json)) {
                    if (json.data.length == 0) {
                        $('#cmbDocSubType').parent().parent().hide();
                        $('#cmbDocSubType').parent().parent().prev().hide();
                    }
                    else {
                        $('#cmbDocSubType').parent().parent().show();
                        $('#cmbDocSubType').parent().parent().prev().show();
                        var html = '<option value="null" style="padding-left:20px;">Без фильтра</option>';
                        for (var i = 0; i < json.data.length; i++)
                            html += '<option value="' + json.data[i].CODE + '" style="padding-left:20px;">' + json.data[i].CODE + ': ' + json.data[i].NAME + '</option>';
                        $("#cmbDocSubType").html(html);
                    }
                }
                UnBlock();
            })
            ...
            $.getJSON('docTypeOptions', {doctid: JSON.ext_data.doctid, viewdays: 1, chkamountsign: 1}, function (JSON) {
                if (!showErr(JSON)) {
                    ...
                    for (var i = 0; i < JSON.data.length; i++) {
                        var $obj = false;
                        if (JSON.data[i].NUMOBJ == '0') {
                            $obj = $('#fromobj');
                        }
                        if (JSON.data[i].NUMOBJ == '1') {
                            $obj = $('#throughobj');
                        }
                        if (JSON.data[i].NUMOBJ == '2') {
                            $obj = $('#toobj');
                        }
                        var id = $obj.attr("id");
                        ...
                        switch (JSON.data[i].ISOBJZONE) {
                            case 's':
                            {
                                ...
                                $.ajax({
                                    url: 'ajaxGetObjects', success: function (JSONOBJ) {
                            ...
                            case 'i':
                            {
                                ...
                                $('#' + id).unbind('click').kObjAutoComplete({
                                    hiddenName: id,
                                    action: 'ajaxGetObjects'
                                });
    ...
    $.tblDocs = function(JSON){
      ...
      function events($el) {
        $el.rowFocus({rfSetDefFocus: false,
          rfFocusCallBack: function () {
            if ($('#divTblDetail').is(':visible')) {
              ...
              var params = {docid: $(this).ID()};
              if ($(this).attr('dbid')){
                params['dbid'] = $(this).attr('dbid');
              }
              if ($('#cmbDocType option:selected').attr('wsetid') != '') {
                $.getJSON('listInitDoc',params, tblDetailInit);
              }
              else {
                $.getJSON('listCargo', params, tblDetail);
              }
            }
            $("#divTblDetail").empty();
          }
        ...
        if ($("#" + mId).length == 0) {
          ...
          $("<ul/>").attr("id", mId).addClass("contextMenu").css("width", "190px")
            .html('<li class="add "><a href="#docAdd">Добавить</a></li>' +
              ...
              '<li class="print"><a href="#docPrintTable">Печать таблицы</a></li>' +
        ...
        if ($('#cmbDocType option:selected').attr('code') == 'SALE')
          ...
          $("#" + mId + ' li.return').hide();
        if ($el.is('table'))
          $el.kTdChk().find(">tbody>tr").contextMenu({menu: mId}, menu).find('td:last').dblclick(printTasks).end();
        else if ($el.is('tr'))
          $el.contextMenu({menu: mId}, menu).kTdChk().find('td:last').dblclick(printTasks).end();
...
//-------------------------------------------------------------------------------------------------------------------------------------
    ...
    function buyReturn() {
      ...
      $docDialog({title: 'Возврат'}, {docid: $(this).ID(), frmSubmit: frmSubmit, btnConfTitle: 'Сохранить',
        ...
        showMes('Ошибка', '<div class="dvMes">Отмеченные документы находятся в архивной базе!</div>');
      }
    };
    ...
    function updatePrintTask(docs, dbid){
        var docs = docs.split(',');
        var dbid =  dbid.split(',');
        if (dbid != '')
        ...
        function frmSubmit(){
            ...
            if( $this.find('>td.chk>input:checkbox').attr('checked') ){
                $this.parents('table:first').kTdChkGet().each(function(){
                  ...
                  docs += parents.ID()+',';
                    dbid += (parents.attr('dbid')==''?' ': parents.attr('dbid'))+',';
                });
            }
            else {
              docs=$this.ID();
            ...
            $.docPrint({docid:docs,callback:updatePrintTask, dbid: dbid});
        };
    ...
    function docPrintTable(){
        const wnd = window.open(sp_reports + '/printAllDocs.html');
        const tbl = $('#divTblMaster').clone();
        tbl.find('#tblDocs td.chk, #tblDocs th.chk').remove();
        const tdST = tbl.find('#tblDocs tbody tr td:nth-child(1)');
        tdST.each(el=> {
            $(tdST[el]).html($(tdST[el]).attr('title'));
        });
        wnd.onload = function () {
            const h2 = wnd.document.createElement('h2');
            h2.innerText = 'Документы';
            h2.style.textAlign = 'center';
            wnd.document.body.innerHTML = tbl.printHTML();
            wnd.document.body.prepend(h2);
            wnd.document.getElementById('tblDocs').style.width = '100%';
            wnd.document.querySelectorAll('#tblDocs th, #tblDocs td').forEach(el=>{
                el.style.cssText = `border: 1px solid #000;
                                text-align: center;`;
            });
        }
    ...
    function docTaskes(){
      ...
      var $taskManagerView = $dv.taskManagerView({
          useviewunit: useviewunit,
          useemployee: true
      });
       $taskManagerView.listTask({'docid':docid})
      /*
                                   toview:false,
    ...
    function docCrossTbl() {
      ...
      $(this).parents('table:first').kTdChkGet().each(function () {
        var parents = $(this).parents('tr:first');
        var db = parents.attr('dbid');
        if (!(db in dbid)) {
          dbid.push(db);
        }
        if (db in d){
          d[db].push(parents.ID());
        }
        else{
          d[db] = [];
          d[db].push(parents.ID());
        }
        docs.push(parents.ID());
      });
      var allJSON = {
        'data': [],
        mes: ''
      ...
      for (var key in d){
        //$.getJSON('docBondCrossTbl', {dbid: key, docs: d[key].join(',')}, function (json) {
        $.ajax({url: 'docBondCrossTbl', data: {dbid: key, docs: d[key].join(',')}, success: function (json){
          if (!showErr(json)) {
            allJSON['data'] = allJSON.data.concat(json.data);
          }
          else {
            allJSON['mes'] = json.mes;
          }
        }, dataType: 'json', async: false})

      }
      docBondCrossTbl(allJSON);
//      crossTbl(docs);
    }
    //функция построения Сводной таблицы по заданному JSON объекту
    //JSON объкт формируется в docCrossTbl()
    function docBondCrossTbl(JSON) {
      if (!showErr(JSON)) {
        var wares = {};
        var docs = {};
        for (var i = 0; i < JSON.data.length; ++i) {
    ...
    function crossTbl(ids){
        $.getJSON('docBondCrossTbl',{docs:ids.join(',')},docBondCrossTbl);
    }

  function reQueue() {
    var $this = $(this);
    var params = {docid: $this.ID()};
    if ($this.attr('dbid') && $this.attr('dbid') != '') {
      params['dbid'] = $this.attr('dbid');
    }
    $.getJSON('docQueueInfo', params, function (JSON) {
      if (!showErr(JSON)) {
        if ($("#dvReQueue").length)
          $("#dvReQueue").dialog("destroy").remove();

        var html = '<table id="tblReQueue"><thead><tr><th>Правило</th><th>Кем выгружен</th><th>Создано</th><th>Выполнено</th>' +
          '<th>Статус</th><th title="Файл обмена">Файл</th><th>&nbsp;</th></tr></thead><tbody>';
        for (var i = 0; i < JSON.data.length; i++) {
          var tr = JSON.data[i];
          html += '<tr><td>' + tr.RULE + '</td>' +
            ...
            '<td>' + tr.ENDTIME + '</td>' +
            '<td>' + tr.STATUSNAME + '</td>' +
            '<td filename="http://'+$(location).attr('host')+ tr.FILENAME +'">' + (tr.FILENAME == '' ? '' : '<a id="openFileEx" href=# title="Открыть файл"><img src="' + eng_img + '/folders/folder.gif" ></a>') + '</td>' +
        ...
        var $dv = $("<div/>").attr("id", "dvReQueue")
          ...
          .html('<div style="width:100%;height:400px;"></div>\
                           <div style="width:100%;height:50px;" class="buttons">\
                            <button type="button" id="dvReQueueApp" title="Перевыгрузить документ" style="' + (($this.attr('dbid') != '') ? 'display: none;' : '') + '">\
                            <img src="' + eng_img + '/actions/accept.png" border="0">Перевыгрузить</button>&nbsp;&nbsp;\
          ...
          .find("table").kTblScroll().tablesorter()
          .find("#openFileEx").click(function () {
            //showMes('Сообщение', $(this).parent().attr('filename'));
            try{
                window.open($(this).parent().attr('filename'));}
            catch(err){
                showMes('Сообщение', 'Файл обмена не найден');
            }
          }).end()
          .find('td[result]').click(function () {
              if ($(this).attr('result'))
...
//-------------------------------------------------------------------------------------------------------------------------------------
    ...
    function waresLot(){
        var $tr = $(this);
        var params = {docid: $tr.attr('id').split('_')[1]};
        if ($tr.attr('dbid') && $tr.attr('dbid') != ''){
          params['dbid'] = $tr.attr('dbid');
        }
        $.getJSON('getWaresLot',params,function(JSON){
            if(!showErr(JSON)){
                ...
                $("#dvTabs").tabs({
                    onShow:function(a){
                        ...
                        if($($(a).attr('href')).children().length == 0) {
                            ...
                            $.getJSON('getWaresLotWares',params,function(JSON){
                                if(!showErr(JSON)){
    ...
    function wlWaresLot(){
        var $tr = $(this);
        var params = {twids: $tr.attr('twids')};
        if ($('#tblDocs').rf$GetFocus().attr('dbid') && $('#tblDocs').rf$GetFocus().attr('dbid') != ''){
          params['dbid'] = $('#tblDocs').rf$GetFocus().attr('dbid');
        }
        $.getJSON('getWaresLotHist',params, function(JSON){
            if(!showErr(JSON)){
