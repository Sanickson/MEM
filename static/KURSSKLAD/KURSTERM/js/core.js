...
function logText(elId,  evt){
    try {
        var dvScanText = document.getElementById(elId);    
        if (dvScanText)
            dvScanText.innerHTML = dvScanText.innerHTML + (evt.ctrlKey ? 'Ctrl' : '') + (evt.shiftKey ? 'Shift' : '') + getKeyCode(evt) + ';';
    } 
    catch (err) {
        alert('Ошибка');
    }
...
function KeyPressAction(evt) {
    //logText('dvScanTextKeyPress', evt);
