$(document).ready(function () {
  ...
  $("#ulWaresGroup").css({"height": h, "width": "15%", "overflow": "auto", "text-align": "left"})
    .treeWaresGroups({
      url: "automaticWaresGroup", click: function () {
        $("#dvWares").empty();
...
//                            <th ksort="false" title="Паллетное пополнение - весь товар с паллета перемещать на МО">ПП</th>\
      ...
      function slotContent() {
        ...
        function frmSubmit() {
          ...
          $("#dvWaresSelOptSlotContent").dialog('close');
          var data = {
            wid: $(this).attr('data-wid'),
            objid: $('#dvWares table:first').attr('data-objid'),
            fld: 'slotcontent',
        ...
        var $dv = $("#dvWaresSelOptSlotContent");
        if ($dv.length) $dv.dialog('destroy').remove();
        $dv = $("<form/>").attr("id", "dvWaresSelOptSlotContent")
          .attr('data-wid', $tr.attr('data-wid'))
          .addClass("flora").addClass("buttons").css("text-align", "center")
          .dialog({
            autoopen: true,
            ...
            resizable: false,
            draggable: false,
            title: 'Вместимость места отборки',
            overlay: {backgroundColor: '#000', opacity: 0.5}
          })
          .html(html)
          .find('select').change(calcVal).end()
      ...
      function trHTML(tr) {
        return '<td class="chk"><input type="checkbox"></td>' +
          ...
          '<td>' + tr.TASKID + '</td>' +
          '<td>' + tr.SLOTNAME + '</td>' +
          '<td data-fld=slotcontent>' + (tr.VUCODE ? viewQuantity(tr.SLOTCONTENT, tr.VUFACTOR, tr.VUCODE, tr.MUFACTOR, tr.MUCODE) : '') + '</td>' +
          '<td data-fld=slotcontent class="number">' + kNumber(tr.SLOTCONTENT) + '</td>' +
...
//                        '<td data-fld=fullpallet><input type=checkbox ' + (tr.FULLPALLET == '1' ? ' checked' : '') + '></td>';
      ...
      function events($trs) {
        ...
        $trs.find('>td')
          ...
          .filter("[data-fld=slotcontent]").dblclick(slotContent).end()
