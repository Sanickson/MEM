...
;(function($){
                ...
                if( !showErr(JSON) ){                 
                    $.getJSON('listObjName',{objid:objid},function(JSONOBJ){
                        if( !showErr(JSONOBJ) ){
                            ...
                            $('#bnChangeSiteStatus').unbind('click').click( function(){
                                var $tr = $('#tblSite').rf$GetFocus();
                                if( $tr.length == 0 ){
                                    showMes('Ошибка','<div class="dvMes">Необходимо выбрать местоположение!</div>');
                                    return;
                                }
                                changeSiteStatus.call($tr);
                            });
                            $('#bnSitePalletVolume').unbind('click').click( function(){
                                var $tr = $('#tblSite').rf$GetFocus();
                                if( $tr.length == 0 ){
                                    showMes('Ошибка','<div class="dvMes">Необходимо выбрать местоположение!</div>');
                                    return;
                                }
                                sitePalletVolume.call($tr);
                            });
                            ...
                            $("<ul/>").attr("id",'dvSiteZoneContextMenu').addClass("contextMenu").css({'width':200})
                                ...
                                '<li class="info"><a href="#siteListStellag">Добавить МО</a></li>')
  ...
  function siteListStellag() {
    var tr = $(this);
    if ($("#dvListStellag").length)
      $("#dvListStellag").remove();
    function dvListStellag() {
      $("#dvListStellag").dialog('option', 'title', '<b>Ряд</b>: ' + tr.find('td >div[name=sitename]').text());
      $("#dvListStellag").dialog("open");
    };
    var html = '<form id="frmListStellag">' +
      '<table style="border:0px;width:100%;">' +
      '<tr>' +
      '<td colspan=1 style="text-align: right;"><b>Количество МО</b></td>' +
      '<td colspan=3 style="text-align: right;">2МО(05,10) <input type="radio" name="MO" value="0"></td>' +
      '<td colspan=3 style="text-align: left;">3МО(04,07,10)<input type="radio" name="MO" value="1"></td>' +
      '</tr>' +
      '<tr>' +
      '<td colspan=3 style="text-align: right;">Стелаж с <input class="number" name="beginnum" style="width: 80px;" ></td>' +
      '<td colspan=4 style="text-align: left;">по <input class="number" name="endnum" style="width: 80px;"></td>' +
      '</tr>' +
      '<tr>' +
      '<td colspan=1 style="text-align: right;"><b>Сторона</b></td>' +
      '<td colspan=2 style="text-align: right;">Четная <input type="radio" name="parity" value="0"></td>' +
      '<td colspan=2>Нечетная <input type="radio" name="parity" value="1"></td>' +
      '<td colspan=2 style="text-align: left;">Все <input type="radio" name="parity" value="2" checked></td>' +
      '</tr>' +
      '</table>' +
      '<hr>' +
      '<div class="buttons" style="width:100%;">' +
      '<button type="button" id="dvListStfOk"><img src="' + eng_img + '/actions/accept.png" border="0">Сохранить</button>&nbsp;&nbsp;&nbsp;' +
      '<button type="button" id="dvListStCanc"><img src="' + eng_img + '/actions/cancel.png" border="0">Отменить</button>' +
      '</div>';
    html += '</form>';
    $("<div/>").attr({"id": "dvListStellag"}).addClass("flora")
      .dialog({autoOpen: false, height: 160, width: 400, modal: true, resizable: false, draggable: false, title: 'Название ряд!!', overlay: {backgroundColor: '#000', opacity: 0.5}})
      .html('<div></div>')
      .find(">div").css({"width": "100%"});
    $('#dvListStellag').find(">div").filter(":first").html(html);
    $('#dvListStCanc').unbind().click(function () {
      $("#dvListStellag").dialog("close");
    });
    $('#frmListStellag').find('input.number').kInputInt();
    $('#dvListStfOk')
      .unbind('click')
      .click(function () {
        var beginnum = $('#frmListStellag').find('input.[name=beginnum]').val();
        var endnum = $('#frmListStellag').find('input.[name=endnum]').val();
        var MO = $('#frmListStellag').find('input[name=MO]:checked').attr('value');
        var rowid = tr.attr('siteid');
        var parity = $('#frmListStellag').find('input[name=parity]:checked').attr('value');
        parity = parity == 2 ? 'all' : parity;
        if ((typeof parity === "undefined") || (typeof MO === "undefined") || (beginnum == ''))
          alert('Заполните все значения');
        else {
          ProcName = MO == 1 ? 'setSiteMO3' : 'setSiteMO2';
          $.getJSON(ProcName, { rowid: rowid, parity: parity, beginnum: beginnum, endnum: endnum }, function (JSON) {
            if (!showErr(JSON)) {
              $("#dvListStellag").dialog("close");
            }
          });
    ...
    if (tr.find('td[name=sitetype]').text() != 'Ряд') {
      alert('Данная функция доступна только для ряда');
    }
    else {
      dvListStellag();
    }
  ...
  function siteZone(){
        ...
        var html = '<table id="tblSite" style="width:100%;"><thead><tr>'+
                   ...
                   '</tbody><tfoot><tr><th colspan="4" style="text-align:right;">'+
                        ((rightUser.rightEditSite)?'<img id="bnSiteCreate" title="Добавить верхний уровень" class="imgButton" src="'+eng_img+'/actions/add.png">'+
							...
							'<img id="bnChangeSiteStatus" title="Изменить стасус местоположения" class="imgButton" src="' + sps_img.KURSSKLAD + '/up_down.png">'+
							'<img id="bnSitePalletVolume" title="Максимальное кол-во паллет для МП" class="imgButton" src="'+eng_img+'/actions/pallet.png">'+
                   '</th></tr></tfoot></table>';
        return html;
    }
    ...
    function tblSiteTrS(JSON,level){
        function tblSiteTr(data,level,end){
            ...
            html += '</div><div class="dvLeft" name="sitename">'+data.NAME+'</div></td>'+
                '<td class="text paddingLeft" name="sitetype">'+data.SITETNAME+'</td>'+
  ...
  function changeSiteStatus() {
    var tr = $(this);
    if (confirm('Вы действительно хотите изменить статус местоположения?')) {
      $.blockUI({message: '<h2>..Выполнение..</h2>'});
      $.getJSON('changeSiteStatus', {siteid: tr.attr('siteid')}, function (resp) {
        if (!showErr(resp)) {
          tr.find('td.tdStatus1,td.tdStatus0').removeClass().addClass('tdStatus'+resp.data.STATUS);
          tr.find('img.imgClickClose').click();
        }
        $.unblockUI();
      });
    }
  ...
  function sitePalletVolume(){
    if ($('#frmSitePalletVolume').length) $('#frmSitePalletVolume').dialog('destroy').remove();
    var tr = $(this);
    $.blockUI({message: '<h2>..загрузка..</h2>'});
    $.getJSON('getSitePalletVolume', {siteid: tr.attr('siteid')}, function(resp){
      if (!showErr(resp)){
        var html = '<table id="tbsSitePalletVolume"><tbody>';
        html += '<tr><td>Кол-во паллет</td><td><input type="text" name="palletvolume">' +
          '<input type="hidden" name="siteid" value="' + resp.data.SITEID + '">' +
          '</td></tr>'
        html += '</tbody></table>';
        html = '<label>Кол-во паллет:</label><input type="text" name="palletvolume" size="12" maxlength="3" value="' + resp.data.PALLETVOLUME + '">' +
          '<input type="hidden" name="siteid" value="' + resp.data.SITEID + '">' +
          '<hr><br>'+
          '<div class="buttons">'+
                       ...
                       '</div>'
        $('<form/>')
          .attr('id', 'frmSitePalletVolume')
          .addClass('flora')
          .html(html)
          .dialog({height: 130, width: 230, modal: true, resizable: false, draggable: true, title: resp.data.NAME,
            overlay: {backgroundColor: '#000', opacity: 0.5}});
        $('#frmSitePalletVolume').unbind('submit').submit(function(){
          var params = {};
          var $frm = $(this);
          params.palletvolume = $frm.find('>input[name=palletvolume]').val();
          params.siteid = $frm.find('>input[name=siteid]').val();
          if (params.palletvolume) {
            $.getJSON('setSitePalletVolume', params, function (json) {
              if (!showErr(json)) {
                $('#frmSitePalletVolume').dialog('close');
                showMes('Сообщение', '<div class="dvMes">Сохранено</div>');
              }
            });
          }
          else{
            showMes('Сообщение', '<div class="dvMes">Введите данные</div>');
          }
          return false;
        }).find('div.buttons').find('button:last').click(function(){
            $('#frmSitePalletVolume').dialog('close');
          }).end().end().find('input[name=palletvolume]').kInputFloat();
      }
      $.unblockUI();
    });
  }
