...
;(function($) {
    ...
    function selgroup(){
        function setPosibleSelgroup(){
            var $tr = $(this).closest('tr');
            if ($tr.length){
                $.getJSON('setSelgroup',{mid:mid,sgid:$tr.attr('sgid'), priority: $tr.find('td.prior input').val()},function(JSON){
                    if (showErr(JSON)){
                        $tr.find('td.prior input').val('');
                        return false;
                    }
                    else{
                        var minval=1000000;
                        var mincode='';
                        var minname='';
                        $('#tblSelgroup').find('input[value]').each(function(){
                           if (kInt($(this).val()) < minval){
                               minval = kInt($(this).val());
                               mincode = $(this).closest('tr').find('td.sgcode').text();
                               minname = $(this).closest('tr').find('td.sgname').text();
                           }
                        });
                        $("#"+$.kID(trM,mid)+">td.selgroup").html(mincode).attr('title',minname)

                    }
                });
            }
            else
                alert('Выберите ГО из списка в таблице!');
        };

        function setSelgroup(){
            $('#tblSelgroup').find('input');
            Math.min.apply( null, $('#tblSelgroup').find('input'));
                    $("#"+$.kID(trM,JSON.ext_data.MID)+">td.Selgroup").html(JSON.data.CURZONENAME);
        }
        ...
        var $this = $('#tblMan').rf$GetFocus();
        var mid = $this.kID();
        $.getJSON('listSelgroup',{mid:mid},function(JSON){
            if (!showErr(JSON)){
                var html = '<table id="tblSelgroup"><thead><tr><th>Приоритет</th><th>Код ГО</th><th>ГО</th><th>Склад</th></tr></thead><tbody>';
                for (var i =0; i<JSON.data.length; i++){
                    var tr = JSON.data[i];
                    html += '<tr sgid="'+tr.ID+'">'+
                            '<td class="prior"><input type="number" min="0" value="'+tr.PRIORITY+'" style="width: 60px"></td>'+
                            '<td class="text sgcode">'+tr.CODE+'</td>'+
                            '<td class="text sgname">'+tr.NAME+'</td>'+
                            '<td class="text">'+tr.FULLNAME+'</td>'+
                            '</tr>';
                }
                html += '</tbody></table>';
                $("<div/>").attr({"id":"dvSelgroup","mid":mid}).addClass("flora")
                    .dialog({autoopen:false,height:$(document.body).height()*0.95,width:$(document.body).width()*0.75,modal:true,resizable:false,draggable:false,
                        title:$this.find('td.lname').text()+' '+$this.find('td.name').text()+' '+$this.find('td.fname').text(),
                        overlay:{backgroundColor:'#000',opacity: 0.5}})
                        .html('<div></div>')
                        .find(">div").css({"width":"100%","height":"95%"}).html(html)
                                .find(">table").kTblScroll().tablesorter({headers:{ 0:{sorter:"digit"},
                                                                    1:{sorter:"text"},
                                                                    2:{sorter:"text"},
                                                                    3:{sorter:"text"}}
                                                         }).rowFocus({rfSetDefFocus:false}).end().end();
                $('#tblSelgroup').find("input").kInputInt().change(setPosibleSelgroup)
            }
        });
    };
    ...
    $.tblMan = function(JSON){    
            ...
            if ($("#"+mId).length==0){
                $("<ul/>").attr("id",mId).addClass("contextMenu").css("width","190px")
                    .html('<li class="add "><a href="#addman">Добавить</a></li>'+
                          ...
                          '<li class="information separator"><a href="#zone">Зона</a></li>'+
                          '<li class="information separator"><a href="#selgroup">Группа отборки</a></li>'+
                          '<li class="information separator"><a href="#gang">Бригада</a></li>'+
                          '<li class="information separator"><a href="#session">Сессия</a></li>'+
                          '<li class="information separator"><a href="#tasks">Задания</a></li>'+
                          '<li class="information separator"><a href="#warningtasks">Открытые задания</a></li>'+
                          '<li class="information separator"><a href="#ps">Пользователь системы</a></li>')
        ...
        function tblManTrBody(data){
            ...
            var html = '<td class="chk"><input type="checkbox"></td>'+
                       ...
                       '<td class="text selgroup" title="'+data.MINSELGROUP+'">'+data.MINSGCODE+'</td>'+
                       td_gang+
        ...
        function tblManMaster(JSON){
            var html = '<table id="tblMan"><thead><tr>'+
                       ...
                       '<th>Отчество</th><th>Должность</th><th>Зона</th><th>ГО</th><th>Бригада</th>'+
			...
			for (var i =0; i<JSON.data.length; i++){
				...
				html += '<tr id="'+$.kID(trM,tr.MANID)+'">'+
                        ...
                        '<td class="text selgroup" title="'+tr.MINSELGROUP+'">'+tr.MINSGCODE+'</td>'+
                        td_gang+
            ...
            html += '</tbody>'+
                    '<tfoot><tr><th colspan="5">Всего:'+JSON.data.length+'</th><th colspan="7" class="buttons">'+
