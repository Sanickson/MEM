    ...
    $(function(){
        ...
        function CMContractors(id){
            $("#"+id).contextMenu({menu: 'cm-contractors'},
                function(action, el, pos){
                    ...
                    if (action=='delete'){ DeleteContractor(el);}
        ...
        function DeleteContractor(el){
            const compid = el.attr('id').split('-')[1];
            const compName = el.find('td.Name').text();
            if (confirm(`Вы действительно хотите удалить контрагента ${compName}?`)) {
                $.getJSON('delContractor',{compid:compid},function(json){
                    if (!showErr(json)) el.remove();
                });
            }
        }
