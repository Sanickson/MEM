          ...
          $.request({
            ...
            success: function(json) {
                ...
                for (var i=0; i<json.data.length; i++)
                    list_zone += '<option value={OBJID}>{OBJNAME}</option>'.format(json.data[i]);
                /*$("#objid").html(html)
                    .change(function() {
                        $('#dvWares').empty();
                    });*/
            ...
            $dialog.html('<div class="tree" style="float:left;position:relative;height:100%;width:25%;background: #ffffff;">'+
                            '<ul class="ulWaresGroup treeview" style="height:90%;width:100%;overflow:auto;text-align:left;"></ul>'+
                            '<table style="width:100%;" id="tblObj"><tbody><tr><th>Объект</th></tr><tr><td><select id="objid" style="width:100%;">'+list_zone+'</select></td></tr></tbody></table>' +
                        '</div>'+
                        '<div style="height:100%;width:75%;float:left;position:relative;">' +
                          '<div class="SelGroup" style="width:100%;height:35%;float:left;position:relative;"></div>'+
                          '<div class="wares" style="width:100%;height:65%;float:left;position:relative;"></div>' +
                        '</div>')
                .find('ul.ulWaresGroup').treeWaresGroups({ url: "waresGroup", click: function() {
                    ...
                    $.getJSON('waresByGroupLocateSG',{wgid:$(this).parents("li").kID(), objid: $('#objid').val()}, function(JSON){
                        ...
                        function clickTr(){                    
                            ...
                            $dialog.find('div.wares').empty();
                            $.getJSON('waresByGroupLocate',{wgid: wgid, wsgid: wsgid, objid: $('#objid').val()}, function(JSON){
                        ...
                        $dialog.find('div.SelGroup').html(html)//.css({'float':'left','width':'700px','height':'35%','padding-left':10})
                            .find('table').kTblScroll().kTblSorter().rowFocus({rfSetDefFocus:false, rfFocusCallBack: clickTr})
                                .find('button')
                    ...
                    function setSelGroup(wgid, setWsgId ){
                        ...
                        if ( inpCh.length ) {
                            ...
                            $.getJSON('waresByGroupSet',{wgid: wgid, wsgids: oldWSGs.join(','), setwsgid: newWSGID, objid: $('#objid').val()},function(json){
                                if (showErr(json)){
                                    return false;
                                }
                                clickthis.click();
            ...
            success: function(json){
                ...
                for (var i=0; i<json.data.length; i++) {
                    html += '<tr data-code="{CODE}" style="height:50px;">\
                                <td class="text" style="font-size:25px;" data-text="{NAME}">{NAME}</td>\
                             </tr>'.format(json.data[i]);
                }
                html += '<tr data-code="" style="height:50px;">\
                                <td class="text" style="font-size:25px;" data-text="">Отвязать</td>\
                             </tr>';
