  ...
  function ubd(e) {
      ...
      $.getJSON('waresGetUBD', {wid: waresID}, function (JSON) {
        if (!showErr(JSON)) {
          $("#dvUBD").attr('wid', JSON.ext_data.WID)
            ...
            .find('>input[name = itValue]').val(JSON.data.WITVAL).end()
            .find('>select[name = itType]').val(JSON.data.WITCODE).end()
            .find('>input[name = itCoeff]').val(JSON.data.WITCOEFF).end()
            .find('>input[name = salValue]').val(JSON.data.WSALVAL).end()
            .find('>select[name = salType]').val(JSON.data.WSALCODE).end()
            .find('>input[name = salCoeff]').val(JSON.data.WSALCOEFF).end()
            .end()
            .dialog("open").find('>form>div>input:first').focus().end();
        }
      });
    }
    ...
    if ($("#dvUBD").length) dv();
      ...
      $.getJSON('rChoice', {chtype: 'P'}, function (JSON) {
        if (!showErr(JSON)) {
          var html = '<form style="text-align:center;"><div><br>Годности: <input type="text"  size="4" name="ubdValue"  value="0"> <select name="ubdType">' +
            '<option value="">Не выбрано</option>';
          for (var i = 0; i < JSON.data.length; i++)
            html += '<option value="' + JSON.data[i].CODE + '">' + JSON.data[i].NAME + '</option>';
          html += '</select></div><br>';
          html += '<div style="width: 48%; display: inline-block; text-align: left"> <input type="radio"  class="intermandC" name="ineterm" id="interm1"/>' +
            'Приемки: <input type="text" disabled size="2" name="itValue"  value="0"> <select disabled name="itType">' +
            '<option value="">Не выбрано</option>';
          for (var i = 0; i < JSON.data.length; i++)
            html += '<option value="' + JSON.data[i].CODE + '">' + JSON.data[i].NAME + '</option>';
          html += '</select>';
          html += '<br><br><input type="radio" checked class="intermandC" name="ineterm" id="interm2"/>' +
            ' КСПриемки: <input type="text" size="4" name="itCoeff"  value="0"></div>';
          html += '<div style="width: 4%; display: inline-block"></div>'
          html += '<div style="width: 48%; display: inline-block; text-align: left"><input type="radio"  class="saltermandC" name="saleterm" id="salterm1"/>' +
            'Продажи: <input type="text" disabled size="2" name="salValue"  value="0"> <select disabled name="salType">' +
            '<option value="">Не выбрано</option>';
          for (var i = 0; i < JSON.data.length; i++)
            html += '<option value="' + JSON.data[i].CODE + '">' + JSON.data[i].NAME + '</option>';
          html += '</select>';
          html += '<br><br><input type="radio" checked class="saltermandC" name="saleterm" id="salterm2"/>' +
            ' КСПродажи: <input type="text" size="4" name="salCoeff"  value="0"></div><br><br><hr><div class="buttons">' +
          ...
          $("<div/>").attr("id", "dvUBD").addClass("flora")
            ...
            param.ubdVal = $(this).find('>div>input:first').val();
            param.ubdType = $(this).find('>div>select:first').val();
            param.itVal = $(this).find('>div>input[name = itValue]').val();
            param.itType = $(this).find('>div>select[name = itType]').val();
            param.itCoeff = $(this).find('>div>input[name = itCoeff]').val();
            param.salVal = $(this).find('>div>input[name = salValue]').val();
            param.salType = $(this).find('>div>select[name = salType]').val();
            param.salCoeff = $(this).find('>div>input[name = salCoeff]').val();
            $.getJSON('waresSetUBD', param, function (JSON) {
              ...
              if (!showErr(JSON)) {
                $("#" + $.kID(trW, JSON.ext_data.WID))
                  ...
                  .filter(":eq(1)").text(kNumber(JSON.data.WITDAYS)).end()
                  .filter(":last").text(kNumber(JSON.data.WITCOEFF)).end()
                  .end()
                  .find('>td.saleterm')
                  .filter(":first").text(JSON.data.WSALVAL + ' ' + JSON.data.WSALNAME).end()
                  .filter(":eq(1)").text(kNumber(JSON.data.WSALDAYS)).end()
                  .filter(":last").text(kNumber(JSON.data.WSALCOEFF)).end()
                  .end()
            ...
            .find('>div>input').kInputInt().focus(function () {
            $(this).select();
          }).end()
            .find('>div>input[name = itCoeff]').kInputFloat().end()
            .find('>div>input[name = salCoeff]').kInputFloat().end()
            .find('.intermandC').click(function () {
            if ($(this).attr('id') == 'interm1') {
              $("#dvUBD").find('>form>div>input[name = itValue]').attr('disabled', false).end();
              $("#dvUBD").find('>form>div>select[name = itType]').attr('disabled', false).end();
              $("#dvUBD").find('>form>div>input[name = itCoeff]').val("");
              $("#dvUBD").find('>form>div>input[name = itCoeff]').attr('disabled', true).end();
            }
            if ($(this).attr('id') == 'interm2') {
              $("#dvUBD").find('>form>div>input[name = itCoeff]').attr('disabled', false).end();
              $("#dvUBD").find('>form>div>input[name = itValue]').val("");
              $("#dvUBD").find('>form>div>select[name = itType]').val("");
              $("#dvUBD").find('>form>div>select[name = itType]').attr('disabled', true).end();
              $("#dvUBD").find('>form>div>input[name = itValue]').attr('disabled', true).end();

            }
          }).end()
            .find('.saltermandC').click(function () {
            if ($(this).attr('id') == 'salterm1') {
              $("#dvUBD").find('>form>div>input[name = salValue]').attr('disabled', false).end();
              $("#dvUBD").find('>form>div>select[name = salType]').attr('disabled', false).end();
              $("#dvUBD").find('>form>div>input[name = salCoeff]').val("");
              $("#dvUBD").find('>form>div>input[name = salCoeff]').attr('disabled', true).end();
            }
            if ($(this).attr('id') == 'salterm2') {
              $("#dvUBD").find('>form>div>input[name = salCoeff]').attr('disabled', false).end();
              $("#dvUBD").find('>form>div>input[name = salValue]').val("");
              $("#dvUBD").find('>form>div>select[name = salType]').val("");
              $("#dvUBD").find('>form>div>select[name = salType]').attr('disabled', true).end();
              $("#dvUBD").find('>form>div>input[name = salValue]').attr('disabled', true).end();
	...
	function cngMainUnitCode() {
		...
		$.getJSON('getMWUCode',function(JSON) {
			if (!showErr(JSON)) {
                ...
                for (var i=0; i<JSON.data.length; i++) {
                    var tr = JSON.data[i];
                    html += '<tr uid="'+tr.UID+'" wid="'+ params.wid+'">'+
                            '<td class="text setDU">'+tr.SHORTNAME+'</td>'+
                            '<td class="text">'+tr.FULLNAME+'</td></tr>';
                }
				html += '</tbody></table>';
    ...
    $.tblWares = function(JSON){
      function events($el) {
        ...
        var mId = 'menuTblWares';
        if ($("#" + mId).length == 0) {
          $("<ul/>").attr("id", mId).addClass("contextMenu").css("width", "150px")
            .html('<li class="information"><a href="#waresRestOnDate">Остаток на дату</a></li>' +
                  '<li class="refresh"><a href="#waresUpload">Выгрузить</a></li>')
            .appendTo($(document.body));
        }
        if ($el.is('table')) {
          events_td($el.find('>tfoot>tr>th>button')
            .filter(':first').click(printSpWares).end()
            .filter(':last').click(printLocBarcode).end()
            .end().find('>tbody>tr').contextMenu({menu: mId}, menu)
            .find(">td"));
        }

        if ($el.is('tr')) {
          events_td($el.kScrollToTr().rfSetFocus().contextMenu({menu: mId}, menu).find('>td'));
        }
      ...
      function waresUpload(){
          $.ajax({url:'waresUpdateLastDate', data:{"wid":$wares().kID()}, success:function(JSON){
              if(!showErr(JSON)){
                showMes("Внимание", "Товар будет выгружен");
              }} ,dataType:'json', async: false});
      }

      function waresRestOnDate(){
        if ($("#dbWaresRestOnDate").length) $("#dbWaresRestOnDate").dialog("destroy").remove();
        var html = '<form style="width: 100%;"><table style="width: 100%;">' +
          '<tr><td><input type="text" id="dbeg" name="dbeg" size="8" value="">&nbsp;<input type="text" id="tbeg" name="tbeg" size="4" value="">&nbsp;&nbsp;' +
          '<button type="submit" id="submit"><img src="' + eng_img + '/actions/magnifier.png"></button></td>' +
          '</tr></table></form><table id="tblWaresRestOnDate">' +
          '<thead><tr><th>Номер паллета</th><th title="Текущее местоположение">МП</th>' +
          '<th title="Дата производства">Дата</th><th title="Количество">Кол-во</th><th>Итого</th></tr></thead><tbody></tbody>' +
          '</table>';
        var $div =  $('<div/>').attr('id', 'dbWaresRestOnDate').addClass("flora").css("text-align", "center")
          .dialog({autoOpen: true, height: 500, width: 450, modal: true, resizable: false, draggable: false, title: 'Остаток на дату',
            overlay: {backgroundColor: '#000', opacity: 0.5}})
          .html(html).find('form').submit(function(){
            var params = $(this).kFormSubmitParam();
            params.waresid = $wares().kID();
            if (params.waresid) {
              Block('...Получение остатков...');
              $.getJSON('waresRestOnDate', params, function (JSON) {
                if (!showErr(JSON)) {
                  ...
                  for (i = 0; i < JSON.data.length; ++i) {
                    ...
                    html += '<tr>' +
                      ...
                      '<td>' + viewQuantity(tr.QUANTITY, ed.VIEWUFACTOR, ed.VIEWUCODE, ed.MAINUFACTOR, ed.MAINUCODE) + '</td>' +
                      '<td>' + tr.QUANTITY + '</td>' +
                      '</tr>';
                  }
                  $('#tblWaresRestOnDate').find('tbody').html(html).end().kTblScroll().kTblSorter();
                }
                UnBlock();
              });
            }
          ...
          .find('#dbeg').mask('99.99.9999').datepicker().val(kToday()).end()
          .find('#tbeg').mask('99:99:99').val('00:00:00').end()
          .end();
//        $('#dbeg').mask('99.99.9999').datepicker().val(kToday());
//        $('#tbeg').mask('99:99:99').val('00:00:00').end();
      }
        function td(tr) {
            return  '<td class="text">'+tr.WGNAME+'</td>'+
              ...
              // '<td class="date">'+kDate(tr.ADDCTRLDATE)+'</td>'+
              // '<td class="text"  title="'+tr.ADDCTRLNAME+'">'+tr.ADDCTRLCODE+'</td>'+
              ...
              '<td class="number incometerm">' + kNumber(tr.WITCOEFF) + '</td>' +
              '<td class="text saleterm">' + (tr.WSALVAL ? (tr.WSALVAL + ', ' + tr.WSALNAME) : '' ) + '</td>' +
              '<td class="number saleterm">' + kNumber(tr.WSALDAYS) + '</td>' +
              '<td class="number saleterm">' + kNumber(tr.WSALCOEFF) + '</td>' +
        ...
        if (!showErr(JSON)) {
            if (!$('#tblWares').length) {
                ...
                var colSpan = 20; // Без ЧЗ, с ним еще +2
                ...
                var html='<table id="tblWares" wgid="'+JSON.ext_data.wgid+'"><thead>'+
                  '<tr><th colspan="2">&nbsp;</th>'+
                  '<th colspan="'+(exists('wselgrp','view')?5:4)+'">Товар</th>'+
                  // '<th colspan="2" title="Честный знак">ЧЗ</th>'+
                  '<th colspan="2">Ед. изм.</th>'+
                  '<th colspan="2">Ед. от.</th>'+
                  '<th colspan="2">Хранение</th>' +
                  ...
                  '<tr><th ksort="digit">№</th>'+
                    ...
                    // '<th ksort="date" title="Дата начала отборки по честному знаку">Дата</th>'+
                    // '<th ksort="text" title="Код отборки по честному знаку">Код</th>'+
                    '<th ksort="digit" title="Кратность">Кр</th>'+
                    '<th ksort="text">Код</th>'+
                    '<th ksort="digit" title="Кратность">Кр</th>'+
                    '<th ksort="text">Код</th>' +
                    '<th ksort="text" title="Срок хранения">Срок</th>' +
                    '<th ksort="digit" title="Срок хранения в днях">Дни</th>' +
                    '<th ksort="text" title="Срок приемки">Срок</th>' +
                    '<th ksort="digit" title="Срок приемки в днях">Дни</th>' +
                    '<th ksort="digit" title="Коэффициент срока приемки">КСП</th>' +
                    '<th ksort="text" title="Срок продажи">Срок</th>' +
                    '<th ksort="digit" title="Срок продажи в днях">Дни</th>' +
                    '<th ksort="digit" title="Коэффициент срока продажи">КСП</th>' +
    ...
    function waresFocus(){
        function listWaresUnits(JSON){
            const wcode = $wares().find('.wcode').text();
            const wname = $wares().find('.wname').text();
                            ...
                            .find('>table').kTblScroll().find('>tbody>tr').dblclick(function(){
                                ...
                                $("<form/>").attr("id","frmAddWU").attr('uid',$(this).attr('uid')).addClass("flora").css("text-align","center")
                                    ...
                                    .submit(function(){
                                        ...
                                        param.mes = `добавил единицу измерения '${$('tr[uid="'+param.uid+'"] > td:nth-child(1)').text()}' для товара ${wname} / Код: ${wcode}`;
                                        param.wid = waresID();
            ...
            function waresUnitDel(){
                if (confirm('Вы уверены, что хотите удалить единицу измерения?')) {
                    const wucode = $(this).find('td.wucode').text();
                    $.getJSON('waresDelUnit',
                            {wid: waresID(),
                            uid: $(this).attr('uid'),
                            mes: `удалил единицу измерения '${wucode}' у товара ${wname} / Код: ${wcode}`},
                        function (JSON) {
                            if (!showErr(JSON)) {
                                $wares().rfSetFocus(false, true);
                            }
    ...
    //UR                      '<b>'+$wu.find("td.wufactor").text()+' '+$wu.find("td.wucode").text()+'</b><br><hr>'+
                    ...
                    .submit(function(){
                        ...
                        param.uid = $(this).attr('uid');
                        param.wid = waresID();
                        var $wu = $('#tblWU>tbody>tr[uid="'+param.uid+'"]');
                        param.mes = `изменил единицу измерения '${$wu.find('.wucode').text()}' для товара ${wname} / Код: ${wcode}`
                        $.getJSON('setWU',param,function(JSON){
                            if (!showErr(JSON)){
                ...
                $.getJSON('listWaresUnitBarcode',{wuid:wuid},function(JSON){
                    ...
                    $('#delBar').unbind('click').click(function(){
                        ...
                        if ($tr.length > 0){
                            if (confirm('Вы действительно желаете удалить выбранный ШК?')){
                                const mes = `удалил ШК: ${$tr.find('td').text()} - для товара ${wname} / Код: ${wcode}`
                                $.getJSON('delWUBarcode',{barcid:$tr.attr('barcid'), mes: mes},function(JSON){
                                    if (!showErr(JSON)){
                                        $tr.remove();
                    ...
                    $('#apply').unbind('click').click(function(){
                        if (action == 'add'){
                            ...
                            if (barcode){
                                const mes = `добавил ШК: ${barcode} - для товара ${wname} / Код: ${wcode}`
                                $.getJSON('addWUBarcode',{wuid:wuid,barcode:barcode, mes: mes}, function(JSON){
                                    if (!showErr(JSON)){
                            ...
                            if (confirm('Вы действительно желаете изменить выбранный ШК?')){
                                var $tr = $("#tblChgWUB").rf$GetFocus();
                                const barcode = $('#inBar').val()
                                const mes = `изменил ШК: ${$tr.find('td').text()} -> ${barcode} - для товара ${wname} / Код: ${wcode}`
                                $.getJSON('chgWUBarcode',{barcid:$tr.attr('barcid'),barcode:barcode, mes: mes},function(JSON){
                                    if (!showErr(JSON)){
        ...
        function listWaresRecipes(JSON){
            ...
            function AddComponent() {
                ...
                var html = '<form class="buttons" >'+
                  ...
                  (showNetto ? 'Нетто <input type=text class="netto" /><br><br>' : '') +
                  (showNum ? 'Номер <input type=text class="num" /><br><br>' : '') +
                  (showGot ? 'Гот. прод. <input title="Количество готовой продукции" type=text class="got" />' : '') +
                  '<br><br><input id="wares-add" type=hidden value="null" />'+
                ...
                var $dv = $("<div/>").attr("id","dvAddComponent")
                    ...
                    .dialog({closeOnEscape:false,title:'Добавление компонента',autoOpen:true,
                             resizable:false,draggable:false,modal:true,overlay:{opacity:0.5,background:"black"},
                             height:180+weightAdd,width:250})
                    .html(html)
                    .dialog('open')
                    ...
                    if (showNetto){
                      $dv.find('input.netto').kInputFloat();
                    }
                    if (showNum){
                      $dv.find('input.num').kInputInt();
                    }
                    if (showGot){
                      $dv.find('input.got').kInputFloat();
                    }

                    $dv.find('form').unbind('submit').submit(function(){
                        var dv = $('#dvAddComponent');
                        debugger
                        var params = {}
                        params.waresid = dv.find('#wares-add').val();
                        params.amount = dv.find('input.amount').val();
                        params.netto = showNetto ? dv.find('input.netto').val() : params.amount;
                        params.num = showNum ? dv.find('input.num').val() : 0;
                        params.got = showGot ? dv.find('input.got').val() : params.amount;
                        ...
                        if($this.hasClass('component-table')) {
                            $this.find('tbody').append("<tr json='"+$.JStr(params)+"'>"+
                              ...
                              (showNetto ? '<td class="number">'+dv.find('input.netto').val()+'</td>' : '') +
                              (showNum ? '<td class="number">'+dv.find('input.num').val()+'</td>' : '') +
                              (showGot ? '<td class="number">'+dv.find('input.got').val()+'</td></tr>' : ''));
                            dv.dialog('close');
                            ...
                            $.getJSON('addComponent',params,function(JSON){
                                if(!showErr()){
                                    $this.find('tbody').append('<tr id="'+JSON.data.COMPID+'">'+
                                      ...
                                      (showNetto ? ('<td class="number">' + params.netto + '</td>') : '') +
                                      (showNum ? ('<td class="number">' + params.num + '</td>') : '') +
                                      (showGot ? ('<td class="number">' + params.got + '</td>') : '' ) +
                                    '</tr>');
                                    dv.dialog('close');
                        ...
                        return false;
                    }).find('input.wares').click().end()//.kUpDown({selectOnFocus:true});
                    .find('button:last').click(function(){
                      $('#dvAddComponent').dialog('close');
                    });
            }
            ...
            function EditComponent() {
                ...
                var html = '<form class="buttons" >'+
                  ...
                  (showNetto ? 'Нетто <input type=text class="netto" value="'+tr.find('td:eq(3)').text()+'"/><br><br>' : '')+
                  (showNum ? 'Номер <input type=text class="num" value="'+tr.find('td:eq(4)').text()+'" /><br><br>' : '')+
                  (showGot ? 'Гот. прод. <input title="Количество готовой продукции" type=text class="got" value="'+tr.find('td:eq(5)').text()+'" /><br><br>' : '')+
                ...
                var $dv = $("<div/>").attr("id","dvEditComponent")
                    ...
                    $dv.find('#wares-edit').val(tr.find('td:eq(1)').attr('waresid'));
            ...
            function addRecipe(){
                ...
                html += '</select><br><br>'+
                  ...
                  'Объект <select class="object" name="objid">'+ $('#tblObj select[name=objid]').html() + '</select><br>'+
                  '</div>'+
                  '<div class="component" style="width:50%;float:left;">'+
                    '<table class="component-table"><thead>' +
                      '<tr><th colspan="'+ colSpan +'" >Компоненты</th></tr>' +
                      '<tr><th>Код</th><th>Товар</th><th>Кол-во</th>' +
                      (showNetto ? '<th>Нетто</th>' : '')+
                      (showNum ? '<th>Номер</th>' : '') +
                      (showGot ? '<th>Гот. прод.</th>' : '') +
                      '</tr></thead>'+
                      '<tbody></tbody><tfoot><tr><th colspan="'+ colSpan + '">'+
                ...
                var $dv = $("<div/>").attr("id","dvAddRecipe")
                    ...
                    .dialog('open')
                    .find('form').kUpDown({selectOnFocus:true}).end()
                    .find('select.object').val($('#tblObj select[name=objid]').val()).end()
                    ...
                    .find('form').unbind('submit').submit(function(){
                        ...
                        params.objid = $(this).find('select[name="objid"]').val();
                        if(params.name == ''){
                            alert('Введите имя!');
                            return false;
                        }
            ...
            function editRecipe(){
                ...
                html += '</select><br><br>'+
                            ...
                            'Дата с <input type=text class="dateb" /><br><br>'+
                            'Дата по <input type=text class="datee" /><br><br>'+
                            'Объект <select class="object" name="objid">'+ $('#tblObj select[name=objid]').html() + '</select><br><br>'+
                ...
                var $dv = $("<div/>").attr("id","dvEditRecipe")
                    ...
                    .find('.datee').val(row.find('td:eq(6)').text()).end()
                    .find('.object').val(row.find('td:eq(7)').text()).end()
                    .find('select.object').val(row.attr('objid')).end()
                    .find('form').unbind('submit').submit(function(){
                        var params = {};
                        ...
                        params.objid = form.find('select[name="objid"]').val();
                        if(params.name == ''){
                            alert('Введите имя!');
                            return false;
                        }
            ...
            function detailRecipe(){
                $.getJSON('getComponents',{recipeid:$(this).attr('id').substr(6)},function(JSON){
                    html='<table id="component-table"><thead><tr><th colspan="'+ colSpan + '">Компоненты</th></tr>' +
                      '<tr><th>Код</th>' +
                      '<th>Товар</th>' +
                      '<th>Кол-во</th>' +
                      (showNetto ? '<th>Нетто</th>' : '') +
                    ...
                    for(var i=0;i<JSON.data.length;++i){
                      ...
                      html+='<tr id="'+r.COMPID+'">'+
                        ...
                        (showNetto ? '<td class="number netto">'+r.NETAMOUNT+'</td>' : '') +
                        (showNum ? '<td class="number num">'+r.NUM+'</td>' : '') +
                        (showGot ? '<td class="number got">'+r.RESAMOUNT+'</td>' : '')+
                        '</tr>';
                    }
                    html+='</tbody><tfoot><tr><th colspan="' + colSpan + '">'+
                                '<a href="#" class="add"><img title="Добавить" src="'+eng_img+'/actions/add.png" /></a>&nbsp;'+
                                '<a href="#" class="edit"><img title="Изменить" src="'+eng_img+'/actions/edit.png" /></a>&nbsp;'+
                                '<a href="#" class="delete"><img title="Удалить" src="'+eng_img+'/actions/delete.png" /></a>'+
                                '</th></tr></tfoot>'+
