  ...
  $.fn.WaresSelectOptions = function (waresid, options) {
    ...
    function listWaresSelectOptions(json) {
      ...
      var html = '<table data-waresid="' + waresid + '"><thead><tr>\
                            ...
                            <th colspan=10>Место отборки</th>\
                            <th colspan=3>Признаки</th>\
                            </tr><tr>\
                            <th ksort="false">А</th>\
                            ...
                            <th ksort="false" title="Вес тары: только для весовых товаров">ВТ</th>\
                            <th ksort="false" title="Алкогольный товар">Алк</th>\
                            <th ksort="false" title="Ветеринарный товар">Вет</th>' +
      ...
      html += '</tbody><tfoot class="buttons"><tr><th colspan="20">\
      ...
      function selopt() {
        ...
        async.parallel(
          [
            function (cb) {
              $.getJSON('ajaxGetMethod', {}, function (json) {
                cb(json);
              });
            },
            function (cb) {
            ...
            function (cb) {
              $.getJSON('ajaxGetSelAccept', {}, function (json) {
                cb(json);
              });
            }
          ],
          function (res) {
            ...
            $('<div/>').attr("id", "dvSelOpt").addClass("flora")
              ...
              .dialog({
                closeOnEscape: false, title: 'Отборка',
      ...
      function vetSet() {
        if (confirm('Вы действительно хотите изменить ветеринарный признак товара?')) {
          var waresid = $(this).parents('table:first').attr('data-waresid');
          var chk = $(this).attr('checked') ? '1' : '0';
          $.getJSON('vetSet', {wid: waresid, flagset: chk}, function (json) {
            if (!showErr(json)) {
              var $tbl = $('#dvWaresSelectOptions').find('table[data-waresid=' + json.data.WID + ']');
              if ($tbl.length > 0) {
                $tbl.find('tbody>tr>td[data-fld=is_vet]>input:checkbox').each(function () {
                  if (json.data.FLAGSET == '1')
      ...
      function useTaraWeightSet() {
        ...
        var mes = tdTaraWeightText(newVal) + ' - ' + tdTaraWeightTitle(newVal);
        if (confirm('Вы действительно хотите изменить признак использования тары?\n\nНовое значение:\n' + mes)) {
          var waresid = $(this).parents('table:first').attr('data-waresid');
          var whid = $(this).parents('tr:first').attr('data-objid');
          $.getJSON('useTaraWeightSet', {wid: waresid, whid: whid, val: newVal}, function (json) {
            if (!showErr(json)) {
              var $tbl = $('#dvWaresSelectOptions').find('table[data-waresid=' + json.ext_data.WID + ']');
              if ($tbl.length > 0) {
                var newVal = json.data[0].USETARAWEIGHT || '';
                $tbl.find('tbody>tr[data-objid=' + json.ext_data.OBJID + ']>td[data-fld=taraweight]').each(function () {
                  $(this).attr('data-val', newVal).html(tdTaraWeightText(newVal)).attr('title', tdTaraWeightTitle(newVal));
                });
              }
            }
          });
      ...
      function tdTaraWeightTitle(val) {
        if (val == '1')
          return 'Использовать тару';
        else if (val == '0')
          return 'Не использовать тару';
        else if (val == '')
          return 'Взять настройку использования тары с группы отборки';
        else
          return 'Не корректная настройка признака использования веса тары';
      }

      function tdTaraWeightText(val) {
        if (val == '1')
          return 'Да';
        else if (val == '0')
          return 'Нет';
      ...
      function trHTML(tr) {
        return '<td data-fld=enabled><input type=checkbox ' + (tr.ENABLED == '1' ? ' checked' : '') + '></td>' +
          ...
          '<td data-fld="taraweight" data-val="' + (tr.USETARAWEIGHT || '') + '" title="' + tdTaraWeightTitle(tr.USETARAWEIGHT) + '">' + tdTaraWeightText(tr.USETARAWEIGHT) + '</td>' +
          '<td data-fld="is_alco"><input type=checkbox disabled ' + (tr.IS_ALCO == '1' ? ' checked' : '') + '></td>' +
          '<td data-fld="is_vet"><input type=checkbox' + (tr.IS_VET == '1' ? ' checked' : '') + '></td>';
      ...
      function events($trs) {
        $trs.find('>td')
          ...
          .filter("[data-fld=taraweight]").dblclick(useTaraWeightSet).end()
          .filter("[data-fld=is_vet]").find('input:checkbox').click(vetSet).end().end()
          .end();
      }
    }
  }
