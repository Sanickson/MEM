...
(function ($) {
  ...
  function trHTML(tr){
    return '<td class="text tasktype" data-val="' + tr.TTID + '">' + tr.TTNAME + '</td>' +
  ...
  $.workTaskTypeRate = function(json){
    var html = '<table id="tblWTR"><thead><tr>' +
      ...
      '<th ksort="digit" title="Тариф за одно задание">Задание</th>' +
  ...
  function add() {
    var $dv = $('#frmAdd');
    if (!$dv.length) {
      var html = '<div><label class="lbl" for="">Тип задания: </label>' +
