...
;(function($) {
	function tr(json) {
		...
		for (var i=0;i<json.data.length;i++) {
			...
			html += '<tr tuid="'+tr.TUID+'">\
			            <td class="chk"><input type="checkbox" value="' + tr.TUID + '"></td>\
	...
	$.tblMain = function(json) {
        function events($el){
            ...
            if ($el.is('table')) 
                $el.kTblScroll().kTblSorter().kTdChk()
		...
		if (!showErr(json)) {
			var html = '<table id="tblTUnit">\
			             <thead>\
						  <tr>\
						    <th ksort="false" class="chk" title="Печать отмеченных элементов"><input type="checkbox"></th>\
						...
						<tbody>'
			            + tr(json)
			            + '</tbody><tfoot><tr><th class="buttons" colspan="8">\
        ...
        function print() {
            var n = $( "#tblTUnit td input:checked" ).length;

            if (n <= 0){
                alert("Не выбраны элементы справочника для печати");
                return false;
            }
            var wnd = window.open(sp_reports+'/barcode.html');
            var $tr;
            var innerHTML = "";
            var row_number = 2, clc = 1;
            $("#tblTUnit td input:checked").each(function () {
                $tr = $(this).parent().parent();
                innerHTML +='<div class="" style="display: inline-block;">'+
                                '<div class="tuname">'+$tr.find('td:eq(3)').text()+'</div>'+
                                '<div class="barcode" style="">*TU'+$tr.attr('tuid')+'*</div>'+
                                '<div class="turegnum">&nbsp'+$tr.find('td:eq(5)').text()+'</div>'+
                            '</div>';
                clc += 1;
                if (clc>row_number) {
                    innerHTML += '<br>';
            ...
            wnd.onload = function(){
                 wnd.document.getElementById("dvData").innerHTML = innerHTML;
            }
        }
