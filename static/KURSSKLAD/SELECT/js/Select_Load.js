$(document).ready(function(){    
    ...
    $('#whid').change(function(){
        var activeTab = $("#divTopTabs").activeTab();
        if ( activeTab==2 ) {
            $.listClients();
        }
    });
...
;(function($) {
  function formatDate(dt)
  {
    ...
    var dvHTML ='<div id=dvShop class=leftrel>\
                    <div id=dvTop class=leftrel>\
                        <div id=divTopTabs style="text-align:left;">\
                            <ul>\
                                <li><a href="#dvCars"><span>Поставки</span></a></li>\
                                <li><a href="#dvListShop"><span>Клиенты</span></a></li>\
                            </ul>\
                            <div id=dvCars></div>\
                            <div id=dvListShop></div>\
                        </div>\
                    </div>\
                    \
    ...
    function listGateAutoEvents($el){
        var menuName = 'menuTblGateAuto';
        if ($("#"+menuName).length==0)
            $("<ul/>").attr("id",menuName).addClass("contextMenu")
                .html('<li class="edit"><a href="#gateAutoSite">Ворота</a></li>'+
                      '<li class="statusUp"><a href="#gateAutoToSelect">В отборку</a></li>')
        ...
        if ($tr)
            $tr.contextMenu({menu:menuName},function(action, el) {
                if (action=='gateAutoToSelect') {
                   $.getJSON("listGateAutoDoc",{taskid:el.attr('taskid'), status: '0,1'},function(json){
                     if (showErr(json)) return;
                     $.progressDo({
                            arr: json.data,
                            arrNullAlert: 'Документы в статусе формируемая или сформированная в данной погрузке не найдены',
                            url: 'docToSelect',
                            ...
                            funcEnd: function(){
                                ...
                                if ( activeTab==1 ){
                                    $("#dvShopDocs").empty();
                                    $.getJSON("listGateAutoDoc",{taskid:el.attr('taskid')},tblClientDocs);
                                }
                            }
                       });
                   });
                ...
                else if (action=='gateAutoSite') {
                   var taskid = el.attr('taskid');
                   $.getJSON("listGates", {taskid: taskid}, function(json){
                      var html = '<table><thead><tr><th ksort="text">Ворота</th></tr></thead><tbody>';
                      for (var i=0; i<json.data.length; i++){
                        var dI = json.data[i];
                        html += '<tr data-id='+dI.GATEID+'><td class="text">'+dI.GATENAME+'</td></tr>';
                      }
                      html += '</tbody></table>';
                      ...
                      $('<div/>').attr("id","gateAutoSite").attr('data-taskid', json.ext_data.TASKID).addClass("flora")
                        .dialog({height:$(document.body).height()*0.8, width:$(document.body).width()*0.5,
                          title:'Выберите ворота',
                        ...
                        .find('tbody>tr').dblclick(function(){
                          $.getJSON("gateAutoSetSite", {taskid: $('#gateAutoSite').attr('data-taskid'), gateid:$(this).attr('data-id')}, function(json){
                            if (!showErr(json)){
                              $('#gateAutoSite').dialog('close');
                              var gatename = $('#gateAutoSite').find('tbody>tr[data-id='+json.ext_data.GATEID+']>td:first').text();
                              $('#tblGateAuto tbody>tr[taskid='+json.ext_data.TASKID+']>td.gate').text(gatename);
                              $('#tblGateAuto').kTblSorter();
                            }
                          })
                        })
                   });
        ...
        $el.rowFocus({rfSetDefFocus:false,
            rfFocusCallBack:function(){
                ...
                if ( activeTab==1 ){
                    $("#dvShopDocs").empty();
                    $.getJSON("listGateAutoDoc",{taskid:$(this).attr('taskid')},tblClientDocs);
                }
                else if ( activeTab==2 ) {
                    $("#dvShopTaskes").empty();
                    var param = {objid:$(this).attr('objid'),date:$(this).attr('rddate')};
                    $.getJSON("listGateAutoTaskes",{taskid:$(this).attr('taskid')},tblClientTaskes);
                }
            }
        });
        $tr.draggable({cursor: 'crosshair',
            helper:function(event) {
                        return $('<div/>').addClass('helper').html( $(this).find(">td.carlic").text()+' ' +  $(this).find(">td.dt").text()).appendTo( $(document.body) );
            },
            helperPos:'mouse'
        });
        return $el;
    ...
    $.listClients = function (){
        var objid = $("#whid").val();
        if (objid){
            $("#dvShopDocs,#dvShopTaskes").empty();
            $.getJSON("listClients",{objid:objid},function(JSON){
                if (!showErr(JSON)){
                    var html = '<table id="tblClients" style="height: 100%" objid='+JSON.ext_data.OBJID+'><thead><tr>'+
                    ...
                    $("#dvListShop").html(html)
                        .find("table")
                        //.kTblScroll()
                        .Scrollable($("#dvCars").attr("height"),'100%').kTblSorter()
    ...
    function listGateAutoTask(){
        $("#dvShopDocs,#dvShopTaskes").empty();
        $.getJSON("listTaskGateAuto",function(JSON){
            if (!showErr(JSON)){
                var html = '<table id="tblGateAuto"><thead><tr>'+
                                '<th title="Общий статус задания на постановка авто на ворота">Ст</th>'+
                                '<th ksort=text >Авто</th>'+
                                '<th ksort=longDate title="Дата выезда">Дата</th>' +
                                '<th ksort=digit title="Процент выполнения отборки">%</th>' +
                                '<th ksort=digit title="Ворота">Ворота</th>' +
                    '</tr><thead><tbody>';
                for (var i=0; i<JSON.data.length; i++){
                    var tr = JSON.data[i];
                    html += '<tr taskid='+kInt(tr.TID)+'>'+
                                $.tdTaskStatus(tr.TSTATUS)+
                                '<td class="text carlic">'+tr.CARNAME+'</th>'+
                                '<td class="dt">'+kDateTime(tr.TPLANDT)+'</th>'+
                                '<td>'+kFloat(tr.CLOSED/tr.ALLD*100,2)+'</th>'+
                                '<td class="text gate">'+tr.SITENAME+'</th>'+
                            '</tr>';
                }
                html += '</tbody><tfoot><tr><th>'+JSON.data.length+'</th>'+
                            '<th colspan=4 class=buttons style="text-align:right;">'+
                                '<button type=button title=Обновить id=btnGateAutoRefresh><img src='+eng_img+'/actions/refresh.png border=0></button>'+
                            '</th>'+
                        '</tr></tfoot></table>';
                ...
                $("#dvCars").html(html)
                    ...
                    .Scrollable($("#dvCars").attr("height"),'100%').kTblSorter()
                                ...
                                //  .end()
                    .end();
                listGateAutoEvents($('#tblGateAuto'));

                $("#btnGateAutoRefresh").click(listGateAutoTask);
            }
        });
    };   
    ...
    function tblClientDocs(JSON){
        ...
        function tblDocToSelect(JSON,manyDocs) {
            ...
            for (var i=0; i<JSON.data.length; i++){
                ...
                var extNameClass = '';
                if (w.TMNAME){
                  if (kFloat(w.WQUANTITY) - kFloat(w.WREST) > 0.000001)
                    extNameClass = ' error';
                }
                else {
                  extNameClass = ' error';
                ...
                html += '<tr>'+
                            ...
                            '<td class="text'+extNameClass+'">'+w.WNAME+'</td>'+
                            '<td>'+w.SITENAME+'</td>'+
        ...
        function trHTML(JSON){
            return  $.tdDocStatus(JSON.STAT,JSON.STATNAME)+
              // '<td class="text">'+JSON.FROMNAME+'</td>'+
              //'<td class="text">'+JSON.CLNAME+'</td>'+
              '<td class=docdate>'+kDate(JSON.RDOCDATE)+'</td>'+
              '<td class=docnum title="'+JSON.CLNAME+'">'+JSON.DOCNUM+'</td>'+
              '<td>'+kInt(JSON.TPRIORITY)+'</td>'+
        ...
        if ($tbl.length==0){
            ...
            var html = '<table id="tblClientDocs"><thead><tr>'+
                            ...
                            '<th ksort=digit title="Порядок погрузки">ПП</th>'+
    ...
    $.fn.SelectManager = function(){
        ...
        $("#dvTop,#dvShopBottom").css({height:(height-$("#dvShopObj").css('width','100%').height())/2,width:"100%"});
        $("#whid").css('width',$("#dvShop").width()/2);
        ...
        $("#dvTaskBottom").css({height:"100%",width:"100%"});

        var h = height / 2;
        h -= ($("#dvShopTaskes").position().top+$("#dvShopTaskes").height());
        
        $("#dvShopTaskes").css({height:h,"min-height":h}).attr("height",h);
        $("#dvShopDocs").css({height:h,"min-height":h}).attr("height",h);

        h = height / 2;
        h -= ($("#dvCars").position().top+$("#dvCars").height());

        $("#dvCars").css({height:h,"min-height":h}).attr("height",h);
        //$("#dvListShop").css({"height":h});
        $("#dvListShop").css({height:h,"min-height":h}).attr("height",h);
        //$("#dvCars, #dvListShop").css({"height":h,"min-height":h}).attr("height",h);
        ...
        $("#frmFilterTask").bind("submit",function(){
            ...
            drop: function(event, ui) { 
                ...
                $('#dvTaskBottom').empty();
                if ($(ui.draggable).closest('table').attr('id') == 'tblGateAuto'){
                    $.getJSON('listTaskesTid',{taskid: $(ui.draggable).attr('taskid'), carlic: $(ui.draggable).find('>td.carlic').text()},listTaskes);
                    return false;
                }
                else{
                    var $frm = $(ui.element);
                    $("#dBeg,#dEnd").val(formatDate( $(ui.draggable).attr('cldate') ));
                    $("#objName").val( $(ui.draggable).find('>td.objname').text() );
                    $frm.find('input[name="toid"]').val($(ui.draggable).attr('clid'));
                    $frm.find('select[name="whid"]').val($(ui.draggable).closest('table').attr('objid'));
                    $frm.submit();
                }
    ...
    function tblClientTaskes(JSON){
        ...
        for (var i=0; i<lenJSON; i++){
            var tr = JSON.data[i];
            html += '<tr id="trS'+kInt(tr.TID)+'">'+
              (tr.TSTAT ? $.tdTaskStatus(tr.TSTAT,tr.TSTATNAME) : '<td></td>')+
                  '<td>'+tr.TID+'</td>'+
    ...
    function listTaskes(JSON){
        ...
        var html = '<table id="tblTaskes"><thead>' +
            (JSON.ext_data && JSON.ext_data.GATETASK ? '<tr><th colspan="10">Погрузка №'+JSON.ext_data.GATETASK+' автомобиль '+JSON.ext_data.CARLIC+'</th></tr>' : '')+
                        '<tr>'+
        ...
        for (var i=0; i<JSON.data.length; i++){
            ...
            html += '<tr id=trT'+kInt(tr.TID)+' stat='+tr.TSTAT+'>'+
                        ...
                        '<td' + (tr.SGNAME ? ' title="' + tr.SGNAME + '"' : '') + '>' + tr.SGCODE + '</td>' +
                        '<td>'+kInt(tr.TID)+'</th>'+
        ...
        html += '</tbody><tfoot><tr><th colspan=3>'+kInt(JSON.data.length)+'</th>'+
