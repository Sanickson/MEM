    ...
    $.tableMaster = function(JSON){
        if (!showErr(JSON)) {
			...
			html += '</tbody>\
						<tfoot>\
							<tr class="buttons">\
								<th><button type=button id=print><img src="'+eng_img+'/actions/printer.png"/>Печать</button>\
								    <button type=button id=btnExcel><img src="'+eng_img+'/apps/excel.png">Файл</button></th>\
								<th>'+i+'</th>\
								<th colspan="6">&nbsp;</th>\
							</tr></tfoot></table>';
			$("#dvMain").html(html)
