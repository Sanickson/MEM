 ...
 /* $.proveRoute = function (inp) {
  ...
  $.searchLic = function (taskid) {
    ...
    $.ajax({
        ...
        success: function (json) {
            json.data.map((m,i)=>{
                if(i===0) {
                    INP_MASK = m.MASK
                    INP_MASKTRAILER = m.MASKTRAILER
                    INP_PHOLDER = m.PLACEHOLDER
                }
                $('#typeLicense').append(`<option value="${m.MASK}" data-id="${m.ID}" data-masktrailer="${m.MASKTRAILER}" data-code="${m.CODE}" data-placeholder="${m.PLACEHOLDER}">${m.NAME}</option>`)
            })
    ...
    $('#typeLicense').change(()=>{
        ...
        INP_MASKTRAILER = option.attr('data-masktrailer')
          ...
          $.getJSON('listNumTrailer',{truckid: data.truckid, type: 'прицеп'}, function (json) {
            if (!showErr(json) && !showErr(JSON) && !showErr(truck)) {
              ...
              $("<div/>").attr("id", "dvTD").addClass("flora")
                ...
                $("#formTrailer input[name='license']").mask(INP_MASKTRAILER);
