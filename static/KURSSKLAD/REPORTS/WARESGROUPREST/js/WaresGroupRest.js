...
;(function ($) {
    ...
    $.tblWares = function (JSON) {
        ...
        if (!showErr(JSON)) {
            ...
            for (var i = 0; i < JSON.data.length; i++) {
                var tr = JSON.data[i];
                if (kFloat(tr.DOCREST) > 0.0001 || kFloat(tr.LOTREST) > 0.0001 || $('#showEmpty').is(':checked'))
                    html += '<tr id="' + $.kID(trW, tr.WID) + '"' + (tr.ARTICUL ? ' warticul="' + tr.ARTICUL + '"' : '') + '><td class="number">' + (++n) + '</td>' + td(tr) + '</tr>';
            }
            var colspan = 11;
