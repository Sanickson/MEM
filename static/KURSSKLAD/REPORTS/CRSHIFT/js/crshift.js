$(document).ready(function() {
	...
	$('#sysmenu>form').unbind('submit').submit(function(){
		var params = {};
        params.dtbeg = $('#dbeg').val()+' '+$('#tbeg').val()+':00';
        params.dtend = (parseInt($('#tend').val().split(':')[0]) < parseInt($('#tbeg').val().split(':')[0])? kIncDate($('#dbeg').val(),1):$('#dbeg').val()) +' '+$('#tend').val()+':00';
		/*
					...
					$('#dvEmployee')//.html(html)
						...
						.rowFocus({'rfSetDefFocus': false, 
								 'rfFocusCallBack':function() {
                                    ...
                                    const date = $('#dbeg').val();
									params.objid = JSON.ext_data.objid;
									params.dtbeg = date + ' ' + $('#tbeg').val();
									params.dtend = date + ' ' + $('#tend').val();
	...
	function getTasks(JSON) {
		if (!showErr(JSON)) {
			...
			for (var i=0;i<JSON.data.length;i++) {
				...
				html += '<tr taskid="'+tr.TASKID+'" db_id="'+tr.DB_ID+'">'+
