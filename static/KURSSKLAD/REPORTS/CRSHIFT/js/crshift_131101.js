$(document).ready(function() {
	...
	$.getJSON('getObjects',function(JSON){
        for(var i=0;i<JSON.data.length;i++)
            $('#zone').append('<option selected value="'+JSON.data[i].OBJID+'" '+(JSON.data[i].OBJID==JSON.ext_data.curzone?'selected':'')+'>'+JSON.data[i].OBJNAME+'</option>');
	...
	$('#sysmenu>form').unbind('submit').submit(function(){
        ...
        params.dtend = (parseInt($('#tend').val().split(':')[0],10) < parseInt($('#tbeg').val().split(':')[0],10)? kIncDate($('#dbeg').val(),1):$('#dbeg').val()) +' '+$('#tend').val()+':00.0000';
		/*
				...
				if( i < tr.length ) {
					...
					$.getJSON('GetEmployersInfo',params,function(JSON){
						if (!showErr(JSON)) {
							...
							cnttask += td.CNTTASK;
							cntpos += td.CNTPOS;
							sumselect += td.SELECTSUM;
							sumdifftime += td.DIFFTIME;
							sumtimebtwsel += td.TIMEBETWEENSEL;
