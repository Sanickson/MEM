$(document).ready(function () {
    ...
    function drawTable(json) {
        ...
        var html = '<table id="tblData"><thead><tr>' +
            ...
            '<th ksort="text" title="Штрихкод">ШК</th>' +
            '</tr></thead><tbody>'
        $.each(json.data, function () {
            html += '<tr data-waresid=' + this.WID + '>' +
                ...
                '<td class="text">' + this.BARCODES + '</td>' +
                '</tr>';
        });
        html += '</tbody><tfoot><tr>' +
    ...
    $.getJSON('listZoneObjects', function (JSON) {
        if (!showErr(JSON)) {
            ...
            $.getJSON('waresNoBarcodeList', {barcode: 1}, function (json) {
                ...
                $('#allWares').click(e => {
                    ...
                    $.getJSON('waresNoBarcodeAllWares', {barcode: 1}, json => drawTable(json));
                })
            });
        }
    });
