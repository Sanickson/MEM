...
;
(function ($) {
  $.byPeriod = function (json) {
    if (!showErr(json)) {
      var html = '<table id="tbl" whid="' + json.ext_data.whid + '" bdate="' + json.ext_data.bdate + '" edate="' + json.ext_data.edate + '">\
        <thead><tr>\
      ...
      $('#dvMain').html(html)
        ...
        .find('button').click(function () {
          ...
          var obj_text = $('#whid>option[value="' + $('#tbl').attr('whid') + '"]').text();
          var html = 'Заблокированные парии. Склад: ' + obj_text + ' Период: ' + $('#tbl').attr('bdate') + '-' + $('#tbl').attr('edate');
          wnd.onload = function () {
            wnd.document.getElementById("dvHeader").innerHTML = html;
