	...
	*/
    ...
    $('#btnPrint').click(function(){
     ...
     $('#btnExcel').click(function () {
        if (/*$('#dvEmployee').get(0)*/$('#dvEmployee tbody tr').length > 0) {
            ...
            function getEmployeeList($progressbar,tr,i,r) {
				if( i < tr.length ) {
                    ...
                    for ( var j=0; j<rowMetaData.length; ++j ){
                    		var rMDspl = rowMetaData[j].split('№')
												var objName = rMDspl[rMDspl.length-1];
                        html += '<th colspan="2" ksort="digit" title="' + rowMetaData[j] + '">'+ objName +'</th>';
                    }
                    html += '</tr><tr>';
	...
	function getTasks(JSON) {
		if (!showErr(JSON)) {
			...
			for (var i=0;i<JSON.data.length;i++) {
				...
				html += '<tr taskid="'+tr.TASKID+'" dbid="'+tr.DB_ID+'">'+
