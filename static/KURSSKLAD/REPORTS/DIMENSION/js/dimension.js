    ...
    $.tblWares = function(json) {
        if (!showErr(json)) {
            ...
            html += '</tbody><tfoot class="buttons"><tr><th>'+i+'</th><th colspan="3">\
                            <button id=print title="Печать"><img src="'+eng_img+'/actions/printer.png" border="0">Печать</button>\
                ...
                .find('table')
                    ...
                    .find('#print').click(function(){
                     ...
                     $('#btnExcel').click(function () {
                        if ($('#dvWares').get(0)) {
                            export_table_to_excel_name($('#dvWares').get(0), $('#sys-name').text() + ' ' + kNow() + '.xlsx')
                        } else {
