$(document).ready(function () {
    ...
    $("#dvFilter").html('<form id="frmSelectObj"><div class="buttons">' +
        '<font style="font-size: 10pt;">Объект </font>' +
    ...
    $("#frmWaresDocs").find('input').css('text-align', 'center').mask("99.99.9999").val(kToday()).datepicker().end()
        .submit(function () {
            ...
            if (param.wid) {
                ...
                showMes('Внимание', 'Не выбрано ни одного товара');
                return false;
            }
            return false;
        });
...
;(function ($) {
    ...
    $.tblWares = function (wjson) {
        var html = '<table id="tblWares"><thead>' +
            ...
            '<th colspan="2" title="Подготовленный товар">Подготовлено</th>' +
            '<th colspan="2" title="План отборки">Заказ</th>' +
            '<th colspan="2" title="Свободный партионный остаток после завершения отборки">План</th>' +
            '</tr>' +
            ...
            '</thead><tbody>';
        var $progressbar;
        if (wjson.data.length)
            $progressbar = $.progressbar({
                canClose: false,
                minValue: 0,
                maxValue: (wjson.data.length),
        ...
        function events($el) {
            ...
            /*if ($el.is('table')) $el.find('>tfoot>tr>th>button')
                .filter(':first').click(printSpWares)*/

            if ($el.is('tr')) $el.kScrollToTr().rfSetFocus()

            return $el;
        }

        function waresCalc($progressbar, i) {
            if (i < wjson.data.length) {
                $.ajax({
                    url: 'waresByWares',
                    dataType: 'json', async: true,
                    data: {objid: $('#selectObj').val(), waresid: wjson.data[i].WID},
                    success: function (json) {
                        var w = json.data;
                        html += '<tr id=trW_' + w.WID + '>' +
                            '<td class="number">' + (i + 1) + '</td>' +
                            '<td class="text">' + w.WGNAME + '</td>' +
                            '<td class="number">' + w.WCODE + '</td>' +
                            '<td class="text">' + w.WNAME + '</td>' +
                            '<td>' + w.MAINUCODE + '</td>' +
                            '<td>' + w.VIEWUCODE + '</td>' +
                            '<td>' + viewQuantity(w.DOCREST, w.VIEWUFACTOR, w.VIEWUCODE, w.MAINUFACTOR, w.MAINUCODE) + '</td>' +
                            '<td class="number">' + kNumber(w.DOCREST) + '</td>' +
                            '<td>' + viewQuantity(w.LOTREST, w.VIEWUFACTOR, w.VIEWUCODE, w.MAINUFACTOR, w.MAINUCODE) + '</td>' +
                            '<td class="number">' + kNumber(w.LOTREST) + '</td>' +
                            '<td>' + viewQuantity(w.SELREADY, w.VIEWUFACTOR, w.VIEWUCODE, w.MAINUFACTOR, w.MAINUCODE) + '</td>' +
                            '<td class="number">' + kNumber(w.SELREADY) + '</td>' +
                            '<td>' + viewQuantity(w.SELBEG, w.VIEWUFACTOR, w.VIEWUCODE, w.MAINUFACTOR, w.MAINUCODE) + '</td>' +
                            '<td class="number">' + kNumber(w.SELBEG) + '</td>' +
                            '<td>' + viewQuantity((kFloat(w.LOTREST) - kFloat(w.SELBEG) - kFloat(w.SELREADY)), w.VIEWUFACTOR, w.VIEWUCODE, w.MAINUFACTOR, w.MAINUCODE) + '</td>' +
                            '<td class="number">' + kNumber(kFloat(w.LOTREST) - kFloat(w.SELBEG) - kFloat(w.SELREADY)) + '</td>' +
                            '</tr>';
                        waresCalc($progressbar.trigger('progressinc'), ++i);
                    },
                    ...
                    error: function (jqXHR, status, errorThrown) {
                        if (jqXHR.status == 403) {
                            ...
                            waresCalc($progressbar, i)
                        } else location.reload();
                    }
                });
            } else
                html += '</tbody><tfoot><tr><th сlass="buttons" colspan="16"> ' +
                    '<button type=button id=btnExcel><img src="' + eng_img + '/apps/excel.png">Файл</button>' +
                    '</th</tr></tfoot></table>';
                ...
                $('#btnExcel').click(function () {
                    if ($('#tblWares').get(0)) {
                        export_table_to_excel_name($('#tblWares').get(0), $('#sys-name').text() + ' ' + kNow() + '.xlsx')
                    } else {
        ...
        $('#dvWaresLotIncomes, #dvWaresCargo, #dvWares').empty();
        waresCalc($progressbar, 0);
        //waresCalc(0,0);
    }
