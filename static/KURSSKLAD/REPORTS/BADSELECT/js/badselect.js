    ...
    $.badSelect = function (JSON) {
        if (!showErr(JSON)) {
            ...
            html += '</tbody><tfoot class="buttons"><tr>' +
                '<th>' + i + '</th>' +
                '<th colspan="4"><button type="button" id=print title="Печать"><img src="' + eng_img + '/actions/printer.png" border="0">Печать</button>' +
                '<button type=button id=btnExcel title="Файл"><img src="' + eng_img + '/apps/excel.png">Файл</button></th>' +
