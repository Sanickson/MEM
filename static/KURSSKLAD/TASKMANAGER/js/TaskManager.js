$(document).ready(function(){
    ...
    var $taskManagerView = $('#divTblData').taskManagerView({
    ...
    $("#frmFilter").submit(function(){
        ...
        */
        $taskManagerView.listTask(param);
        return false;
    });
    ...
    $("#frmScan").submit(function(){
		...
		$.getJSON('scan',param,function(json){
			if (!showErr(json)) {
                $taskManagerView.listTask(json.ext_data);
			}
		});
        return false;
    });
      ...
      confirm: function() {
          $.getJSON('deltask',{taskid:$row.attr('taskid')},function(JSON){
              if(!showErr(JSON)){
                  ...
                  var $th = $tbl.find('>tfoot>tr>th:last');
