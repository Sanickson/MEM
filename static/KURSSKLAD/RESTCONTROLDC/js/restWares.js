...
;(function($){
    ...
    function restWares(objid,zoneid){
        ...
        $.getJSON('qRestWares',{objid:objid,zoneid:zoneid},function(JSON){
            if (!showErr(JSON)){
                var REST = 0;
                var html = '<table id="tblRS" objid='+JSON.ext_data.OBJID+' zoneid='+JSON.ext_data.ZONEID+'><thead>'+
                                '<tr><th colspan=3>Товар</th><th colspan=3>'+getTitle(JSON.ext_data.OBJID,JSON.ext_data.ZONEID)+'</th></tr>'+
