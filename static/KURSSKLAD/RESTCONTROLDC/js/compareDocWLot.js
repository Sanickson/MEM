...
;(function($){
    ...
    function docWLWares(objid){
        ...
        $.getJSON('docWLWares',{objid:objid},function(JSON){
            if (!showErr(JSON)){
                var d = 0, l = 0, t = 0, a = 0;
                var html = '<table id="tblCompare" objid='+JSON.ext_data.OBJID+'>'+
                ...
                html += '</tbody><tfoot><tr>'+
                            ...
                            '<th class=buttons>'+
                                ...
                                '<button type=button title="Файл" id=btnExcel><img src='+eng_img+'/apps/excel.png border=0></button>'+
                        ...
                        '</tr></tfoot></table>';
                $dvDocWLWares.html(html).find('>table').kTblScroll().tablesorter().tblCompareEvents()
                    .find('>tfoot>tr>th>button')
                        ...
                        .filter(":eq(2)").click(excelRestWares).end()
                        .filter(":eq(3)").click(exportRest).end()
                    .end()
    ...
    function excelRestWares() {
        if ($('#tblCompare tbody tr').length > 0) {
            export_table_to_excel_name($('#tblCompare').get(0), $('#sys-name').text() + ' ' + kNow() + '.xlsx')
        } else {
    ...
    $.fn.tblCompareEvents = function(){
        var contextFundAdd = opt('view', null, 'contextFundAdd');
		function menu(action, el){
			if (action == 'fundAdd')
                fixRestCompare($("#tblCompare").rf$GetFocus().attr('waresid'),
                               $("#tblCompare").rf$GetFocus().find('td:eq(0)').text());
		};

		var mId = 'menuTblCompare';
		if ($("#"+mId).length==0){
				$("<ul/>").attr("id",mId).addClass("contextMenu").css("width","200px")
					.html('<li class="edit"><a href="#fundAdd">Выровнять расхождения</a></li>')
	...
	function fixRestCompare(wid, wcode) {
        $.getJSON('fixRestCompare', {wid: wid, wcode: wcode},
                        function(json) {
                        if (!showErr(json))
                            alert("Операция выравнивания выполнена");
                    });
    }
