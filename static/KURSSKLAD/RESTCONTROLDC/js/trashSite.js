...
;(function($){    
    ...
    function delPallet() {
        var $tr = $(this);
        $.getJSON('delPallet',{pid: $(this).attr('palletid')},function(json) {
            if (!showErr(json)) {
                $tr.hide()
                    .parents('table:first')
    ...
    function pallets(objid,zoneid){
        ...
        if (!$("#"+mId).length) {
            $("<ul/>").attr("id",mId).addClass("contextMenu").css("width","190px")
                .html('<li class="delete"><a href="#delPallet">Расформировать</a></li>')
            .appendTo($(document.body));
        }

        $.getJSON('qTrashSitePallets',{objid:objid,zoneid:zoneid},function(JSON){
            if (!showErr(JSON)){
                var html = '<table id="tblRS" objid='+JSON.ext_data.OBJID+' zoneid='+JSON.ext_data.ZONEID+'><thead><tr>'+
                ...
                for (var i=0; i<JSON.data.length; i++){
                    var tr = JSON.data[i];
                    html += '<tr palletid="'+tr.PID+'" class="'+(tr.PSTAT=='0'?'delpallet':'')+'">'+
                ...
                $dvTrashSite.find('>div:first').html(html)
                    ...
                    .find('tr.delpallet').contextMenu({menu:mId},function menu(action, el) { eval(action+'.call($(el))') });
            }
        });
    };
