(function(jQuery){
    ...
    {   // defaults
        ...
        function countDoc(data){
            ...
            $.getJSON('getReports',{docid:options.docid, dbid: options.dbid},function(JSON){
                if(countDoc(JSON.data)==(options.docid.split(',').length-1 || ((options.docid.split(',').length == 1) && countDoc(JSON.data) == 1))) {
                    for(var i=0;i<JSON.data.length;++i) {
                        ...
                        for(var j=0;j<JSON.data[i].DOCS.length;j++)
                            ...
                            db+= options.dbid+',';
                        if(docs!='')
                            window.open('report?docid='+docs+'&reportid='+JSON.data[i].REPORTID+'&dbid='+db);
                    }
                    $.unblockUI();
                    return;
                }
                ...
                function printDocs() {
                    ...
                    for(i=0;i<reports.length;++i) {
                        ...
                        table.find('tr[reportid="'+reports[i]+'"] td>input:checked').each(function(){
                            ...
                            db += $(this).parent().parent().attr('dbid')+',';
                        });
                        if(docs!=''){
                            window.open('report?docid='+docs+'&reportid='+reports[i]+'&dbid='+db);
                            if(typeof(options.callback) == 'function') options.callback(options.docid, options.dbid);
                        }
                    }
                    return true;
                }
                ...
                for(var i=0;i<JSON.data.length;++i) {
                    ...
                    for(var j=0;j<JSON.data[i].DOCS.length;j++) {
                        ...
                        html+='<tr reportid="'+JSON.data[i].REPORTID+'" dbid="' + doc.dbid + '"><td>O'+doc.docid+'</td><td>'+doc.docdate+'</td><td>'+doc.from+'</td><td>'+doc.to+'</td><td><input docid='+doc.docid+' type=checkbox></td></tr>';
                    }
                    html+='</tbody></table>';
                }
                //html+='</tbody></table>';
