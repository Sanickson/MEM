(function(jQuery){
    ...
    function TaskServerStatusImg(status){
        switch (status) {
            case '0':
                return sps_img.KURSSKLAD+'/status/doc/controlling.png';
            case '1':
                return sps_img.KURSSKLAD+'/status/task/2.png';
            case 'E':
               return sps_img.KURSSKLAD+'/status/task/i.png';
            case 'L':
                return sps_img.KURSSKLAD+'/status/doc/checked.png';
            case 'P':
                return sps_img.KURSSKLAD+'/status/doc/forming.png';
            case 'R':
                return sps_img.KURSSKLAD+'/status/task/1.png';
            case 'S':
                return sps_img.KURSSKLAD+'/status/task/0.png';
            case 'Z':
                return sps_img.KURSSKLAD+'/status/doc/formed.png';
        }
    };

    function TaskServerStatusTitle(status,title){
        if (title) return title;
        else switch (status){
            case '0': return 'Ожидающее';
            case '1': return 'Выполненное успешно';
            case 'E': return 'Выполненное с ошибкой';
            case 'L': return 'Заблокированное';
            case 'P': return 'Выполненное частично';
            case 'R': return 'Выполняемое';
            case 'S': return 'Отложенное';
            case 'Z': return 'Пустое';
        }
        return '';
    }

    $.tdTaskServerStatus = function(status,title){
        return '<td class="status" title="'+TaskServerStatusTitle(status,title)+'"><img src="'+TaskServerStatusImg(status)+'" border="0"></td>';
    };

    $.fn.tdTaskServerStatus = function(status,title){
        return this.attr('title',TaskServerStatusTitle(status,title)).html('<img src="'+TaskServerStatusImg(status)+'" border="0">');
    };
