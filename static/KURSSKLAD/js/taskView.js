...
(function ($) {
  ...
  function tblTask(opt, data) {
    ...
    if (opt.taskTrFunc && typeof opt.taskTrFunc == 'function')
      ...
      tblTaskTr = function (data) {
        return '<tr taskid="' + data.TID + '" status="' + data.TSTAT + '" dbid="' + data.DB_ID + '">' +
          ...
          '<td class="number tweight">' + kFloat(data.TWEIGHT, 3) + '</td>' +
          '<td class="number tcapacity">' + kFloat(data.TCAPACITY, 3) + '</td>' +
          '<td class="number tcntunits">' + kInt(data.TCNTUNITS) + '</td>' +
          '<td class="number tcntunitsw">' + kFloat(data.TCNTUNITSW, 3) + '</td>' +
          '</tr>';
      }
    ...
    var sWeight = 0, sCapacity = 0, sCntUnits = 0, sCntUnitsW = 0;
    for (var i = 0; i < kolData; i++) {
      html += tblTaskTr(data[i]);
      sWeight += kFloat(data[i].TWEIGHT);
      sCapacity += kFloat(data[i].TCAPACITY);
      sCntUnits += kInt(data[i].TCNTUNITS);
      sCntUnitsW += kFloat(data[i].TCNTUNITSW);
    }
    ...
    html += '</tbody><tfoot><tr><th colspan="4" class="buttons">\
                    ...
                    <th colspan="4">' + kolData + '</th>' +
      '<th>' + kFloat(sWeight, 3) + '</th>' +
      '<th>' + kFloat(sCapacity, 3) + '</th>' +
      '<th>' + kInt(sCntUnits) + '</th>' +
      '<th>' + kFloat(sCntUnitsW, 3) + '</th>' +
      '</tr></tfoot></table>';
  ...
  function setWorker(tid, eid, opt, fl) {
    ...
    $.getJSON('setWorker', params, function (JSON) {
      if (!showErr(JSON)) {
        $('#' + opt.id + 'tblTaskes>tbody>tr[taskid="' + JSON.ext_data.taskid + '"]')
          .find('>td.employee').text((JSON.data.EMPLNAME ? JSON.data.EMPLNAME : ''))
      }
    });
  }
  ...
  function tblTaskPallet(opt) {
    ...
    var params = {taskid: $(this).attr('taskid')};
    if ($(this).attr('db_id'))
      params['db_id'] = (this).attr('db_id');

    $.getJSON(opt.tpallet, params, function (JSON) {
      if (!showErr(JSON)) {
  ...
  function tblTaskWares(opt) {
    ...
    $.getJSON(opt.twares, params, function (JSON) {
      if ($('#' + opt.id + 'tblTaskWaresLot').length)
      ...
      if (!showErr(JSON)) {
        ...
        if (JSON.ext_data.KBEGINTIMECAPTION) {
          html += '<th rowspan=2>' + JSON.ext_data.KBEGINTIMECAPTION + '</th>';
          cntPal++;
        }
        if (JSON.ext_data.KENDTIMECAPTION) {
          html += '<th rowspan=2>' + JSON.ext_data.KENDTIMECAPTION + '</th>';
          cntPal++;
        }
        if (JSON.ext_data.KPALLETCAPTION) {
          html += '<th colspan=2>' + JSON.ext_data.KPALLETCAPTION + '</th>';
          pallet++;
        }
        if (JSON.ext_data.KQUANTITYCAPTION) {
          html += '<th colspan=' + (1 + opt.useviewunit) + '>' + JSON.ext_data.KQUANTITYCAPTION + '</th>';
          cntTH++;
        }
        if (JSON.ext_data.KSCANCOUNTCAPTION) {
          html += '<th colspan=' + (1 + opt.useviewunit) + '>' + JSON.ext_data.KSCANCOUNTCAPTION + '</th>';
          cntTH++;
        }
        if (JSON.ext_data.KSUCCESSSCANCAPTION) {
          html += '<th colspan=' + (1 + opt.useviewunit) + '>' + JSON.ext_data.KSUCCESSSCANCAPTION + '</th>';
          cntTH++;
        }
        if (JSON.ext_data.KSCANQUANTCAPTION) {
          html += '<th colspan=' + (1 + opt.useviewunit) + '>' + JSON.ext_data.KSCANQUANTCAPTION + '</th>';
          cntTH++;
        }
        if (cntTH > 6) {
          html += '</tr><tr>';
