...
function Site(config,data){    
    ...
    if( checkInt(config.x) ) this.x = config.x;
    else this.x = 1;
        
    if( checkInt(config.y) ) this.y = config.y;
    else this.y = 1;     
    
    if( checkInt(config.z) ) this.z = config.z;
    else this.z = 1;        
    
    if( checkInt(config.addTop) ) this.addTop = parseInt(config.addTop,10);
    else this.addTop = 0;
        
    if( checkInt(config.addLeft) ) this.addLeft = parseInt(config.addLeft,10);
    else this.addLeft = 0;
    
    if( checkInt(config.swidth) ) this.swidth = config.swidth;
    else this.swidth = 1;           
        
    if( checkInt(config.slength) ) this.slength = config.slength;
    else this.slength = 1;        
    
    if( checkInt(config.sheight) ) this.sheight = config.sheight;
    else this.sheight = 1;        
    
    if( config.spcode != undefined && config.spcode ) this.spcode = config.spcode;
    else this.spcode = false;
        
    if( checkInt(config.siteid) ) this.siteid = config.siteid;
    else this.siteid = false;        
        
    if( config.sname != undefined && config.sname ) this.sname = config.sname;
    else this.sname = false;
        
    if( config.borderDrow != undefined && config.borderDrow ) this.borderDrow = true;
    else this.borderDrow = false;            
    
    if( checkInt(config.borderWidth) ) this.borderWidth = config.borderWidth;
    else  this.borderWidth = 1;
        
    if( data != undefined && data && data.length > 0 ) this.child = data;
    else this.child = [];
    ...
    this.drawSiteXY = function($dv,bLen){
        ...
        }).mouseout(function(){statusBar.text('')});
