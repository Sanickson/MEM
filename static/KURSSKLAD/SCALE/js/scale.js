...
$(function () {
        ...
        $.getJSON('GetScales', {
            ...
            if (data.data.length) {
                ...
                for (var i = 0; i < data.data.length; i++) {
                    html += GetRowScale(data.data[i].EQUIPMENTID, data.data[i].SCALETYPE, data.data[i].SCALETYPENAME, data.data[i].NAME, data.data[i].SERIALNUM);
                }
            } else {
    ...
    function ShowAddWInSetId () {
        $('#addwinsetid').show().unbind('click').bind('click', function () {
            AddMasterAction();
        });
    }
    ...
    function RowMasterDrow (data, id) {
        var row = '<tr id="' + data.WINSETID + '"><td class="num">' + (vscaletype.indexOf('SCALESS') != -1 ? data.NUMPOSITION : RowMasterCount) + '</td>' + '<td class="Name">' + data.WARESNAME + '</td>' + '<td class="Code">' + (data.WARESCODE || '') + '</td>' + '<td class="Code">' + (data.UNITNAME || '') + '</td>' + '<td class="GroupName">' + data.GROUPNAME + '</td>' + '<td class="WhName text">' + data.WHNAME + '</td>' +
        //'<td class="ProducerName">'+data.SETIDNAME+'</td>'+
        '<td class="Checkbox"><input type="checkbox" /></td>' + '</tr>';
    ...
    function RowMasterDrowOnButtons (data, id) {
        ...
        var row = '<tr id="' + data.WARESID + '"><td id="num-' + num + '" class="num">' + num + '</td>' + '<td class="Name">' + data.NAME + '</td>' + '<td class="Code">' + data.CODE + '</td>' + '<td class="ProducerName">' + data.PRODUCERNAME + '</td>' + '<td class="GroupName">' + data.GROUPNAME + '</td>' + '<td class="GroupName">' + data.SUBGROUPNAME + '</td>' + '<td class="SalePrice">' + data.SALEPRICE + '</td>' + '</tr>';
        if (id) {
            th.replaceWith(row);
        } else {
            $("#tmaster-body").append(row);
    ...
    function DlgAMAEmptyResults () {
        $("#t-results > tbody").empty();
    }

    function DlgAddMasterAction (id) {
        $('#r-search').unbind("change").bind("change", function () {
            DlgAMAEmptyResults();
        });
        $("#t-search").val('');
        ...
        $('#a-search').unbind("click").bind("click", function () {
            WaresOnSearch(id);
        });
        DlgAMAEmptyResults();
        if (id) {
    ...
    function WaresOnSearch (id) {
        ...
        if (($("#t-search").val().length > 0 && $("#t-search").val().length <= 80) || (mode != 'wares')) {
            $.getJSON('WaresSearch', {
                strsearch: $("#t-search").val(),
                scaleid: vscaleid,
                mode: mode
            }, function (data) {
                WaresListDrow(data, id);
            });
        }
    }
    ...
    function WaresOnSearchOnButtons (id) {
        ...
        if ($("#t-search").val().length > 0 && $("#t-search").val().length <= 80) {
            $.getJSON('WaresSearchOnButtons', {
                strsearch: $("#t-search").val(),
                scaleid: vscaleid
            }, function (data) {
                WaresListDrowOnButtons(data, id);
            });
        } else {
    ...
    function WaresListDrow (data, id) {
        for (var i = 0; i < data.data.length; i++) {
            $('#t-results > tbody').append('<tr id="t-results-' + data.data[i].ID + '">' + '<td class="num">' + (i + 1) + '</td>' + '<td class="Name">' + data.data[i].NAME + '</td>' + '<td class="Code">' + data.data[i].CODE + '</td>' + '<td class="dlgWaresGroup">' + data.data[i].GROUPNAME + '</td>' + '<td class="CheckBox"><input type="checkbox" disabled ' + ((data.data[i].INSCALE == '1') ? ('checked') : ('')) + '></input></td>' + '<td class="Image"><a class="Save" href="#"><img src="' + eng_img + '/actions/save.png" title="Сохранить" alt="Сохранить" /></a></td>' + '</tr>');
        }
        $("#t-results").tablesorter({
            widgets: ['zebra'],
            headers: {
                0: {
                    sorter: "integer"
        ...
        $("#t-results > tbody > tr a.Save").unbind("click").bind("click", function () {
            ...
            if (!$('#' + tr_id + ' > td.CheckBox > input:checkbox').attr("checked")) {
                ...
                $.getJSON('AddWInSetOnScale', {
                    scaleid: vscaleid,
                    id: tr_id.split('-')[2],
                    mode: $(':radio[name="r-search"]:checked').val()
                }, function (data) {
                    if (data.data[0].ERRMES) {
                        alert(data.data[0].ERRMES);
                    } else {
                        if (id) {
                            WaresOnSearch(id);
    ...
    function WaresListDrowOnButtons (data, id) {
        ...
        $("#t-results > tbody > tr a.Save").unbind("click").bind("click", function () {
            var tr_id = $(this).parents('tr').attr('id');
            if (!$('#' + tr_id + ' > td.CheckBox > input:checkbox').attr("checked")) {
                $.getJSON('AddWaresOnScale', {
                    scaleid: vscaleid,
                    id: tr_id.split('-')[2],
                    numposition: ( id ? vnumposition : 0)
                }, function (data) {
                    if (id) {
    ...
    function DeleteWaresOnButtons (id) {
        $.getJSON('DeleteWares', {
            ...
            if (!showErr(data)) {
                $('#' + id + ' td[class!="num"]').empty();
            }
        });
    }

    function DeleteWares (id) {
        $.getJSON('DeleteWInSet', {
            winsetid: id
        }, function (data) {
            if (!showErr(data)) {
                $('#' + id).remove();
    ...
    function ExportMasterAction () {
        ...
        if (vscaleid && vscaleid != 'n') {
            ...
            if (scaletype != 'SCALEMASSA') {
                $.getJSON('GetScaleWares', {
                    scaleid: vscaleid,
                    objid: $('#shopid').val(),
                    scaletype: $("#scaleid option:selected").attr('scaletype')
                }, function (data) {
                ...
                window.open('GetScaleWaresMassa?scaleid=' + vscaleid);
            }
            /*}
             else{
    ...
    function GetWaresInfo (row) {
        if (rowindex < WaresCount && $("#dvProgress").filter(":visible").length > 0) {
            $.getJSON('GetScaleWaresInfo', {
                waresid: row.WARESID,
                scaleid: vscaleid,
                scaletype: $("#scaleid option:selected").attr('scaletype'),
                isdonetsk: row.ISDONETSKREGION
            }, function (data) {
            ...
            if (rowindex >= WaresCount) {
                ...
                $(this).showConf({
                    text: 'Файл загружен неполностью! Сохранить?',
                    confirm: function () {
                        $.getJSON('GetScaleLinkFile', function (data) {
                            location.href = data.ext_data.linkfile;
                        });
    ...
    function AddScale () {
        if ($("#dialog-addscale").length == 0) {
            ...
            $('#dialog-addscale').dialog({
                ...
                $('#new-scale-cansel').unbind('click').bind("click", function () {
    ...
    function initWaresOnButtonsData (full, id) {
        ...
        if (id)
            $('#ss-tmaster-body').rfSetFocus('#' + id);
        //$('#tmaster').width('100%');
        $('#ss-tmaster').tablesorter().Scrollable('400', '100%');
        if (id) {
            $('#' + id).kScrollToTr();
        }
    }
    ...
    function ContextMenuMasterInit (id) {
        $("#" + id).contextMenu({
            ...
            if (action == 'add')
                AddMasterAction();
            if (action == 'delete')
                DeleteMasterAction(el.attr('id'));
            if (action == 'multidelete')
                MultiDeleteMasterAction();
            if (action == 'showdetail')
                ShowDetailMasterAction();
            if (action == 'priceprint')
                PricePrintAction();
            if (action == 'export')
                ExportMasterAction();
        });
                ...
                if (!showErr(json) && json.data.length) {
                    ...
                    wnd.onload = function () {/*var html = '';
                        ...
                        for (var i = 0; i < json.data.length; i++) {
                            ...
                            '<td class=Code>' + json.data[i].CODE + '</td>' + '<td class=Name>' + json.data[i].NAME + '</td>' + '<td class=SalePrice>' + json.data[i].SALEPRICE + '</td>' + '</tr>';
                        }
                        html += '</tbody>';
    ...
    function ContextMenuWaresOnButtonsInit (id) {
        $("#" + id).contextMenu({
            menu: 'cm-waresonbutton'
        }, function (action, el, pos) {
            if (action == 'edit')
                EditMasterAction(el.attr('id'));
            if (action == 'clean')
                CleanMasterAction(el.attr('id'));
            if (action == 'export')
                ExportWaresOnButtons();
            if (action == 'print')
                PrintMasterAction();
            if (action == 'printbtn')
                PrintBTNMasterAction();
        });
    }
    ...
    function PrintBTNMasterAction () {
        ...
        if (vscaletype == 'SCALESSCL') {
            ...
            wnd.onload = function () {
                ...
                list1btn = 0;
                list2btn = 0;
                $('#ss-tmaster tbody tr').each(function () {
    ...
    function RowDetailDrow (data) {
        var row = '<tr id="' + data.WARESID + '"><td class="num">' + RowDetailCount + '</td>' + '<td class="Name">' + data.NAME + '</td>' + '<td class="Code">' + data.CODE + '</td>' + '<td class="ProducerName">' + data.PRODUCERNAME + '</td>' + '<td class="GroupName">' + data.SUBGRNAME + '</td>' + '<td class="SalePrice">' + data.SALEPRICE + '</td>' + '</tr>';
    ...
    function initDetailData (full, id) {
        ...
        if (id)
            $('#tdetail-body').rfSetFocus('#' + id);
        //alert($('#tdetail').width());
        //$("#tdetail").width($("#tmaster").width());
        $("#tdetail").css('width', '100%').tablesorter().Scrollable('300', '100%');
        if (id)
            $('#' + id).kScrollToTr();
    }
    ...
    function AdminPermission () {
        $("#addwares").unbind('click').bind('click', function () {
            AddScale();
        });
        $("#loadallwares").unbind('click').bind('click', function () {
            LoadAllWaresOnScale();
        });
    }
    ...
    function GetScaleWaresOnButtons () {
        ...
        if (vscaleid != 'n') {
            Block('Загрузка состава весов...');
            $.getJSON('GetScaleWaresOnButtons', {
                'scaleid': vscaleid
            }, function (res) {
                $('#ss-master').show();
                RowOnButtonsCount = 0;
    ...
    function Start () {
        ClearMainData();
        $('#editbuttons').hide();
        $("#shopid").unbind('change').bind('change', function () {
            ScalesReload();
        });
        $("#editbuttons").unbind('click').bind('click', function () {
            GetScaleWaresOnButtons();
        });
        ...
        $("#scaleid").change();
        $("#loadwares").unbind('click').bind('click', function () {
            GetScaleWares();
        });
        $("#printwares").remove();
        $("#user-actions").append('<a href="#" id="printwares"><img src="' + eng_img + '/actions/printer.png" title="Печать товаров" alt="Печать товаров" /></a>');
        $("#printwares").unbind('click').bind('click', function () {
            PricePrintAction();
        });
        $("#refreshscale").unbind('click').bind('click', function () {
            RefreshScale();
        });
        $("#loadfile").unbind('click').bind('click', function () {
            ExportMasterAction();
        });
        $("#tmaster > thead > tr :checkbox").unbind('click').bind('click', function () {
            CheckAll();
        });
        var m = $('#user-message');
        $('#user-message').remove();
        $('#ms-Filter').append(m);
        $('#div-actions, #admin-actions, #user-actions').css({
            'position': 'relative',
            'float': 'left',
            width: '100%',
            'text-align': 'center'
