...
def get_mapped_sysinfo(fullhref):
    ...
    """
    res = {}
    for key, value in list(systems_params.items()):
        if value['system_full_ref'] == fullhref:
            res = {'id_system': key, 'id_base': value['id_base']}
            break
    return res

def get_id_base_by_id_system(id_system=None, use_cache=False, conEngine=None):
    """Получение ID базы данных, с которой работает система. ID БД запрашиваемой системы ищется как
    ...
    :type id_system: int
    :param use_cache: возвращать результат из кэша или перечитывать из БД (поскольку вызов функций API, использующих эту функцию, возможен вне веб-интерфейсов, по умолчанию полагаем=False)
    :type use_cache: bool
...
def find_id_system_by_full_ref_name(fullhref, use_cache=False, conEngine=None):
...
def get_full_ref_name_by_id_system(id_system=None, use_cache=False, conEngine=None):
...
def get_full_folder_name_by_id_system(id_system=None, use_cache=False, conEngine=None):
...
def get_id_system(level=0, none_if_not_found=False):
    """Возвращает текущий id_system. Для этого ищет параметр self в стеке вызовов начиная с уровня level.
...
def get_systems(higher=None, recurse=1, systems_order=None, spaces_per_level=24, conEngine=None):
    """Дерево всех замапленных систем.
...
class Mappings(object):
    ...
    def mapSystem(self, mapTo, item, pathPrefix):
        ...
        sys.stdout.write(str(self.cntSystems) + ") " + item['REF_NAME'] + " (" + pu.convToConsole(item['SHOW_NAME']) + ")")  # CLASS_NAME может не быть

        # default-версия каждой подсистемы
        version = None
        # читаем VERSION
        if item['FOLDER_NAME']:
            try:
                exec("from %s import VERSION as version" % (pathPrefix))
                if version:
                    sys.stdout.write(" v. " + str(version))
            except ImportError:
                pass
        sys.stdout.write("\n")
    ...
    def find_map_obj(self, full_ref, from_obj=None, from_ref='/'):
        """Поиск объекта маппинга по полному внутреннему URL.
        ...
        # Убираем слэш
        if len(full_ref) > 0 and full_ref[-1] == '/':
            full_ref = full_ref[:-1]  # w/o slash

        if from_obj is None:
            from_obj = self.root

        if full_ref == from_ref or full_ref == '' and from_ref == '/':
            found_obj = from_obj
        else:
