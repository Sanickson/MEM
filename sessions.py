...
def set_sessions_handlers():
    ...
    #     for id, (data, expiration_time) in self.cache:
...
def session_open(uid, fio, equip_kind, termnum=None):
    """Открытие pc-, терминальной или мобильной сессии. Предварительно выполняет закрытие старой сессии и очистку истории за
...
def session_close(uid, fio, equip_kind, reason, closed_by=None, conEngine=None):
    """Закрытие pc-, терминальной или мобильной сессии по пользователю.
...
def session_update(uid, fio, equip_kind, ip_addr, user_agent, id_system):
    """Обновление параметров pc-, терминальной или мобильной сессии по пользователю.
...
def session_last_session_info(uid, fio, equip_kind):
    """Информация о последней открытой или закрытой pc-, терминальной или мобильной сессии.
    equip_kind - вид оборудования: 'T'-терминал, 'M'-мобильне устройство, 'P' и другое -PC
